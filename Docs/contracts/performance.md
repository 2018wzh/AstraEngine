...
`windows_media_performance_budget` 是当前第一个产品 budget owner。`WindowsNativeMediaSession::open` 强制接收 performance identity 和 budget；profile identity 不一致时，在打开 decoder、surface output 或 audio session 前失败。Session 测量 open、tick、audio status、scheduler、present、pump、queue 深度、live payload bytes、WASAPI underflow、dropped video 和 device recovery，并在 shutdown 完成资源释放后生成 report。
...
普通 debug test 只验证真实采样、身份连续性和 blocked/pass 两种语义。正式 E3 性能放行必须由 release build 在声明的参考环境运行，且 pass report 与 capability、conformance、Player/package session 共享 identity。当前这条同 run release 接线尚未完成，因此不能把 contract 或 debug report 当成性能门禁已经关闭。
