...
Migration 12 已切换到 `astra.target_manifest.v2` 与 `vn.compiled_project` root。带 UI 的 target 必须声明唯一 `ui_provider`，package 由 `vn.story_ir`、`vn.ui_blueprint_bundle`、`vn.ui_binding_manifest`、`vn.ui_source_map`、`vn.ui_controller_manifest`、`vn.ui_theme_manifest`、`ui.backend_manifest` 和 component trust/artifact section 组成。旧 target v1 与 `vn.compiled_story` reader直接拒绝并要求 recook，不提供 runtime migrator。
...
| `astra.headless_session_batch.v2` | `E2` | 全局 worker limit 为 1..=8 的上限；runner 按该上限、job 数量与 `available_parallelism` 自动选择实际并发度；job id 与串行/并行 artifact root 必须唯一；`route`/`replay` 禁止 performance 配置，`performance` 必须绑定 budget、warmup 与非零 measurement start |
| `astra.headless_session_batch_report.v2` | `E2` | 按 session id 稳定排序，绑定 profile/package/input/build hash、串行与并行 output identity、配置上限、硬件并行度、实际并发度、排队/运行时间、吞吐、worker 利用率，以及所有 job 的串行/并行 private-memory peak；performance job 额外记录 CPU/E2E percentile |
...
Migration 11 Headless 测试格式已经由 Rust 类型实现，仍不得进入 package 或发布 profile：
...
| `vn.presentation_provider_manifest` | `IN_PROGRESS` | `postcard` 编码的 `astra.vn.presentation_provider_manifest.v2`。Package、Player 和 Release Gate 共用同一 validator，校验 renderer/filter provider、shader profile、movie/voice/timeline await capability，以及 `minimal`/`classic`/`modern`/`advanced-vn` 的 preset、filter、fallback 和 effect budget。`minimal` 只供 Engine 自动化测试使用，release validation 必须返回 `ASTRA_ENGINE_TEST_PROFILE_RELEASE`；v1 package 必须重新 Cook。真实 timeline/video/audio 执行仍未闭合 |
