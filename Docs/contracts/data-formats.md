...
Migration 12 planned 直接切换到 `astra.target_manifest.v2` 与 `vn.compiled_project` root。带 UI 的 target 必须声明唯一 `ui_provider`，package 由 `vn.story_ir`、`vn.ui_blueprint_bundle`、`vn.ui_binding_manifest`、`vn.ui_source_map`、`vn.ui_controller_manifest`、`vn.ui_theme_manifest`、`ui.backend_manifest` 和 component trust/artifact section 组成。完成时旧 target v1 与 `vn.compiled_story` reader 直接拒绝并要求 recook，不提供 runtime migrator。当前 writer/reader 仍是 v1/`CompiledStory`，不能把 planned section 当成现有格式。
...
| `vn.presentation_provider_manifest` | `IN_PROGRESS` | `postcard` 编码的 `astra.vn.presentation_provider_manifest.v2`。Package、Player 和 Release Gate 共用同一 validator，校验 renderer/filter provider、shader profile、movie/voice/timeline await capability，以及 `classic`/`modern`/`advanced-vn` 的 preset、filter、fallback 和 effect budget；v1 package 必须重新 Cook。真实 timeline/video/audio 执行仍未闭合 |
