...
| `astra.text_layout_replay.v1` / `astra.text_layout_replay_snapshot.v1` | `IN_PROGRESS` | bounded postcard transcript 固化 package/build/session/provider/font/request/layout/glyph identity，支持事务性 restore continuation 与 provider-free replay；Windows hardware glyph consumer 已闭合，Web 与正式产品 presentation/release consumer 尚未闭合 |
