...
旧引擎资源包通过 Asset VFS 挂载为 `legacy_pack`。Family reader 只能实现 `VfsMountProvider`，不能替代 `.astrapkg` 或直接读取 host filesystem。`.astrapkg` 保存 case profile、family provider binding、reader identity/hash、sanitized scenario refs 和 release report；legacy pack mount 提供 entry map、offset、size、hash、media kind 和 bounded read。

Patch、翻译覆盖和本地调试替换走 `overlay` mount。未声明 overlay allowlist 的同 key 多命中必须 blocking。Report 只记录 alias、relative key、pack/entry、offset、size、hash、media kind、coverage 和 diagnostic，不记录本地 root、payload、完整脚本或 bytecode。
