...
持续音频由 Player 持久 mixer owner 管理。mixer 使用固定 sample rate/channel contract、显式 bus、bounded voice/render budget、逐 frame fade、loop cursor、sample clamp 和稳定 completion；重复 voice、格式漂移、非法 gain/fade、voice/render 超限必须 blocking。平台 `audio.query` 返回 queued/consumed/submitted frame、meter 和 callback underflow，Player 只补到目标 queue 水位；启动期建立 underflow baseline，稳定泵送后 underflow 增长必须以 `ASTRA_PLAYER_AUDIO_UNDERFLOW` 终止受影响 session。Windows/WASAPI 和 Web/AudioWorklet 使用同一 queue-state contract，drain deadline 按实际提交时长加 callback margin 计算，不能使用固定两秒或十秒超时。退出时必须 drain 并 close。pause/resume、显式 stop、设备切换恢复及 Web Player 产品接线仍是独立未完成门禁，不能由 mixer unit test 替代。
