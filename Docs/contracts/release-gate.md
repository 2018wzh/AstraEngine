...
Migration 11 完成后，真实产品平台验收还要先通过 `platform.headless_preflight`。它要求同一 build fingerprint、cooked package hash、input sequence hash、scenario、target 和 content identity 的 `astra.headless_run_report.v1`、`astra.headless_review_bundle.v1`、`astra.headless_review.v1`、`astra.platform_run_identity.v1` 与 `astra.headless_preflight_link.v1`。Release CLI 必须同时读取 review bundle；Headless blocked、缺 required artifact/model review、bundle/report hash 漂移或 identity mismatch 时，不得通过正式平台验收。该 check 仍只提供 E2 前置证据，不能让 `player.full_playable`、Windows/Web host conformance 或 E3 自动通过。
