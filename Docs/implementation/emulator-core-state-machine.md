...
Family pack reader 必须实现 VFS provider，并注册到单一 `vfs_provider` slot。`.astrapkg` 保存 case profile、reader identity、package sections、release report 和 sanitized scenario；旧 pack 只作为 `legacy_pack` backend 被读取。报告只记录 `vfs_uri`、prefix、pack/entry、offset、size、hash、media kind、coverage 和 diagnostic，不记录本地 root 或 payload。
...
3. `GraphLoad`、`GraphMove`、`GraphFade` 变成 `PresentationCommand`，asset ref 使用 `fvp:/graph_bg/BG001_000` 这类 provider URI。
