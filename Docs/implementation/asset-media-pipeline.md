...
| --- | --- | --- |
| Renderer2D | optional `wgpu` provider | Migration 11 `CpuRendererProvider`，仅用于 Headless E1/E2 |
...
| Audio Output | platform-owned output provider（Windows 当前为 WASAPI） | Migration 11 Headless output 保存完整 48 kHz stereo PCM S16LE WAV；不具备 shipping eligibility |
