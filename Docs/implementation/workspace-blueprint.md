...
| `astra-emu-siglus` | 5 in progress | pinned `siglus_rs` hosted fork 与 Family ABI v8 resource/private-material/save host adapter；完整 provider session 尚未接入 | `astra-emu-family-api`, `astra-byte-source`, `siglus_rs` hosted fork | 本地路径、secret、商业 payload、第二套 RuntimeWorld 或平台 renderer/audio device |
