...
| `Engine/Source/Platform/` | Stage 1/2 implemented, Headless verification in progress | Target 与 Platform crate：`astra-target`、`astra-platform`、六个平台 host crate，以及 `publish = false` 的 `astra-platform-headless`；Migration 12 Scene2D/Mesh2D UI consumer 仍 planned |
