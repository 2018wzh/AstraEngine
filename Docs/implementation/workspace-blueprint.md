...
| `Emulator/Source/` | Stage 5 `IN_PROGRESS` | 已接入 family ABI v4 stat/range/revision、FVP compatibility core、修复后的 fixed-step 与 streaming archive、`AstraEmuRuntimeProvider`、RuntimeWorld bridge、SQLite Library v5、auto probe、Trusted Luau、ECNU translation、host audio/movie、Slint Manager、共享 wgpu host、签名工具、evidence encoder、Android package 和 iOS static-registry scaffold；实际游戏逐帧 parity、正式平台签名与 E3 仍开放 |
...
| `astra-emu-fvp` | 5 in progress | FVP v1 `LegacyRuntimeProvider`、固定 rfvp revision、HCB/VM/syscall/snapshot、renderer-neutral frame 输出，以及通用 family-core `.bin` mount/stat/range/stream factory | `astra-emu-family-api`, `astra-emu-family-core`, `astra-byte-source`, `astra-emu-fvp-rfvp-core` | window/event-loop/GPU/audio native handle ownership |
