...
| 路径 | 状态 | 职责 |
| --- | --- | --- |
| `Engine/Source/Runtime/` | Stage 1 implemented, Stage 2/3 reopened | EngineCore runtime crate：`astra-core`、`astra-runtime`、`astra-engine` Rust dylib facade、`astra-plugin-abi`、`astra-plugin`；Stage 2 `astra-media-core` 提供轻量 media contract，`astra-asset` 需要升级为 VFS contract owner；Stage 3 `astra-player-core` automation report contract 已开始落地 |
| `Engine/Source/Modules/` | Stage 3 reopened | 产品垂直模块 crate。AstraVN 目标路径是 `Engine/Source/Modules/AstraVN/`，其中 `astra-vn` 只作为 facade，具体实现拆到 `astra-vn-script`、`astra-vn-core`、`astra-vn-policy`、`astra-vn-presentation`、`astra-vn-commands`、`astra-vn-system`、`astra-vn-save`、`astra-vn-package`、`astra-vn-plugin`、`astra-vn-editor` 和 `astra-vn-runtime-provider` |
...
| `Engine/Plugins/Providers/` | Stage 2/4 reopened | 通用 provider 插件由 Stage 1/2 registry 和 gate 管理；`VfsMountProvider`、`ProductRuntimeProvider`、OpenAI、Ollama、ComfyUI 和 ONNX 仍是后续 provider |
...
| `astra-plugin` | 1/2/3 | descriptor gate、registry、module slot、真实 loader、action adapter、extension registry backend、VFS mount provider 和 gameplay runtime provider selection backend | `astra-core`, `astra-plugin-abi`, `astra-runtime`, `libloading`, `semver` | Editor widget、native handles in public API |
...
| `astra-asset` | 2 reopened | AssetId、VFS mount contract、`VfsLocator`、`VfsMountDescriptor`、resolve report、sidecar schema、AssetRegistry | `astra-core`, `serde`, `schemars` | decoder native handle, package semantics |
| `astra-cook` | 2 | Importer/CookProcessor、DDC key、cook audit | `astra-asset`, `astra-package`, `image` | Editor UI |
| `astra-package` | 2 | binary package/save container、section reader/writer、Zstd codec、crypto descriptor、plugin registry sections、package-backed VFS mount source | `astra-core`, `postcard`, `serde`, `zstd` | story/runtime semantics, legacy pack reader |
...
| `astra-vn-script` | 3 reopened | `.astra` source、parser/compiler、`CompiledStory`、source map、debug symbol、route graph、story/variable/command manifest | `astra-core`, `serde`, `schemars`, `miette` | RuntimeWorld, Luau, Editor UI |
| `astra-vn-presentation` | 3 reopened | StageModel、layer/camera/video/audio/timeline/fallback、headless presentation execution、presentation provider manifest | `astra-core`, `astra-media-core`, `serde`, `schemars` | decode/text/audio native provider, VN policy VM |
| `astra-vn-core` | 3 reopened | `VnRuntime`、command cursor、runtime state、choice、call/return、backlog、read-state、voice replay、wait state、system state、replay UI | `astra-core`, `astra-runtime`, `astra-vn-script`, `astra-vn-presentation`, `postcard`, `serde` | Luau VM handle, Editor UI, package writer |
| `astra-vn-policy` | 3 reopened | Luau sandbox、policy state、mutation/query/trace、policy bundle manifest、source cache、`standard_policy.luau` | `astra-core`, `astra-vn-core`, `mlua`, `serde`, `schemars` | fs/network/system capability, native handle |
| `astra-vn-commands` | 3 reopened | standard command library、command schema、usage validation、command manifest | `astra-core`, `astra-vn-script`, `astra-vn-presentation`, `serde`, `schemars` | runtime provider selection |
| `astra-vn-system` | 3 reopened | system stories、save/config/backlog/gallery/replay/route chart/localization profile | `astra-core`, `astra-vn-script`, `astra-vn-core`, `serde`, `schemars` | Editor UI |
| `astra-vn-save` | 3 reopened | `vn.runtime_state`、`vn.policy_state` save sections、save blob、hash、migration glue | `astra-core`, `astra-vn-core`, `astra-vn-policy`, `astra-package`, `postcard` | Luau function/thread/userdata |
| `astra-vn-package` | 3 reopened | `vn.*` package section plans、profile manifest、commercial baseline、advanced presentation manifest、package evidence | `astra-core`, `astra-package`, `astra-vn-script`, `astra-vn-policy`, `astra-vn-presentation`, `astra-vn-system` | release gate ownership |
| `astra-vn-plugin` | 3 reopened | VN extension points、extension manifest、provider slot ids | `astra-core`, `astra-plugin-abi`, `astra-vn-package`, `serde`, `schemars` | plugin loader internals |
| `astra-vn-editor` | 3/4 reopened | Graph/Timeline authoring metadata、source round-trip metadata、NativeVN `RuntimeEditorMetadata` | `astra-core`, `astra-vn-script`, `astra-vn-presentation`, `astra-vn-plugin`, `serde`, `schemars` | Qt/QML widget, Editor native handle |
| `astra-vn-runtime-provider` | 3 reopened | `NativeVnRuntimeProvider` prepare/probe/open/step/save/restore/package/release/editor metadata composition | `astra-runtime`, `astra-target`, `astra-vn-core`, `astra-vn-policy`, `astra-vn-save`, `astra-vn-package`, `astra-vn-editor` | RPG/EMU base class, direct RuntimeWorld pointer ownership |
| `astra-vn` | 3 reopened | Rust ABI dylib facade 和兼容 re-export，不承载 parser/runtime/policy/package 实现 | AstraVN 子 crate | platform-native handles, second runtime, C ABI promise, heavy decode/text provider dependencies in facade, RPG/EMU base class |
