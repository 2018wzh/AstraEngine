...
| `astra-emu-family-api` | 5 in progress | ABI-safe descriptor、probe/session/step/effect/wait/snapshot/redaction DTO 和 host-neutral render packet | `astra-core`, `abi_stable`, `serde`, `postcard` | in-process VFS trait、Manager UI、RuntimeWorld、GPU/audio handle |
...
| `astra-emu-fvp` | 5 in progress | FVP v6 `LegacyRuntimeProvider`、pinned thin `rfvp` hosted fork、owned HCB/VM/snapshot 与语义 scene/media/text delta，以及通用 family-core `.bin` mount/stat/range/stream factory | `astra-emu-family-api`, `astra-emu-family-core`, `astra-byte-source`, `rfvp` hosted fork | window/event-loop/GPU/audio native handle ownership |
| `astra-emu-siglus` | 5 in progress | pinned `siglus_rs` hosted fork、Family ABI v8 resource/private-material/save adapter、dynamic provider lifecycle 与基础 2D typed scene lowering | `astra-emu-family-api`, `astra-byte-source`, `astra-core`, `siglus_rs` hosted fork | 本地路径、secret、商业 payload、第二套 RuntimeWorld 或平台 renderer/audio device |
...
| `astra-emu-minori` | 5 in progress | Minori PAZ v0-v2 family factory、唯一纯 Rust decrypt provider、分卷 VFS、`.sc` lossless parser、受限确定性 VM 与签名 runtime provider | `astra-emu-family-core`, `astra-emu-family-support`, `astra-emu-family-api`, `blowfish`, `rc4`, `flate2` | key、商业 payload、Luau decrypt callback、私有 renderer/font/audio stack 或本地路径 |
