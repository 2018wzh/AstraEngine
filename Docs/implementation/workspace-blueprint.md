...
| `astra-emu-fvp` | 5 in progress | FVP v5 `LegacyRuntimeProvider`、pinned thin `rfvp` hosted fork、HCB/VM/snapshot 与语义 scene/media/text delta，以及通用 family-core `.bin` mount/stat/range/stream factory | `astra-emu-family-api`, `astra-emu-family-core`, `astra-byte-source`, `rfvp` hosted fork | window/event-loop/GPU/audio native handle ownership |
