...
| 路径 | 状态 | 职责 |
| --- | --- | --- |
| `Engine/Source/Runtime/` | Stage 1/2 implemented, Stage 3 in progress | EngineCore runtime crate：`astra-core`、`astra-runtime`、`astra-engine` Rust dylib facade、`astra-plugin-abi`、`astra-plugin`；Stage 2 `astra-media-core` 提供轻量 media contract，`astra-asset` 已成为 Provider URI VFS contract owner；Stage 3 `astra-player-core` automation report contract 已开始落地 |
| `Engine/Source/Modules/` | Stage 3 implemented for AstraVN split | 产品垂直模块 crate。AstraVN 已位于 `Engine/Source/Modules/AstraVN/`，其中 `astra-vn` 只作为 facade，具体实现拆到 `astra-vn-script`、`astra-vn-core`、`astra-vn-policy`、`astra-vn-presentation`、`astra-vn-commands`、`astra-vn-system`、`astra-vn-save`、`astra-vn-package`、`astra-vn-plugin`、`astra-vn-editor` 和 `astra-vn-runtime-provider` |
...
| `astra-vn-save` | 3 implemented | `vn.runtime_state`、`vn.policy_state` save sections、save blob、hash、migration glue | `astra-core`, `astra-vn-core`, `astra-vn-policy`, `astra-package`, `postcard` | Luau function/thread/userdata |
...
| `astra-vn-runtime-provider` | 3 implemented | `NativeVnRuntimeProvider` prepare/probe/open/step/save/restore/package/release/editor metadata composition and DTO+FFI adapter | `astra-runtime`, `astra-target`, `astra-plugin-abi`, `astra-vn-core`, `astra-vn-policy`, `astra-vn-save`, `astra-vn-package`, `astra-vn-editor` | RPG/EMU base class, direct RuntimeWorld pointer ownership |
