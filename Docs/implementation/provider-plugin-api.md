...
`ProductRuntimeProvider` 是 packaged `Game` target 的玩法 runtime selector。NativeVN、AstraEMU 和后续 AstraRPG 都通过这个 slot 显式绑定；AstraVN 不作为所有玩法的基类。Provider ABI v4 的 step output 是 typed live DTO 与 control transaction，并支持 retained ephemeral text 的显式对齐和清除；save/package/report 使用独立 persisted DTO。动态 action bytes invoke、通用 payload 与 effect envelope 已删除。
