...
`ProductRuntimeProvider` 的 trait 是 host/in-process 设计形状。跨 ABI 使用 `astra-plugin-abi` 中的 `FfiRuntimeProviderRegistration`，每个 entrypoint 只收发 bounded `RVec<u8>` DTO payload，并返回统一 `FfiRuntimeProviderResult`。`FfiPluginRegistration.runtime_providers` 注册后，loader 会同时写入现有 provider slot 记录，release gate 继续读取 `provider.policy` 与 `plugin.extension_registry`。当前已落地的 NativeVN provider 通过 in-process provider 执行 scenario；FFI adapter 只承诺 ABI shape、注册、调用和卸载 smoke，不把外部 dylib gameplay 完整加载写成本阶段完成项。
