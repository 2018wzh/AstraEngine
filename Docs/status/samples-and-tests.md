...
| EngineCore Native Smoke | Stage 1 EngineCore contract regression | `astra-runtime`/`astra-plugin` 受控 Rust tests，覆盖 dialogue/choice event、save/load、delayed event、provider 与 replay hash；旧 YAML runner 已退役 |
...
所有平台无关 Runtime、Player 和 full-flow 测试现在由统一 `HeadlessTestContext` 生命周期编排，产品 adapter 接受序列化物理输入并产出真实 PNG/WAV。正式产品级样例仍要补齐全帧/全音频比较、required checkpoint 具名审查和同 build/package/input 的真实平台 preflight link；实现存在不能冒充这些正式 evidence。
