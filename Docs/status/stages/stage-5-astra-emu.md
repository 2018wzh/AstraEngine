...
2026 年 8 月 3 日的真实安装原生 10 分钟 mixed-run 未通过性能与音频门禁。35,578 个 fixed tick 中，RFVP core p99 为 2.923 ms，完整 tick p99 为 16.690 ms、最大值为 5.892 s，device underflow 累计 656 次。第 651 step 的 effect dispatch 阻塞 3.940 s，而同 step 的 RFVP core 只用 2.696 ms。与 RFVP `0.5.0` 的 native renderer 对照后，根因边界已经收窄：原版按 `GraphBuff generation` 对同尺寸动态纹理执行原位 GPU 更新；hosted 路径仍把像素复制、序列化到 family ABI，随后分配新 scene resource generation，通用 WGPU renderer 又把资源变化放大为全 atlas rebuild。音频 producer 仍依赖 fixed tick 补充 120–180 ms device queue，所以图形长帧会直接造成撕裂。稳定 subresource update、独立 audio producer 和复跑通过前，本 Stage 的 FVP 性能、native audio 与 Windows E3 状态保持 `IN_PROGRESS`。
