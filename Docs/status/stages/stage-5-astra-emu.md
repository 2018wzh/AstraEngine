...
同日后续完成 `.char trans`/`.char vis` 的原程序语义复核与 typed runtime 接入。transition 由 fixed tick 线性推进并阻塞脚本，完整 continuation 状态进入 v22 snapshot；retained Scene2D 在等待期间持续输出。合成 256→128→0 透明度序列已通过 Headless WGPU capture 和人工视觉检查。真实样本 census 没有这两类命令，因此该证据不提升真实路线 coverage。后续 clean Release 120 Hz run 完成 36600 fixed tick、73200 presentation 和同身份 Perfetto/report/trace manifest，runtime 与 presentation p99 分别为 0.3491 ms 和 0.92406 ms；2 次 presentation 超过 8.333 ms，零容忍 deadline 预算仍阻断正式 GPU 性能 E2。
