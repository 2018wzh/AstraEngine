...
**Current Evidence:** Family ABI v8 contract、FFI round-trip、dynamic instance host-service dispatch 和 v7 拒绝已落地。`siglus_rs` 的 `astra-hosted` revision 已固定为 `1e752ba96ab08cf3fb224e247815492c5dd8c5e4`：semantic session core 已与 standalone renderer owner 分开；`Scene.pck` 只常驻 metadata，scene body 通过 revision-bound bounded range 读取；scene bytecode 使用 reference-counted owned storage。host port 已覆盖显式原版/汉化 profile、root/`StartData/GameData` 歧义阻断、private material 精确长度、logical save 和 revision-bound range cache。fork 的 retained delta 已带舞台尺寸、稳定 texture identity、generation 与 owned RGBA；hosted feature 不再携带 desktop capture owner。`astra-emu-siglus` 现已实现 Family ABI v8 provider、dynamic FFI export、显式 profile probe、严格 fixed tick/input、基础 2D scene lowering 和 session lifecycle。CLI 可从授权根内的 `key.toml` 解析 16-byte material，跨 ABI 只传 logical secret id 与短生命周期 buffer；desktop package 工具可生成签名 Siglus development distribution。真实 Rewrite+ 的签名 dynamic Headless GPU smoke 已完成 10 tick、3 条物理输入、8 次 presentation 和正常 shutdown，未输出 key、路径或商业内容。该 run 全部是黑帧，且未启用 snapshot；它只能证明 private open、RuntimeWorld、dynamic provider 和 GPU host lifecycle，不能算视觉或路线 E2。VM 内图片、音频、影片、字体和 runtime save 仍未全部改接 host port，snapshot/save/restore、完整 typed mesh/effect/text/media delta、同输入逐帧差分和完整路线仍为 blocking。
...
Siglus 的 fork 基线为 `a8a3376049f47a141a673a49f15ab7de8746e1e1`，维护分支为 `astra-hosted`，当前 pinned revision 是 `1e752ba96ab08cf3fb224e247815492c5dd8c5e4`。standalone renderer 与 semantic session core 已分离，lazy `Scene.pck` archive、VM lazy scene switch、owned scene storage、resource/private-material/save host port、显式 case binding和 retained texture delta 已经落地。`HostedSession` 已提供 open、deterministic step 和 shutdown，并在时钟、input order 或 runtime 错误后 poison；公开单场景 fixture 已覆盖 public lifecycle。hosted feature 的依赖树不包含 cpal、winit、wgpu、egui 或 ureq，默认 standalone library仍可构建。Astra workspace 的 `astra-emu-siglus` 已导出 v8 runtime provider 和 dynamic plugin lifecycle，基础 2D typed scene 可进入共享 Headless GPU host。

当前公开 fixture 是 provider lifecycle E1。真实 Rewrite+ 的签名 dynamic Headless smoke 已闭合 private material、probe/open、RuntimeWorld、10 个 fixed tick、8 次 GPU presentation 和 shutdown，但输出仍是黑帧，且没有 snapshot、视觉 checkpoint、差分或路线覆盖，所以只计为 E2 基础设施证据。图片/音频/影片/字体/runtime save 的 host 化、完整 typed delta、fork headless oracle、同输入逐帧精确比对、模拟输入加速完整路线、性能和 Windows E3 均未完成。
