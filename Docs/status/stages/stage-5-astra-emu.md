...
**Current Evidence:** Family ABI v8 contract、FFI round-trip、dynamic instance host-service dispatch 和 v7 拒绝已落地。`siglus_rs` 的 `astra-hosted` revision 已固定为 `a815b590f944bc9b40374e76efb0f0fdef97e183`：semantic session core 已与 standalone renderer owner 分开；`Scene.pck` 只常驻 metadata，scene body 通过 revision-bound bounded range 读取，VM 后续 scene 切换也复用同一 lazy archive，不会重新落回文件路径；host port 已覆盖显式原版/汉化 profile、root/`StartData/GameData` 歧义阻断、private material 精确长度、logical save、LRU range cache，以及首个空槽原子写和复读校验。`astra-emu-siglus` 已加入 workspace，并把 Family ABI v8 VFS、private material 和 save host 转成 fork 的 instance-bound port；2 项 adapter 单测通过。fork 的 hosted/standalone library check、7 项 hosted port 测试和 2 项 lazy archive 测试也已通过。仓库外授权 material 已通过严格 parser、原版 Gameexe 解码和 bounded `Scene.pck` 首 scene 校验；测试没有输出 key 或商业内容。完整 `HostedSession`、provider lifecycle、公开 fixture 与 E2 仍未完成，因此真实 Rewrite+ 长流程、存档 round-trip、差分、性能和 Windows E3 继续保持 blocking。
...
Siglus 的 fork 基线为 `a8a3376049f47a141a673a49f15ab7de8746e1e1`，维护分支为 `astra-hosted`，当前 pinned revision 是 `a815b590f944bc9b40374e76efb0f0fdef97e183`。standalone renderer 与 semantic session core 已分离，lazy `Scene.pck` archive、VM lazy scene switch、resource/private-material/save host port、显式 case binding、revision-bound range cache 和空槽原子写校验已经落地。hosted feature 的依赖树不再包含 cpal、winit、wgpu、egui 或 ureq，默认 standalone library 仍可构建。Astra workspace 已加入 `astra-emu-siglus`，当前只承载 v8 host-port adapter，不导出尚未实现的 runtime provider。

这仍是 E0/E1。VM 内部的图片、音频、影片、字体、时钟和 runtime save 路径尚未全部改接 instance-bound host port，`HostedSession` 也没有形成可供 provider 调用的 `open/step/save/restore/shutdown` 完整生命周期。授权 material 只通过 parser、Gameexe 和首 scene 的 bounded reader 校验，不等于真实 provider E2。公开 fixture、dynamic Headless E2 和真实长流程证据都没有生成。
