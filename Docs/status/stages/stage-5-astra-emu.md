...
2026-08-25 follow-up：Manager 的 Minori AVI streaming/first-frame preview 已与 `astra-emu-minori` 的 bounded WMV3/PCM adapter 统一；Minori video extension 现在在进入 FVP/Windows provider 前严格只接受 `avi`，其他 codec 返回 `ASTRA_EMU_MINORI_VIDEO_CODEC_UNSUPPORTED`。这只是 provider-boundary correctness 与 focused test evidence，尚未生成新的真实 Manager preview 或 Windows E3 artifact。
