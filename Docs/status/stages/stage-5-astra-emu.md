...
**Current Evidence:** Family ABI v8 contract、FFI round-trip、dynamic instance host-service dispatch 和 v7 拒绝已落地。`siglus_rs` 的 `astra-hosted` revision 已固定为 `1b0814595ae83321666357a3b27b79802a328e76`：semantic session core 已与 standalone renderer owner 分开；`Scene.pck` 只常驻 metadata，scene body 通过 revision-bound bounded range 读取，VM 后续 scene 切换也复用同一 lazy archive，不会重新落回文件路径；scene bytecode 使用 reference-counted owned storage，最后一个 stream 释放后底层 buffer 同步释放。host port 已覆盖显式原版/汉化 profile、root/`StartData/GameData` 歧义阻断、private material 精确长度、logical save、LRU range cache，以及首个空槽原子写和复读校验。fork 已提供 `HostedSession::open/step/shutdown`，step 校验有理数 fixed tick 与严格递增的 input sequence，失败后 poison session；公开单场景 fixture 已覆盖 public open/step/ledger/shutdown，仓库外 Rewrite+ 已完成 private open 和首个 fixed step。`astra-emu-siglus` 已加入 workspace，并把 Family ABI v8 VFS、private material 和 save host 转成 fork 的 instance-bound port；2 项 adapter 单测通过。测试没有输出 key、路径或商业内容。snapshot/save/restore、完整 resource/media/text host 化、有界 typed delta、provider、完整公开 fixture 与 E2 尚未完成，因此真实 Rewrite+ 长流程、存档 round-trip、差分、性能和 Windows E3 继续保持 blocking。
...
Siglus 的 fork 基线为 `a8a3376049f47a141a673a49f15ab7de8746e1e1`，维护分支为 `astra-hosted`，当前 pinned revision 是 `1b0814595ae83321666357a3b27b79802a328e76`。standalone renderer 与 semantic session core 已分离，lazy `Scene.pck` archive、VM lazy scene switch、owned scene storage、resource/private-material/save host port、显式 case binding、revision-bound range cache 和空槽原子写校验已经落地。`HostedSession` 已提供 open、deterministic step 和 shutdown，并在时钟、input order 或 runtime 错误后 poison；公开单场景 fixture 已覆盖 public lifecycle。hosted feature 的依赖树不再包含 cpal、winit、wgpu、egui 或 ureq，默认 standalone library 仍可构建。Astra workspace 已加入 `astra-emu-siglus`，当前只承载 v8 host-port adapter，不导出尚未完整实现的 runtime provider。

这仍是 E0/E1。VM 内部的图片、音频、影片、字体和 runtime save 路径尚未全部改接 instance-bound host port，`HostedSession` 也没有形成 `save/restore` 与完整 typed delta。授权 material 已用于仓库外 Rewrite+ 的 open 和一个 fixed step，但不等于真实 provider E2。公开 fixture、dynamic Headless E2 和真实长流程证据都没有生成。
