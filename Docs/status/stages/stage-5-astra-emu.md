...
2026 年 8 月 23 日增量：动态 family loader、CLI 与 Manager 已绑定 ABI v9 四个 Host port，公共 support 层提供 retained surface store、私有 writable-file Host 和像素规范化。Minori resource/text 已走 `Native + MultiLayer`；文字先经过同步 translation Hook，再由 CosmicText 与 Astra Renderer2D 写入 Host surface。typed filter graph 基线为 `e6bc3d960b87373160acd8507faeac4cc589975b`，Manager 使用公共 validator 和 WGPU 执行，不再使用字符串 binding 或 CPU fallback。Windows current-user ACL 断言、旧 Scene2D 清理、Headless composition 和 v9 真实样本复验仍未闭合，Stage 状态不变。
