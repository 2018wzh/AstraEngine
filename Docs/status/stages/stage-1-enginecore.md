...
4. 配置 CI 顺序：`python Tools/check_docs.py`、`cargo fmt --check`、`python Tools/run_cargo_isolated.py clippy --workspace --all-targets -- -D warnings`、`python Tools/run_cargo_isolated.py test --workspace`。
...
## S1-BUILD-IDENTITY-01 Checkout-bound build identity

**ID:** `S1-BUILD-IDENTITY-01`

**Goal:** 让多 worktree 的 Cargo 验证、动态 fixture 和证据只使用当前 checkout 的构建身份。

**Depends On:** `S1-BOOT-01`、`S1-PLUGIN-01`

**Target Paths:** `Tools/run_cargo_isolated.py`、`Engine/Source/Runtime/astra-plugin/src/loader.rs`、`.github/workflows/ci.yml`

**Steps:**

1. 计算 commit/dirty state、workspace manifest、Cargo.lock、Rust toolchain 和 feature/target/profile fingerprint。
2. 从完整 identity 派生独立 Cargo target root，并原子写入 `astra.build_identity.v1`。
3. 让动态 fixture build/load 共同解析同一 `CARGO_TARGET_DIR`，禁止回退到共享 `target/debug`。
4. 报告只记录 target role、相对 artifact path、binary hash 和 byte size；identity 不一致或报告损坏时阻断。
5. CI 的 clippy、test 和 native smoke 统一通过该入口执行。

**Done Evidence:** Python identity tests、`astra-plugin` artifact path tests、隔离 `astra-cli` logging tests和完整 workspace tests通过；report不含本地绝对路径。

**Linked Test IDs:** `T-S1-BUILD-IDENTITY-01`
...
**Target Paths:** `Engine/Source/Runtime/astra-runtime/tests/`、`Engine/Source/Runtime/astra-plugin/tests/`、`Engine/Source/Developer/astra-test/src/report.rs`、`Engine/Source/Programs/astra-cli/src/main.rs`
...
**Done Evidence:** 原 native smoke 的 replay hash、FFI action provider、event/choice、save/load 和 delayed event checks 已拆入 Runtime/provider Rust tests，并由 Migration 11 的统一 `HeadlessTestContext` lifecycle 执行。旧 YAML runner 和语义快捷命令已删除，不再作为当前命令证据。
