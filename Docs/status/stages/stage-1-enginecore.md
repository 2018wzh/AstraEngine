...
Stage 1 已交付 EngineCore 的可运行闭环：UE 风格 workspace、核心类型、Runtime tick、StateMachine action provider、AwaitToken、DelayedEventQueue、Save/Replay、Plugin ABI、PropertySystem 和 headless scenario runner。后续 Stage 的页面仍记录未实现工作。
...
**Done Evidence:** fixed tick 测试证明 event 顺序不受提交顺序外的因素影响；multi-action transition 按定义顺序执行。
...
**Done Evidence:** `state_machine_tick` 覆盖 multi-action 顺序和失败隔离；`delayed_event` 覆盖 schedule、cancel、drain order 和 save/load。
