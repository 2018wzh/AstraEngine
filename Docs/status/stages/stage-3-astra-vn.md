...
2026-07-17 主线迁移状态：TsuiNoSora 已删除“脱敏 route 元数据生成合成剧情”的产品路径。`nativevn-project` 只接受完整 `tsuinosora.native_story_ir.v1`，并逐项阻断 source、handler、command、route、terminal、choice、media 和 wait coverage 缺口。当前私有 full coverage report 已通过，记录 578 个 source resource、724 个 handler、24,191 条 command、36 条自动化路线、1 个 terminal、1,293 个 choice、3,053 条 media command 与 17,151 个 wait command；2527/2527 个 ProjectorRays binary chunk 已完成转换，诊断数为 0。Classic/Modern Yakui UI 已进入实际 package，右键经 typed UI action router 打开 Classic Save/Load 或 Modern Quick Panel。Headless 已通过 Classic 4 个、Modern 10 个视觉/交互 checkpoint，Modern 覆盖 `ja`、`zh-Hans`、`en` 三套系统文案；Title、真实存档缩略图/metadata 和跨进程目录恢复也已通过。Web portable graph 使用精确锁定的 pure-Rust Luau runtime，`wasm32-unknown-unknown` 构建与 standalone bundle 已通过，且 graph 不含 native-only `mlua`、`abi_stable` 或 `libloading`。上一最终路线包的 36/36 Headless route matrix 继续有效；当前 package 仅有 12/36 route report 为 `passed`，外层时限中止后没有完整汇总。Classic 使用 score-derived `fit:native` 和 absolute UI placement 保留 800×600 sky、752×424 stage、754×82 dialogue frame 的原始构图；Modern Save/Load/Backlog/Config 复用相同 sky texture，以四边约束、gold rule 和 paper surface 保留原作画框语言。网络原版截图与独立实机视频交叉核验新增了三个必须覆盖的 Classic checkpoint：舞台内压暗居中文字、纯黑居中文字、人物越过舞台底缘；当前普通 message checkpoint 不能证明这些状态。Windows E3、当前 package 完整矩阵、上述特殊表现态与 human formal signoff 仍未完成。
...
上段中的旧 private acceptance 状态只保留为历史记录，不代表当前阻断。当前完整 typed story IR、2527/2527 resource conversion、Classic/Modern package、Title、真实存档缩略图/metadata、跨进程存档目录、Web portable graph/bundle E2 和上一最终路线包的 36/36 Headless matrix 已通过；当前阻断是当前 package 的剩余 24 条 route、网络参考新增的三个 Classic 特殊表现 checkpoint、Windows E3 前台焦点和 formal signoff。Windows E3 必须继续使用真实 `SendInput`，不能因桌面环境拒绝前台焦点而改用直接 runtime action。Web 已使用精确锁定的 pure-Rust portable Luau runtime 消除 `mlua`/`abi_stable` native graph 泄漏，但 Web E3 仍不在本轮完成声明内。
