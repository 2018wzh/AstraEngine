...
2026-07-20 private RC 增量证据：当前 Classic package 在 `wgpu_offscreen`、Windows DX12 离散 GPU 上通过 43 个视觉 checkpoint；独立的 Y 路线 E2 用 445 条序列化物理输入和 27 次选择抵达 K movie 首个权威 wait。RC 的 13 项像素预检全部通过，模型已查看全部五联图；`006` 仍是唯一允许使用具名 `astra.headless_tolerance_approval.v2` 的色彩 profile。30 张输入与 12 组 A/B 稳定复帧已闭合，release gate 会 fail-closed 校验 node validation。K movie 的 UI004/UI015 只留在全量研究报告中，不阻断 Y RC。Windows E3 显式延期，不作为本轮 RC 门禁；最终同身份重跑与 formal signoff 继续 blocking。
...
**Current Evidence:** NativeVN 已切换到唯一 `runtime.world`/`astra.runtime.save_blob.v3` section；定向 provider/FFI 测试覆盖 v3 save/restore，Runtime 测试覆盖 inverse journal、Action ABI v2、conflict-DAG 和 1/2/4/8 worker hash 一致性，VN provider 测试覆盖 100k dialogue 下有界 hot state。2026-07-30 clean Release build `e679d107`、build fingerprint `sha256:ed5c5bbf3c747f241668ff3e3df559512a6c202e488062a5c6749a6a145e26df` 的四 job route/replay batch 在配置上限 8、硬件并行度 24 下自动选择 4 个并发 session，stable-sort 通过，逐 session 串行/并行 output identity 全部一致，serial baseline wall time 899,059 us、concurrent wall time 298,801 us、peak workers 4、worker utilization 976 permille。每条 job 的串行/并行 private-memory peak 已由 runner 按 PID 写入正式 batch report 字段，约为 30.5–31.8 MiB。相同 clean build 的集显 DX12 1920×1080、120 Hz、72,000 帧正式运行通过：CPU p95 665,300 ns、GPU p95 478,210 ns、e2e p99 1,257,480 ns、deadline miss 为零、private memory max 281,083,904 bytes、增长 max 29,151,232 bytes，稳定段 heap allocation、upload、readback 均为零；Perfetto manifest 记录 45,000 个事件、零丢失、未截断、时间戳单调。旗舰 `advanced-vn` 的 streaming E2 使用显式 `ffmpeg-vcpkg` binding、800×600 viewport、build identity `sha256:ddc02b39be12ee0456a8ec7c126d4f0e55ddeb11d31c1abf1f2f560c8bf3c3ac` 与 package `sha256:af6c5a592eee132174869526adf24261c39964a8ed7dab4bed0c5a6d7e44d155`，真实字体、Yakui、Runtime v3、音频和完整 288 帧 decode-to-private-spool 路径通过；Player 只跨 boundary 保留单帧，128 MiB decoded-cache profile 的外部 20 ms 采样得到 peak private 391,589,888 bytes（约 373.45 MiB），低于 384 MiB 门槛。3 GiB 是完整流的逻辑 decoded-byte/spool 上限，不再是内存聚合大小；run 仍只提交 8 帧、光栅化 2 帧。`python Tools/check_docs.py`、fmt、workspace clippy、`cargo build -p astra-headless`、workspace test 和 Windows dynamic-export audit 已在最新源码通过。100k product-route 正式报告与多 session performance p95/p99 仍开放，因此保持 `IN_PROGRESS`。
