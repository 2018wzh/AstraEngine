...
2026-07-19 GPU E2 增量证据：Classic 现为 18 个 checkpoint，Modern 现为 11 个 checkpoint；两次运行均显式绑定 `wgpu_offscreen`，由 Windows DX12 硬件 adapter rasterize，并在 v2 run report/artifact manifest 中校验同一 renderer identity、scene submission 和 checkpoint rasterization。Classic 的 15 张原版参考已统一裁剪为 800×600，v43 自动比较有 13 项通过；剧情节点先由脱敏文字/选项 hash 的完整候选闭包定位，再绑定 Score、handler、resource 和物理输入 occurrence。`006` 使用具名 `astra.headless_tolerance_approval.v2` 绑定的固定色彩 profile，标准门禁不变；`TSUI1999-UI-005/009` 因 source/presentation 或 transition 稳定性证据不足要求重新连续捕获两帧。该证据取代下文旧的 4/10、5/11 checkpoint 计数，但不改变当前 package 完整路线、Windows E3 和 formal signoff 的 blocking 状态。
...
2026-07-19 主线迁移状态：TsuiNoSora 已删除“脱敏 route 元数据生成合成剧情”的产品路径。`nativevn-project` 只接受完整 `tsuinosora.native_story_ir.v1`，并逐项阻断 source、handler、command、route、terminal、choice、media 和 wait coverage 缺口。当前私有 full coverage report 已通过，记录 578 个 source resource、724 个 handler、27,013 条 command、37 条自动化路线、1 个 terminal、1,328 个 choice、3,073 条 media command 与 17,161 个 wait command；2527/2527 个 ProjectorRays binary chunk 已完成转换，诊断数为 0。Classic/Modern Yakui UI 已进入实际 package，右键经 typed UI action router 打开 Classic POPUP 或 Modern Quick Panel。Classic 的两种 Opening、舞台叙述、choice、人物越框、原灰色系统窗口、8 槽、Config、Exit 与隐藏测试菜单都由 typed story/UI/presentation 主路径表达；system page underlay 从序列化 return cursor/wait/choice 重建，不保存 Host texture。Headless 已通过 Classic 18 个、Modern 11 个视觉/交互 checkpoint，Modern 覆盖 `ja`、`zh-Hans`、`en` 三套系统文案；v43 的 15 项 Classic 原版参考比较有 13 项通过，`005/009` 因原版 source/presentation 或 transition 证据不稳定保持 blocking。Web portable graph 使用精确锁定的 pure-Rust Luau runtime，`wasm32-unknown-unknown` 构建与 standalone bundle 已通过，且 graph 不含 native-only `mlua`、`abi_stable` 或 `libloading`。上一最终路线包的 36/36 Headless route matrix 继续有效；当前 37 路线 package 尚无完整汇总。Windows E3、当前 package 完整矩阵、两项 reference 重取证与 human formal signoff 仍未完成。
...
上段中的旧 private acceptance 状态只保留为历史记录，不代表当前阻断。当前完整 typed story IR、2527/2527 resource conversion、Classic/Modern package、Classic 18 个和 Modern 11 个 GPU checkpoint、15 项 Classic v3 比较中的 13 项、真实存档缩略图/metadata、跨进程存档目录、Web portable graph/bundle E2 和上一最终路线包的 36/36 Headless matrix 已通过；当前阻断是当前 37 路线 package 的完整矩阵、`005/009` 原版参考重取证、Windows E3 前台焦点和 formal signoff。Windows E3 必须继续使用真实 `SendInput`，不能因桌面环境拒绝前台焦点而改用直接 runtime action。Web 已使用精确锁定的 pure-Rust portable Luau runtime 消除 `mlua`/`abi_stable` native graph 泄漏，但 Web E3 仍不在本轮完成声明内。
