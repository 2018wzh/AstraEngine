...
2026-07-17 主线迁移状态：TsuiNoSora 已删除“脱敏 route 元数据生成合成剧情”的产品路径。`nativevn-project` 只接受完整 `tsuinosora.native_story_ir.v1`，并逐项阻断 source、handler、command、route、terminal、choice、media 和 wait coverage 缺口。当前私有 full coverage report 已通过，记录 578 个 source resource、724 个 handler、24,190 条 command、36 条自动化路线、1 个 terminal、1,293 个 choice、3,053 条 media command 与 17,151 个 wait command；2527/2527 个 ProjectorRays binary chunk 已完成转换，诊断数为 0。Classic/Modern Yakui UI 已进入实际 package，右键经 typed UI action router 打开 Classic Save/Load 或 Modern Quick Panel。Headless 已通过 Classic 4 个、Modern 10 个视觉/交互 checkpoint，Modern 覆盖 `ja`、`zh-Hans`、`en` 三套系统文案；上一最终路线包的 36/36 Headless route matrix 已通过。Classic 使用 score-derived `fit:native` 和 absolute UI placement 保留 800×600 sky、752×424 stage、754×82 dialogue frame 的原始构图；Modern Save/Load/Backlog/Config 复用相同 sky texture，以四边约束、gold rule 和 paper surface 保留原作画框语言。最新模型审查确认 Classic message 与 `Game.png` 在画布、外框、金线、角饰、舞台和对白条结构上同构，Modern Save 的空槽也已使用云纹缩略图而非无主题色块。该审查没有覆盖 Title 真实启动路径和已占用存档的场景缩略图/metadata，也未替代 Windows E3 或 human formal signoff。最新视觉包的矩阵复验完成前 11 条后被外层执行时限中止；Windows E3、Web portable graph/bundle E2 与 formal manual signoff 仍未完成。
...
上段中的旧 private acceptance 状态只保留为历史记录，不代表当前阻断。当前完整 typed story IR、2527/2527 resource conversion、Classic/Modern package 和上一最终路线包的 36/36 Headless matrix 已通过；当前阻断是最新视觉包完整矩阵、Title 真实启动检查点、真实存档缩略图/metadata、Windows E3 前台焦点、Web portable graph/bundle E2 和 formal signoff。Windows E3 必须继续使用真实 `SendInput`，不能因桌面环境拒绝前台焦点而改用直接 runtime action。Web `wasm-pack build` 当前会重新拉入 `mlua` 和 `abi_stable`，portable graph 需要修复后才能生成本轮 Web bundle。
