...
Control 快进增量：IDA 复核确认 `.pragma enable_control`/`.pragma disable_control` 与 `.pragma skip_enable`/`.pragma skip_disable` 是两组独立 gate。当前 Minori runtime state v20 保存 gate、Control pressed/released 与 `Normal/Auto/Skip` 三态；有效快进由这些权威状态推导，不保存重复布尔值。Host 仍逐 tick 推进，不自动完成 media/presentation fence 或 provider completion。未知 pragma 返回 `ASTRA_EMU_MINORI_RUNTIME_PRAGMA` blocking diagnostic；同一签名动态 plugin 的既有 Control Headless sequence 完成 300 fixed steps、54 个呈现帧、9 条输入、snapshot round-trip 和非静音音频，diagnostic 为 0。Auto 的新 v20 主路径已有 2255-fixed-step 真实八包短程证据，严格证明物理菜单输入切换模式并在 500 ms 后推进正文；完整路线 Auto 仍未关闭。
