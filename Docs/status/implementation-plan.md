...
| 21 | `S5-MINORI-VFS-01` | `IN_PROGRESS` | VFS/decrypt/factory contract 已硬迁移到 `astra-emu-family-core`，公共 Luau/profile/cache/viewer/verify/extract/FUSE 进入 `astra-emu-family-support`；通用 CLI 为 `vfs --family`，Minori import/census 使用独立 CLI。Minori 只保留纯 Rust decrypt provider，Luau 仅注册 data-only private profile。重新递归扫描后确认 8 个逻辑 archive、18 个物理 PAZ 文件；no-cache full verify 覆盖 14502 entry、43818 range read 与 6624958365 decoded bytes。89 脚本 census 为 33728 行、33695 command、29 token、0 unknown opcode。当前 runtime state v21 已按 IDA 合同和全量真实样本绑定 7,047 个 message voice identity，并通过公共 Ogg audio command 表达 stop/load/play；107 个 Minori 定向测试与该 crate clippy 通过。v20 签名 plugin 已完成 Auto 短程与 33498-fixed-step 完整路线 E2，但 v21 真实 Headless voice E2 尚未重跑。正式 review bundle 的 33 个 frame 已逐项检查，整段语音试听仍未完成，`validate-review` 保持阻断。持久 Auto 整路线、voice replay、Config 控件、cache identity、Linux FUSE、Manager media preview 和 Windows E3 仍开放 |
