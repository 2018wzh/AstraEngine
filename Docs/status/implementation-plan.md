...
2026 年 8 月 30 日 Minori 影片 provider 决策：原版行为证据确认其使用 DirectShow Filter Graph + windowless VMR7，并在 HRESULT 失败时退出当前初始化路径。公共有界只读 `IStream` adapter 和分轨 video/audio reader 已落地；授权 AVI 完成 2106 个视频帧、2110 个 PCM chunk 和 8439808 个交错 sample，均到达 EOS且未返回错误，与 FFmpeg 的 7 个 concealment blocker 形成明确差异。下一实现项是统一 WMF audio/video packet provider、seek/cancel 和 registry binding。Windows launch profile 只能显式选择 WMF 或 FFmpeg，任何失败都不得切换 provider。完成真实 movie checkpoint 和原版同点复核前，现有 Release binding 与 Stage 5 状态不提升。
