...
2026-08-28 no-device Sandbox validation: the current signed Manager reached its runtime-active window and Diagnostics reported no blocking diagnostic when the host had no default audio output. `FamilyAudioService` used the bounded null endpoint; the run deliberately remains non-E3 because the sink cannot provide physical audio meter evidence and no artifact channel was available.
