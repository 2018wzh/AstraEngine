...
| 21 | `S5-MINORI-VFS-01` | `IN_PROGRESS` | VFS/decrypt/factory contract 已硬迁移到 `astra-emu-family-core`，公共 Luau/profile/cache/viewer/verify/extract/FUSE 进入 `astra-emu-family-support`；通用 CLI 为 `vfs --family`，Minori import/census 使用独立 CLI。Minori 只保留纯 Rust decrypt provider，Luau 仅注册 data-only private profile。重新递归扫描确认 8 个逻辑 archive、18 个物理 PAZ 文件；no-cache full verify 覆盖 14502 entry、43818 range read 与 6624958365 decoded bytes。89 脚本 census 为 33728 行、33695 command、29 token、0 unknown opcode。runtime state v24 已覆盖真实路线的 message voice/backlog replay、阻塞式 `.char trans/.char vis`，并新增原版 29 类 Config action 的 draft transaction、pointer hit map、state overlay、WAV 试听和公共音量/静音映射。显式 writable-file binding 下的 `astra.emu.minori.config.v1` 已实现 bounded identity-bound 原子写回，gameplay save restore 保留当前 Config；真实八包 Config 短程已用物理 pointer 验证 checkmark、knob 和试听的视觉/自动音频 E2；Manager 已接入 Minori mount profile、解密 URI range reader 和静态 runtime provider，文本 preview 使用 UTF-8/UTF-16 BOM 与 CP932 检测，binary malformed input 保持 hex；完整媒体 binding/audio review 仍阻断。合成动态立绘已通过 Headless WGPU 视觉 E2，真实脚本不含这两类命令。同一 clean Release identity 连续三次完成十分钟 120 Hz GPU 测量，每次 72000 个正式 sample 的 deadline miss 均为 0。完整路线和 33 个 required frame 已有通过证据；整段语音听审、第四条 clear route 后鉴赏、Config 剩余 Host/VM 行为、cache identity、Linux FUSE、Manager family-mounted media preview 和 Windows E3 仍开放 |
