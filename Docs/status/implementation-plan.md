...
| Stage 2 Media + Package | `IN_PROGRESS` | Asset/Cook/Package、局部 headless media contract、release report、Target manifest、strict scenario runner、Windows/Web host slice 和 Provider URI Asset VFS 已落地。Migration 11 重新打开完整 Headless Platform 完成口径：当前没有 `astra-platform-headless`、统一 JSONL 物理输入、真实 PNG/WAV artifact、全 Runtime test 收束、模型审查或真实平台 preflight link。`S2-MEDIA-01/03` 保持 `DONE`，只证明局部 renderer/audio contract；九个 `S2-HEADLESS-*` 均为 `SPEC_READY`。Linux/macOS/iOS/Android 仍留 Stage 6 |
...
| `S3-PRESENT-01` | `IN_PROGRESS` | `SceneCommand` resource upload/release、sprite source rect、glyph run、clip/transform/camera/filter 与扩展 StageModel 已通过 CPU reference tests；timeline scheduler 已接 Windows completion。NativeVN Player 已从 package/VFS 唯一 entry 做 bounded audio read、hash 与 codec signature 校验；Windows 主链已接设备格式协商、bounded sinc resampling、受限声道映射、真实 sample mixer、BGM loop、bus fade、pause/resume/stop、stable voice completion、queue backpressure/underflow、时长感知 drain 和显式 close。Web AudioWorklet 已提供等价 format/queue state，但 Web Player 产品接线、设备热切换恢复、Windows wgpu/WebGPU command stream 和 formal evidence 尚未闭合 |
...
| `S3-SYSTEM-01` | `IN_PROGRESS` | `SystemUiModel` 覆盖 title/message/choice/save/load/config/backlog/gallery/replay/voice replay/route chart/localization preview；Windows Player 的 F5/F9 已接入平台 `BeginSave/WriteSave/CommitSave/ReadSave` transaction、写入或提交失败后显式 abort、provider restore 和恢复帧重新 present。Web 等价主路径、真实渲染与 formal host evidence 尚未闭合 |
...
| `S3-PLAYER-AUTOMATION-01` | `IN_PROGRESS` | `cargo test -p astra-player-core`、`cargo test -p astra-player --test windows_input_automation`、`cargo test -p astra-player --test web_input_automation`、`cargo test -p astra-player-vn --test native_vn_host_source` 和 release gate 测试覆盖 transcript v2、scenario-derived physical input、Runtime route/terminal evidence、原子 save transaction/abort、Player save integrity、timeline/audio completion、decode/audio resource cleanup、drain meter、direct `route_scenario`/DOM click/JS callback/VN command 阻断；持久 mixer、多 route 独立 session、Web CDP 和同 run visual/audio evidence 尚未闭合，因此保持 `IN_PROGRESS` |
