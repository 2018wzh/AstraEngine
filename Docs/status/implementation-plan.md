...
| 4 | `S2-MEDIA-01` 到 `S2-MEDIA-05` media contract/providers | `IN_PROGRESS` | shared contract、字体 E2、typed FilterGraph、AudioGraph v2、audio-master scheduler、WMF one-shot decode、optional FFmpeg timestamped stream 和 Windows WASAPI/wgpu native media session 已落地；Player/package session identity、GPU FilterGraph、性能预算和正式 Windows E3 未闭合，不能沿用旧 `DONE` |
