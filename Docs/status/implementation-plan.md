...
| `S2-WEB-HOST-01` | `IN_PROGRESS` | Chrome canvas/WebGPU、WebCodecs、OPFS、File/fetch、typed lifecycle/input 与 AudioWorklet queue 已落地，独立 `astra-player-web` 已读取 package profile 并呈现；policy contract 已与 native mlua executor 分层，Web graph 不再包含 Luau C++ VM。当前真实 wasm 编译仍被未分层的 `abi_stable` dynamic loader 阻断，用户手势 audio meter、device/context loss 和正式 Player evidence 尚未闭合 |
