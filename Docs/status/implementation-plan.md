...
| `S3-PRESENT-01` | `IN_PROGRESS` | `SceneCommand` resource upload/release、sprite source rect、glyph run、clip/transform/camera/filter 与扩展 StageModel 已通过 CPU reference tests；timeline scheduler 已接 Windows/Web completion。NativeVN Player 已从 package/VFS 唯一 entry 做 bounded audio read、hash 与 codec signature 校验；共享 `NativeVnProductAudioHost` 已供 Windows/Web 使用，覆盖 user-activation resume、设备格式协商、bounded sinc resampling、受限声道映射、真实 sample mixer、BGM loop、bus fade、pause/resume/stop、stable voice completion、queue backpressure/underflow、时长感知 drain 和显式 close。设备热切换恢复、Windows wgpu/WebGPU command stream 和 formal E3 evidence 尚未闭合 |
...
| `S3-PLAYER-AUTOMATION-01` | `IN_PROGRESS` | `cargo test -p astra-player-core`、`cargo test -p astra-player --test windows_input_automation`、`cargo test -p astra-player --test web_input_automation`、`cargo test -p astra-player-vn --test native_vn_host_source --test product_audio_host` 和 release gate 测试覆盖 transcript v2、scenario-derived physical input、Runtime route/terminal evidence、Windows/Web 原子 save/load、Player save integrity、共享 timeline/audio completion、persistent mixer、decode/audio cleanup、drain meter、direct `route_scenario`/DOM click/JS callback/VN command 阻断；多 route 独立 session、真实 Web CDP browser run 和同 run visual/audio evidence 尚未闭合，因此保持 `IN_PROGRESS` |
