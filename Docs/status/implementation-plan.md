...
2026-08-23 AstraEMU ABI v9 增量：Product Runtime Provider 已切到 ABI v4，Family fingerprint 为 `astra.emu.family_abi.v9`，surface lease 修正固定到 `3f9a771ac`。Minori 必须使用 `Native + MultiLayer`，在 framebuffer acquire 前调用同步 Hook，并通过独占可写 Host surface、retained `Layer2D` transaction 和 writable-file port 完成呈现与存档。旧 root export 与 provider-result payload 已从生产入口删除，Minori library 定向编译通过；family-owned raster、MultiLayer transaction 和 provider tests 尚未迁移，当前呈现保持明确阻断。既有 ABI v8 Headless 报告只保留为历史行为基线，不能关闭当前 E2。
