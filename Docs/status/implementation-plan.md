...
FVP 已在 Sandbox 的独立游戏副本完成一次原生存读档往返：新游戏进入剧情，在空槽位保存并显示缩略图与时间，推进文字后读取本轮存档，恢复保存时的文字和画面；退出系统页后可再次推进到下一句。测试保持同一 GPU 会话和 RDP 连接，使用显式 NullAudioDevice。此项不包含跨进程读档、实际声音、损坏存档或完整结局；原有源存档未改写。终之空完整 Classic 路线继续运行，已越过第 2681 条物理输入；优化版 Release Headless 构建完成，尚未用该构建完成路线或性能验收。
