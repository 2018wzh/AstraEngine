...
| Stage 2 Media + Package | `IN_PROGRESS` | 生产完备度审查已重开本 Stage。VFS/container/schema/scenario authority 与 Cook transactional dependency pipeline 已完成；`astra.plugin_extension_registry.v2` / `astra.provider_policy.v2` 已由 Package、Runtime test、Release Gate 和 VFS 共用 typed validator，阻断 binding/hash/capability/fingerprint/target/profile drift。仍缺非 Web renderer/media 设备恢复、队列、资源释放和性能门禁。Migration 11 另行重开完整 Headless Platform 口径：当前没有 `astra-platform-headless`、统一 JSONL 物理输入、真实 PNG/WAV artifact、全 Runtime test 收束、模型审查或真实平台 preflight link；九个 `S2-HEADLESS-*` 均为 `SPEC_READY`。这些缺口关闭前不能恢复 `DONE`；Linux/macOS/iOS/Android 仍属于 Stage 6 |
...
| `S2-WINDOWS-HOST-01` | `IN_PROGRESS` | `host_backend`/`host_services` 已覆盖 real window、hardware wgpu ordered frame、malformed frame rollback、resize、present/readback、WASAPI bounded queue/callback meter/close drain、WMF diagnostic continuity 与 failed-sequence retry；可注入 device-loss、设备变化、正式 conformance/Player continuity 尚未闭合 |
...
| 4 | `S2-MEDIA-01` 到 `S2-MEDIA-05` media contract/providers | `IN_PROGRESS` | shared contract、字体 E2、typed FilterGraph、AudioGraph v2、`astra.media_playback.v1` audio-master scheduler/seek/EOS/snapshot、显式 decode binding、WMF one-shot decode 和 optional FFmpeg timestamped stream 已落地；stream→scheduler→WGPU/WASAPI 产品 session、GPU FilterGraph、设备变化、性能预算和 Windows E3 未闭合，不能沿用旧 `DONE` |
