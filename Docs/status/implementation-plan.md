...
2026-08-23 AstraEMU ABI v9 增量：Product Runtime Provider 已切到 ABI v4，Family fingerprint 为 `astra.emu.family_abi.v9`，typed filter graph 修正固定到 `e6bc3d960b87373160acd8507faeac4cc589975b`。Minori 使用 `Native + MultiLayer`，并已将 resource scene 拆为 background、foreground、effect、panel 四层：图像由 `image` 解码、由公共 `CpuRendererProvider` 栅格化，再按 stride 写入独占可写 Host surface 并提交 retained `Layer2D` transaction。动态 loader 现一次绑定 VFS、surface、Hook 和 writable-file；公共 support 层持有有界 retained surface store，Product host 会阻断 presentation lane 混用。Layer filter 在 Family 与 Product 边界均保留 typed node/target/parameter，不再使用 string/hash resolution。旧 root export 与 provider-result payload 已从生产入口删除，storage test 已切到同步 writable-file，既有 119 个 library tests 与新增 surface 用例通过。同步 translation Hook 和 CosmicText text surface 尚未迁移；CLI、Manager 与 Headless composition也未完成。既有 ABI v8 Headless 报告只保留为历史行为基线，不能关闭当前 E2。
