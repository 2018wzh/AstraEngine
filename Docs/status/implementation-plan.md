...
| Stage 3 AstraVN | `IN_PROGRESS` | Migration 6 frontend 与 Migration 9 shared 1–3 focused implementation 已落地。Engine workspace 的 NativeVN 自动化改用仅供测试的 `minimal` profile，覆盖 cook、package、完整 system page、物理输入和 Headless checkpoint；`minimal` 不能进入 release。旗舰内容和正式 Windows/Web E3 仍按独立验收推进。TsuiNoSora 已移出 Engine workspace 测试，不再作为 Engine 主线门禁 |
...
| `S3-TSUI-INTERNAL-DEMO-01` | `IN_PROGRESS` | TsuiNoSora 相关测试已从 `Engine/` 移除，不再进入 Engine workspace test、NativeVN `minimal` 或主线提交门禁。现有 `Tools/TsuiNoSora` 与私有素材验证保留为独立工作流，其结果不能关闭 Engine/NativeVN 状态 |
| `S3-TSUI-GATE-01` | `IN_PROGRESS` | TsuiNoSora 专用 Rust/CLI/Player 测试已从 Engine 测试集合删除。`Tools/TsuiNoSora` 的工具测试和私有验收继续独立维护，不属于本次 Engine 验证范围，也不阻断 NativeVN `minimal` profile |
