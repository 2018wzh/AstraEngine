...
| 21 | `S5-MINORI-VFS-01` | `IN_PROGRESS` | `astra-emu-minori`、PAZ v0-v2/分卷 reader、trusted Luau decoder registry、private cache、viewer、desktop extract、纯 Rust GARbro NRBF importer 与 CP932 `.sc` lossless parser 已接入；真实 scheme 生成补丁通过 9837 个 entry 全读，同 identity 第二轮为 9837 cache hits/0 misses，89 个脚本的 payload-free census 通过；Linux FUSE、macOS 和未确认 operand 语义仍开放 |
