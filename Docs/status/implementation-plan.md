...
Modern 当前仍处于定向修复和路线重测阶段。已修复的三个直接问题是：Modern choice controller 生成了错误的 `root/choices` 焦点路径，标题音频引用未进入 sidecar/cook 资源集合，以及 Automation 把异步媒体 fence 反向配对到禁跳过对白、随后又只用一次 Enter 越过文字渐显；生成器现在保持媒体、对白渐显和物理 Enter 的顺序，Runtime 的 Skip All/Skip Read 也会尊重脚本的 `skip_allowed=false` 并在恢复后继续跳过。Modern007 曾因沿用 100,000 的过小 submitted-frame 上限在长流程中触发 `QueueOverflow`，已按既有 Classic profile 的 864,000 上限准备后续运行。Modern008 的首次私有采样把 checkpoint 放在 reveal await 起始 tick，实际运行触发 `ASTRA_HEADLESS_AWAIT_TICK_SHIFT_INVALID`；该失败产物保留，Modern009 已将 checkpoint 放到 await 预算结束、下一次 Enter 之前并通过 `validate-input`，尚未完成新的 GPU 路线结果。上述修复和定向回归不关闭 Modern Player、系统页、真实音频、完整 37 路线或真实 Windows Player 验收。
