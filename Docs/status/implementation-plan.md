...
FVP 混音器增加只读 `voice_count(kind)`，让 Manager 错误日志分别记录容量占用和 Playing 数量。回归确认暂停与淡出阶段仍占用槽位，淡出结束后才释放；本次不改变混音、容量或调度。RFVP 重新整理为上游 0.6.0 基线加单个本地适配提交 73d3b44，旧本地适配提交保留在备份分支，未推送。对应 gitlink、Family 构建身份与 MODIFICATIONS.md 同步更新；真实同存档重测尚待执行。
