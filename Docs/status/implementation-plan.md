...
CMVS 解码缓存、范围读取和流复用 Engine `OwnedByteBuffer`，缓存命中不再复制完整条目。新回归直接检查共享分配，并验证缓存/归档释放后既有范围与流仍有效，源文件变化仍拒绝新读取。151 项普通测试、全目标 Clippy、格式和文档检查通过；本次不新增真实 GPU 或游戏完成记录。
