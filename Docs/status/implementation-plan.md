...
Manager family-mounted PNG/JPEG/BMP/WebP preview now binds `astra.decode.image` through `DecodeProviderRegistry` and transfers bounded RGBA8 pixels to Slint. Family audio previews bind `astra.decode.symphonia` explicitly and expose bounded metadata; Windows RIFF/AVI video now binds the explicit Media Foundation provider, while proprietary ANI/SQZ and targets without a bound video provider remain open.
