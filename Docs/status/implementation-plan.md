...
2026-08-28 Family ABI v10 hard cut: `astra.emu.family_abi.v10` adds the typed `LegacySystemMenuRequestV1` to the step contract. Manager converts a pressed physical `pointer.secondary` edge, including the latest bounded pointer coordinates, into `Open` and strips the duplicate generic edge. Minori validates that request before opening the family-owned Save page, requires a stable gameplay wait and the writable-file host service, and blocks ambiguous input, active choices/media, completions and missing host bindings. Family API wire round-trip and Minori provider coverage are included in the focused tests. A Sandbox direct native run then reached real Minori scenes and completed its windowed route with audio explicitly disabled; unavailable WASAPI output and a read-only share still block the audio/artifact portions of Windows E3, so no E3 artifact was recovered.
