...
2026-08-23 AstraEMU ABI v9 增量：Product Runtime Provider 已切到 ABI v4，Family fingerprint 为 `astra.emu.family_abi.v9`。Minori 必须使用 `Native + MultiLayer`，在 framebuffer acquire 前调用同步 Hook，并通过 Host-owned surface、retained `Layer2D` transaction 和 writable-file port 完成呈现与存档。旧 scene/snapshot/text/session-resource/budget 接口已删除且不保留兼容层。当前 Minori consumer 的定向编译仍在 79 个旧调用点阻断，既有 ABI v8 Headless 报告只保留为历史行为基线，不能关闭当前 E2。
