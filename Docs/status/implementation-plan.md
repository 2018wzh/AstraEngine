...
2026-08-13 立绘与 GPU follow-up：IDA 对原程序 `CCharLayerManager` 的复核确认 `.char trans <slot> <duration_ms> <opacity>` 是阻塞式线性透明度 transition，`.char vis` 使用首字符布尔规则。runtime state 已硬切 v22，并保存起止透明度、持续时间、已过时间和完成状态；provider 在等待期间持续提交 retained Scene2D。合成 runtime/provider 测试覆盖中间帧、最终帧、snapshot continuation 和非法 operand；256→128→0 三个状态又通过 Headless WGPU capture 与人工视觉检查。真实 290 条 `.char` 只有 `load/pos/keep`，没有 `trans/vis`，因此这项只关闭合成动态立绘 E2，不能冒充真实路线覆盖。clean Release 120 Hz run 已完成 36600 fixed tick、73200 presentation、Perfetto、performance report 和 trace manifest；runtime p99 0.3491 ms、presentation p99 0.92406 ms，其余预算项通过，但 2 次 presentation 超过 8.333 ms，零 deadline miss 门禁继续 blocking。
