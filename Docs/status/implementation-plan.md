...
Control 快进增量：IDA 复核确认 `.pragma enable_control`/`.pragma disable_control` 与 `.pragma skip_enable`/`.pragma skip_disable` 是两组独立 gate。当前 Minori runtime state v23 保存 gate、Control pressed/released 与 `Normal/Auto/Skip` 三态；有效快进由这些权威状态推导，不保存重复布尔值。Host 仍逐 tick 推进，不自动完成 media/presentation fence 或 provider completion。未知 pragma 返回 `ASTRA_EMU_MINORI_RUNTIME_PRAGMA` blocking diagnostic；同一签名动态 plugin 的既有 Control Headless sequence 完成 300 fixed steps、54 个呈现帧、9 条输入、snapshot round-trip 和非静音音频，diagnostic 为 0。活动消息的 Auto 切换现通过同 token `Input`/`Time` modality 重绑定生效，最快 Auto 把设置值 `0` 映射为一个 10 ms timing unit。真实八包持久 Auto 首路线推进 25552 fixed steps、25557 个 GPU frame并到达 terminal；正文阶段没有周期性 Enter，snapshot round-trip、coverage identity 与零 diagnostic 均通过。Skip 整路线仍未关闭。
