...
| `S3-PRESENT-01` | `IN_PROGRESS` | `SceneCommand` resource upload/release、sprite source rect、glyph run、clip/transform/camera/filter 与扩展 StageModel 已通过 CPU reference tests；timeline scheduler 已接 Windows completion。NativeVN Player 已从 package/VFS 唯一 entry 做 bounded audio read、hash 与 codec signature 校验，并在 Windows 主链执行 decode/open/submit/drain/close、decoded hash/PCM contract/meter 校验、失败 close 与 `voice_end` completion；持续 BGM/loop 会显式返回 `ASTRA_PLAYER_AUDIO_PERSISTENT_UNSUPPORTED`。持久 mixer voice、Windows wgpu/WebGPU command stream 和 formal evidence 尚未闭合 |
