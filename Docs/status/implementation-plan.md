...
| `S3-PRESENT-01` | `IN_PROGRESS` | `SceneCommand` resource upload/release、sprite source rect、glyph run、clip/transform/camera/filter 与扩展 StageModel 已通过 CPU reference tests；NativeVN provider 把 `astra.vn.timeline_task.v1` 作为 validated effect output 返回，Player timeline scheduler 覆盖 start/cancel/deadline/capacity/clock regression，Windows host 用单调时钟将真实 scheduler completion 回注 `complete_wait`。Windows wgpu/WebGPU 同 command stream、真实 decode/audio callback 和 formal evidence 尚未闭合 |
