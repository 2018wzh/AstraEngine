...
| 21 | `S5-MINORI-VFS-01` | `IN_PROGRESS` | VFS/decrypt/factory contract 已硬迁移到 `astra-emu-family-core`，公共 Luau/profile/cache/viewer/verify/extract/FUSE 进入 `astra-emu-family-support`；通用 CLI 为 `vfs --family`，Minori import/census 使用独立 CLI。Minori 只保留纯 Rust decrypt provider，Luau 仅注册 data-only private profile。真实 Release verify 覆盖 6 source、9837 entry、29720 range read 与 2403596354 decoded bytes，同 identity 复核全 cache hit 且聚合 hash 一致；89 脚本 census 为 33728 行、33695 command、29 token、0 unknown opcode。Linux FUSE、macOS、Manager media preview 和未确认 operand 语义仍开放 |
