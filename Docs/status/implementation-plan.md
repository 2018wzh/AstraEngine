...
2026-07-30 增量：Runtime、Action/Plugin ABI、Product Runtime Provider ABI 已一次性切到 v3/v2 breaking contract。Runtime 使用 inverse journal/overlay transaction、增量状态根、强制 Action access/StableId 声明和 deterministic conflict-DAG；NativeVN 使用 hot/cold history、mutation journal、四 presentation region 与统一 worker budget。Provider factory/session、opaque FFI session、ordered mailbox 和 Headless `astra.headless_session_batch.v2` 已落地。Batch runner 现在按配置上限、job 数与硬件并行度自动选择并发度，并按子进程 PID把 route/replay/performance 的串行和并行 private-memory peak 全部写入报告。clean Release build `e679d107` 的四 job batch 在 24-way 主机自动选择 4 个并发 session，串行基线 899,059 us、并发 298,801 us，四条输出 identity 一致，worker utilization 976 permille。相同 clean build 的集显 DX12 1920×1080、120 Hz、72,000 帧正式运行通过：CPU p95 665,300 ns、GPU p95 478,210 ns、e2e p99 1,257,480 ns、deadline miss 为零、private memory max 281,083,904 bytes、增长 max 29,151,232 bytes，稳定段 allocation/upload/readback 均为零。workspace clippy/test、动态导出预算、NativeVN content validation 与真实旗舰 FFmpeg Headless 功能 E2 均通过。product-route 长流程与 100k 正式报告仍开放，因此相关工作项保持 `IN_PROGRESS`。
...
| --- | --- | --- |
| Stage 1 EngineCore | `IN_PROGRESS` | Runtime v3、Action ABI v2、Provider ABI v2、inverse journal/overlay transaction、增量 root、conflict-DAG、session worker 与全局 worker budget 已通过 workspace test；shipping/evidence integrity profile 已分离。正式 clean Release 性能与规模 evidence 尚未闭合，Stage 1 暂不按新契约恢复 `DONE` |
...
| `S1-RUNTIME-V3-01` | `IN_PROGRESS` | `cargo test -p astra-runtime` 覆盖 v3 save/replay 拒绝旧 schema、inverse journal、增量 root、shipping/evidence integrity、Action ABI v2 fail-closed access/StableId contract 与 1/2/4/8 worker conflict-DAG hash 一致；clean Release `e679d107` 的集显 DX12 1920×1080、120 Hz、72,000 帧正式运行通过，CPU p95 665,300 ns、e2e p99 1,257,480 ns、deadline miss 为零、private memory max 281,083,904 bytes、增长 max 29,151,232 bytes，稳定段 allocation/upload/readback 均为零。`cargo clippy --workspace --all-targets -- -D warnings` 和 `cargo test --workspace` 通过；100k product-route 正式报告仍开放 |
| `S1-RUNTIME-SESSION-BATCH-01` | `IN_PROGRESS` | `cargo test -p astra-plugin --test concurrent_runtime_host --test session_batch` 与 `cargo test -p astra-worker-budget` 覆盖 per-session ordered mailbox、跨 session 并行、poison 隔离、FIFO budget 和稳定排序；clean Release build `e679d107` 的四 job batch 在 24-way 主机上从配置上限 8 自动选择 4 个并发 session，逐 session 串行/并行 output identity 一致，serial baseline 899,059 us、concurrent wall time 298,801 us、throughput 13.386 sessions/s、peak workers 4、utilization 976 permille，所有 job 的串行/并行 private-memory peak 均由 PID sampler 写入报告。多 session performance job p95/p99 仍开放 |
