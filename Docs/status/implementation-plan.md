...
Minori 已准备独立的《夏空的英仙座》18 分卷测试副本，原游戏目录保持只读。新增 `list-garbro-titles`，与导入共用有界 NRBF graph，只列出 PAZ scheme 名称，不输出密钥或写文件；重复名称和损坏结构拒绝。实际数据库列出 19 项，以 `Natsuzora no Perseus` 成功生成新私有配置。CLI 11 项测试与 Clippy 通过，并修正文档中 `--profile` 相对游戏目录的用法。真实脚本盘点与已知入口的 GPU 启动已开始，尚未确认完成，不能据此关闭真实游戏或结局验收。
