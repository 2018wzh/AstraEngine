...
随后用同一真实 profile 的 `launch_mode=direct`、默认 `test.sc` 和 `render_scale=1.0` 做了 750 个固定 60 Hz tick 的物理 Enter 对照。日志中的 GPU 创建为 DX12 discrete GPU；启动初段的黑帧之后，抽样帧出现原生背景和稳定日文对白，未复现永久文字消失。该对照只确认 direct 入口的短流程，不替代 title 存读档、完整路线或 Sandbox 窗口验收。
