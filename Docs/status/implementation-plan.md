...
| Stage 3 AstraVN | `IN_PROGRESS` | AstraVN 已迁到 `Engine/Source/Modules/AstraVN/`，并拆成 `astra-vn-script`、`astra-vn-core`、`astra-vn-policy`、`astra-vn-presentation`、`astra-vn-commands`、`astra-vn-system`、`astra-vn-save`、`astra-vn-package`、`astra-vn-plugin`、`astra-vn-editor`、`astra-vn-runtime-provider` 和 facade-only `astra-vn`。`NativeVnRuntimeProvider` 已通过 in-process provider 和 DTO+FFI smoke 接入，`astra-test` scenario runner 通过 selected provider 推进 VN scenario，`astra-package`/`astra-release` 复用 `provider.policy` 和 `plugin.extension_registry` 校验 `runtime_provider.binding`、`runtime_provider.native_vn`。已落地范围仍覆盖 `.astra` parser/compiler、VN Core、Luau policy、presentation/system/save/package/plugin/editor metadata、NativeVN sample cook/package/full playthrough、Windows/Web bundle route report、VN package sections、VN Scenario DSL slice、route-bound `mount_assets` 和 `vn.*` release gate。Stage 3 顶层仍是 `IN_PROGRESS`，因为 Windows/Web live player automation、TsuiNoSora 完整 Director/Shockwave cast parser/source-map reader、真实本地 conversion gate 和正式 release signoff 仍未完成 |
...
| 13 | `S3-TSUI-INTERNAL-DEMO-01` TsuiNoSora internal classic full-resource demo bundle | `IN_PROGRESS` | 本期只覆盖 `tsuinosora-internal-game` 的 `classic` profile，但完成条件是全量资源转换、原体验还原和 100% 可玩：ProjectorRays 本地 dump adapter、full dump coverage report、NativeVN asset sidecar、cooked asset package section、`asset.vfs_manifest`/`asset.catalog` package VFS evidence、同一 `.astrapkg` 派生 Windows/Web bundle 和 live automation report release check 入口已落地；当前阻断是 2527 个 ProjectorRays binary chunk 尚无 converted resource evidence。`modern`、Patch-only、Runtime Patch/VFS 插件移出本期 |
...
| 16 | `S3-RUNTIME-PROVIDER-01` NativeVN gameplay runtime provider | `DONE` | `astra-vn-runtime-provider`、DTO+FFI shape、provider.policy binding、release gate 和 scenario runner provider path 已落地 |
