...
2026-08-23 ABI v9 consumer 更新：分支已同步 typed filter graph 修正 `e6bc3d960b87373160acd8507faeac4cc589975b`。CLI、Manager 和 Minori 已改用 Host-owned surface、retained Layer2D、同步 Hook 与 writable-file port；Minori 文字由 family-owned CosmicText/Astra Renderer2D 生成，不再依赖已删除的 text lease。当前只形成 E1：Manager per-layer typed filter graph 尚未执行，Windows 私有目录缺 current-user ACL 断言，Headless 尚未用真实样本重跑。旧 ABI v8 路线和性能报告继续作为行为基线，不能关闭当前身份的 E2/E3 门禁。
