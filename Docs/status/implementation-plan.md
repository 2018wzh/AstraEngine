...
| `S1-RUNTIME-V3-01` | `IN_PROGRESS` | `cargo test -p astra-runtime` 覆盖 v3 save/replay 拒绝旧 schema、inverse journal、增量 root、shipping/evidence integrity、Action ABI v2 fail-closed access/StableId contract 与 1/2/4/8 worker conflict-DAG hash 一致；`cargo clippy --workspace --all-targets -- -D warnings` 和 `cargo test --workspace` 通过。clean Release 120 Hz 集显性能、100k 长流程正式报告和长期内存门禁仍开放 |
| `S1-RUNTIME-SESSION-BATCH-01` | `IN_PROGRESS` | `cargo test -p astra-plugin --test concurrent_runtime_host --test session_batch` 与 `cargo test -p astra-worker-budget` 覆盖 per-session ordered mailbox、跨 session 并行、poison 隔离、FIFO budget 和稳定排序；当前 dirty-checkout build identity `sha256:d4a06a4eb37ecf09d03896bb05c2ff9a76d09457e244d41ee019a8716b6f3e7e` 的四 job batch 在 24-way 主机上从配置上限 8 自动选择 4 个并发 session，逐 session 串行/并行 output identity 一致，serial baseline 6,074,290 us、concurrent wall time 2,053,044 us、peak workers 4、utilization 956 permille，所有 job 的串行/并行 private-memory peak 均已由 PID sampler 写入报告。正式 clean Release batch report 和 performance job p95/p99 仍开放 |
