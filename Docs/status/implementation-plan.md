...
| 4 | `S2-MEDIA-01` 到 `S2-MEDIA-05` media contract/providers | `IN_PROGRESS` | shared contract、字体 E2、typed FilterGraph、AudioGraph v2、`astra.media_playback.v1` audio-master scheduler/seek/EOS/snapshot、显式 decode binding、WMF 与 optional FFmpeg 真实 one-shot decode 已落地；decoder→scheduler→WGPU/WASAPI、GPU FilterGraph、设备变化、性能预算和 Windows product E3 未闭合，不能沿用旧 `DONE` |
