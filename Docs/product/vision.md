...
- AstraEMU 使用 Manager + `AstraEmuRuntimeProvider` + RuntimeWorld + in-process family plugin + `LegacyRuntimeProvider` facade，统一管理通过 auto probe 和 profile override 完成，现代化能力通过 TranslationProvider、Asset VFS 和 FilterGraph preset 接入；family 私有 key 由有界文件接口交给纯 Rust reader，不提供 Luau patch/decode。
