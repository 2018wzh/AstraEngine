...
| Stage 3 AstraVN | `.astra` 编译、AstraVN Rust dylib facade、Luau policy、Graph/Timeline 同源、NativeVN Game target、商业 VN 系统 UI、标准命令库、演出模型、完整 playthrough scenario 和 Windows/Web live player automation。 | `astra-vn` 产出 `rlib`/Rust ABI `dylib`；`.astra` sample 完成 dialogue、choice、backlog、auto、skip、save/load、config、video、system UI、advanced presentation opt-in、Game target 和 replay hash gate；Windows/Web `player.full_playable` 必须由平台原生输入、input transcript、视觉变化、音频 meter 和 host evidence 证明。 | [stage-3-astra-vn](../status/stages/stage-3-astra-vn.md) |
