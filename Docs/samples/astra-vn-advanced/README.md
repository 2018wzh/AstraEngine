...
| 文件 | 用途 |
| --- | --- |
| [project.yaml](project.yaml) | profile、command provider、Luau policy 和 system story 绑定 |
...
```bash
astra package build Examples/AdvancedVN --out target/advancedvn.astrapkg
astra test run scenarios/advanced_presentation.yaml --package target/advancedvn.astrapkg --headless --report target/reports/advanced-vn.yaml
astra package validate target/advancedvn.astrapkg --profile advanced-vn --report target/reports/advanced-release.yaml
```
