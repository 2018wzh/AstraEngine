...
2026-07-16 起，`tsuinosora.native_story_ir.v1` 是唯一可生成私有 NativeVN 项目的剧情输入。它必须完整表达 score/frame、状态、分支、媒体、文本/选择、等待点和物理输入自动化，并由 `tsuinosora.full_conversion_coverage_report.v1` 证明全部 source、handler、command、route、terminal、choice 和 media 已转换。仓库已加入 Classic/Modern Yakui UI project template，Modern 作为默认 profile，仅包含 Title、Quick Panel、Save、Load、Backlog、Auto、Skip 和 Config；两套 profile 均保持 800x600/4:3。当前只完成严格资源转换与模板/contract 验证，尚缺 `VWSC` score 和全部 Lingo handler 到 typed IR 的完整转换，所以本分支没有新的 package、Windows/Web bundle、Headless 全路线、Windows E3 或视觉 signoff。旧生成物不得补作当前证据。
