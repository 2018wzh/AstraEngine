...
本页是 Stage 3 的执行蓝图。当前仓库已有 `astra-vn` 基础 slice、VN package/release gate slice，以及 `Tools/TsuiNoSora` 的脱敏 inventory、direct-readable extract preflight、Director `imap`/`mmap` resource map preflight、受限 `XFIR` RIFF/RIFX exact wrapper reader、Director `KEY*`/`CAS*` cast map preflight、Director `Lctx`/`Lnam`/`Lscr` Lingo map preflight、从 Director cast map 与 child resource id/FourCC/extracted payload hash 派生 cast source map、受限 RIFF/RIFX readable chunk reader、opaque/compressed `XFIR` 与尾随未验证 bytes reader-required 阻断、ProjectorRays 本地 dump adapter、ProjectorRays full dump coverage report、可读 mapped `Lscr` 自动生成脱敏 source-map sidecar、script source map report、route graph report、visual reference report、Asset analysis、conversion report、modern profile report、route scenario、mount policy helper、NativeVN package input writer、NativeVN asset sidecar、project-level `package_sections`、TsuiNoSora package section release gate、公开 synthetic internal/patch player route gate、公开 synthetic patch Web direct-read route check，以及 `player.full_playable` report 校验。`S3-TSUI-INTERNAL-DEMO-01` 的 repo-side pipeline 已落地，私有 `.local` full dump 已能生成 `tsuinosora.projectorrays_full_dump_report.v1`；当前 blocker 是 full-resource conversion 尚未实现，2527 个 ProjectorRays binary chunk 的 converted evidence 为 0，`internal-demo-bundle` 会在 cook/package 前返回 `TSUI_INTERNAL_DEMO_FULL_RESOURCE_CONVERSION_BLOCKED`。真实 internal playable bundle 尚未产出；Stage 3 总体仍是 `IN_PROGRESS`。完整 ProjectorRays raw chunk 到 NativeVN asset/command 的转换器、formal release signoff、modern、Patch-only 和 Runtime Patch/VFS 插件仍是后续计划，不能把本页写成 Stage 3 `DONE` 证据。
...
该命令只在 full-resource ProjectorRays conversion coverage、`demo-slice`、cook/package、Windows/Web bundle manifest 和 `player.full_playable` release check 都通过时输出 `tsuinosora.internal_demo_bundle_report.v1 status: pass`；缺 `.local` config、缺 ProjectorRays tool/dump、缺私有源、缺 converted resource evidence 或缺 live report 时必须 blocking。模板和 scaffold 只写 repo-relative `.local` 示例路径，不写本地绝对路径；真实 Windows/Web bundle 仍需要把合法私有源、ProjectorRays dump、converted NativeVN assets 和 live player report 填入 ignored config。
