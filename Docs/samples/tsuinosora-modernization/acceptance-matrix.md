...
原版首句实际保存、推进和读取后直接返回正文；Classic 原先读档恢复了保存页，已改为完整恢复后执行系统返回。局部回归检查剧情 cursor/等待保留，以及显式快照恢复仍保留系统页；新版真实 Player 复测待完成。原版系统窗口部分文字在测试环境显示方块，当前只确认可辨认的页签、槽位及操作结果，未据此宣称字体对齐。
