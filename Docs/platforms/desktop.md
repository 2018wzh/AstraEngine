...
| `S6-MACOS-HOST-01` | macOS | `IN_PROGRESS` | 主线程 AppKit/winit runner、Metal/wgpu、IME/gamepad、CoreAudio、Application Support、AVFoundation、Universal 2 `.app` 和 packaged Player 已接线；等待 osxcross 静态编译及真实 macOS evidence |
...
macOS 静态目标是 macOS 13 Universal 2。Linux 开发机将完整、可重定位的 osxcross 放在 ignored 的 `.tmp/osxcross/`，其中必须有 `bin/o64-clang`、`bin/oa64-clang` 和 `SDK/MacOSX13.3.sdk`。`Tools/run_macos_cross.py` 会校验这些文件，并将工具链只读挂入 `cross` 容器；缺项时直接停止，不回退到宿主 Cargo。Apple SDK 不得提交到仓库。

当前只执行两个 Apple target 的 `check` 和 `clippy`。AppKit、Metal、CoreAudio、AVFoundation、CGEvent、ScreenCaptureKit、AccessKit、codesign、notarization 和 Player 同 run evidence 必须在 macOS 13+ 真机补齐。cross compile 只计 E1。
