...
| `S2-WINDOWS-MEDIA-SESSION-01` | Windows | `IN_PROGRESS` | 显式 `[wmf, ffmpeg]` fallback profile 可运行 FFmpeg timestamped stream→audio-master scheduler→WASAPI/wgpu，真实测试覆盖视觉、音频 meter、pause/resume、seek、backpressure、device-loss recovery 和资源释放；Player/package identity 与性能 gate 尚未闭合 |
