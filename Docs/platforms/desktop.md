...
| `S2-WINDOWS-MEDIA-SESSION-01` | Windows | `IN_PROGRESS` | Windows release profile 固定唯一 WMF decoder、Kira mixer 与 WASAPI output；多 provider decode fallback 已阻断。typed stream、pause/resume、seek、backpressure、device-loss cleanup 与资源释放已有局部测试；正式真实线路、同 revision Perfetto、Player/package identity 和 10 分钟 Windowed evidence 尚未闭合 |
