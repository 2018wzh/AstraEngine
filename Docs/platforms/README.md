...
Headless 不属于第七个发布平台。[Migration 11](../migrations/headless-platform-test-backend-migration.md) 规划独立 `HostKind`、测试 profile 和 `publish = false` 的完整后端，用于平台无关测试与真实平台验收前置；当前状态为 `SPEC_READY`，不能计入六平台完成度。
