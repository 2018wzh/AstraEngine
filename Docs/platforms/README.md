...
六平台 host contract、Target binding、capability report 和 profile gate 见 [Target And Platform Blueprint](../implementation/target-platform.md) 与 [Platform Host Blueprint](../implementation/platform-host.md)。Migration 8 当前为 `IN_PROGRESS`：Windows/Web 仍待完整同 run evidence；Linux、macOS host 和 packaged Player 已进入静态实现；Android 已进入 GameActivity/Gradle、bundle identity 与 native host 实施，但设备 E3 尚未闭合；iOS 继续使用显式 `Unavailable` factory。真实验收留在 [Stage 6 Platform Completion](../status/stages/stage-6-platform-completion.md)。
