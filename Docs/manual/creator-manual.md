...
开发期可以先检查 UI 产物，再生成单页快照或语言/分辨率矩阵：

```bash
astra ui check Examples/NativeVN/project.yaml --profile classic --target nativevn-game
astra ui preview Examples/NativeVN/project.yaml --profile classic --target nativevn-game --locale ja --width 1280 --height 720 --out .tmp/ui-preview.png
astra ui snapshot Examples/NativeVN/project.yaml --profile classic --target nativevn-game --locale ja --width 1280 --height 720 --out .tmp/ui-snapshot
astra ui matrix Examples/NativeVN/project.yaml --profile modern --target nativevn-game --locales en,ja,zh-Hans --sizes 1280x720,1920x1080 --scales 1.0,1.5,2.0 --out .tmp/ui-matrix
```

这些命令会重新 Cook 项目，并从 packaged `CompiledVnProject` 启动产品 UI。`snapshot` 输出 PNG、脱敏语义树、Scene2D 命令计数/hash 和性能报告；它不会把字形位图、纹理或商业文本写进 scene evidence。开发快照中的性能结果只用于定位问题，不能替代 release-reference hardware report 或 Windows/Web E3。
