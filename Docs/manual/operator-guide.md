...
读取时发现损坏、旧格式或不属于当前包的存档，会保留当前游戏，将该槽显示为不可用并禁止覆盖；可返回剧情或选择其他槽。设备故障及恢复提交后的失败仍会结束会话并显示错误。

Player 的新窗口按显示器工作区放置；窗口缩放不改变剧情逻辑尺寸。新保存时间使用系统本地时间，无法取得时区时会显示 UTC 后缀，旧槽时间不重写。
