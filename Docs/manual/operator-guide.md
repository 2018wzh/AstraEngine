...
桌面分发统一由 `python Tools/build_astraemu_desktop.py --output Build/AstraEMU --family <family>` 构建。该入口只接受本机 target，并在同一个 Release target root 中构建 Manager、CLI 和选定 family。Windows MSVC 构建器强制启用 `+crt-static`，成品不依赖机器上另行安装的 VC runtime DLL；调用环境若明确要求动态 CRT 会直接阻断。Windows Musica 默认显式绑定 `wmf`；需要 FFmpeg 时使用 `--video-provider ffmpeg-vcpkg`，构建器才会给 Manager 与 CLI 启用相应 feature。成品只记录并使用所选默认 binding，运行时失败不会切换到另一 provider。构建器先从成品 ABI root module 校验 build-script descriptor，再生成签名 manifest，把两个 Program、动态库、manifest 和第三方 notices 原子提交到分发目录。`astra.emu.desktop_package_evidence.v1` 只记录相对文件名、hash、target、build identity、signer identity 和显式 video provider。`--development-ephemeral-signer` 仅用于本机 E3 调试，私钥只存在于当前进程且不会写盘；它生成的 development 包不得作为正式签名证据。
