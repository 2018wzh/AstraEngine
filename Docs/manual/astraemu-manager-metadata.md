...
独立 Host 首先支持 Windows 和 FVP。点击资料库的添加目录按钮，选择授权游戏目录并扫描；Family 动态库及其依赖放在 Manager 数据目录的 `cores/`，Manager 启动时按平台扩展和 `astra_emu_`（Unix 为 `libastra_emu_`）前缀自动发现，并复用 ABI、descriptor 和 capability 校验。坏文件和重复 `plugin_id` 会按文件显示诊断，其他唯一核心继续可用。替换核心文件后重启 Manager；运行期间不热加载。插件和游戏文件不能在运行期间移动或替换。
...
Manager 使用全新的 SQLite schema 4，不读取或迁移旧资料库。游戏的原生存档由 Family 管理，和 Manager 数据分开。外观页保存主题、配色、密度与视图；选择系统主题时响应 Windows 主题变化。手柄设置可保存为全局默认，也可为选中游戏单独覆盖。清除逐游戏设置后恢复使用全局映射；修改输入或翻译设置前先退出游戏。

滤镜页提供原画、缩放、锐化与 Anime4K 2×，也可加载 Magpie format 4 HLSL。加载后调整参数并保存；编译或保存失败会显示错误并保留旧有效效果。游戏内菜单使用同一组预设。外部效果只接受当前解析器支持的指令，未知指令会报错。Windows 发布包将锁定版本的 `dxcompiler.dll` 与其依赖 `dxil.dll` 放在 Manager 可执行文件旁边；未设置 `ASTRA_EMU_DXC_PATH` 时按可执行文件目录查找，不搜索当前工作目录或系统 PATH。设置该变量时必须指向通过校验的 `dxcompiler.dll`，失败会显示诊断，不改用 CPU 编译。
