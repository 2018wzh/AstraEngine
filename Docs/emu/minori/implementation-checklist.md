...
- [x] 物理 Enter 推进与受 pragma 门控的 Control 快进可跑完首条剧情路线；Control 现在也会把已显示消息的同 token `Input` wait 重绑定为 10 ms `Time`，真实八包以 25496 fixed steps、25498 帧和 28 条输入跑完首路线，正文阶段没有周期性 Enter。backlog 的当前记录、滚轮导航、关闭恢复和当前记录 voice replay 已进入真实完整路线 E2。Auto/Skip 三态、原版菜单命中区和 500 ms Auto wait 已完成定向测试；真实八包也已在最快 Auto 下持续跑完首路线。持久 Skip 菜单路线和 backlog 多记录翻页仍开放。
