...
| Minori runtime provider / `cdylib` ABI | E2 增量 | Family ABI v14 已 hard cut；共享 Host adapter、writable-file、资源/文字 surface、同步 Hook、system-menu/confirmation/system-command/text-input 与四层 `Native + MultiLayer` 已接通，受影响 library/CLI/Manager tests 通过。仍缺 Save 页面注释绘制、完整四路线与正式 Windows host evidence |
