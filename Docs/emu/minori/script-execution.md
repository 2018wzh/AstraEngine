...
同一 effect batch 先用 `astra.emu.text_presentation.v1` 提交不含正文的 `LegacyTextPresentationV1`，再按 lease id 与 `TextCapture` 关联。这个结构保留现有 `LegacyEffect` v1 wire layout，避免在 ABI fingerprint 不变时偷偷改变 postcard 数据。Minori 只声明语言、字体 family、文字区域、字号、行高、最大行数与颜色，Host 必须用显式绑定的 `cosmic_text_cpu` 和仓库内 Noto Sans JP 完成 shaping，再交给 Astra `Renderer2D` 合成。Config 的 `text_shadow` 只控制这一 typed presentation 的既有 2 px 黑色 outline：启用时提交 outline，关闭时提交 `None`，不改变 shaping、layout 或字体身份。缺 provider、字体绑定不一致、stage 不是 1280×720、区域越界、重复或悬空 layout、shaping diagnostic 都会阻断；没有系统字体、位图文字或字符宽度估算 fallback。消息提交后建立非零物理输入 mask 的 await，等待 confirm、space 或主指针输入。
