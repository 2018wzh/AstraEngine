...
The active Minori runtime section is `astra.emu.minori.runtime_state.v28`.
...
`transition` 只配置后续 stage，不自行提交替代帧。`stage` 按已确认顺序更新前景、背景和 stand state。family effect 只保存 VFS URI、编码 hash、尺寸与绘制指令；Headless/Manager Host 通过 session resource channel 读取编码数据，并交给显式绑定的 Astra `DecodeProviderRegistry`。解码后的 RGBA 只存在于 Host 临时渲染帧，不进入 effect、snapshot 或 report，也没有 Minori 私有 renderer。当前 host 绘制路径对已验证的静态 PNG stand 使用中心 X、stage 底边 Y 的锚定几何；非 PNG、尺寸或坐标不能表示时返回 `ASTRA_EMU_MINORI_STAGE_STAND_*`。该静态几何已有合成回归证据，但原版动态资源与 transition 动画时序仍未闭合，不能据此声明原版演出一致。
