...
- 立绘的 `resource_parameter` 传给 `sub_430980`，参与资源可见高度裁剪，不是缩放比例。该函数先扣除图像对象的末端边界值，再在差值为正时减少可见高度；裁剪覆盖全部高度时加载失败。`sub_431030` 根据资源左右边界和宽度计算横向中心，根据裁剪后的高度计算底部对齐；独立的原点模式可以改用显式位置。这些边界值的来源和原点配置尚未完成接入，不能直接把参数加到屏幕坐标。
