...
- FFmpeg demux 读取改为直接调用 `Packet::read`，不再使用会吞掉非 EOF 错误的 `Input::packets()` 迭代器。截断或损坏的媒体现在保留底层错误并返回 `ASTRA_FFMPEG_STREAM_PACKET_READ`，不会静默变成正常 EOF；FFmpeg stream 的真实样本回归仍为 9/9。
