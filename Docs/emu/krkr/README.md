...
KrKr family 覆盖 KiriKiri2/KAG/TJS 常见游戏形态：XP3 archive、KAG `.ks`、TJS source/bytecode、PSB 形态的 `.ks.scn`、TLG/PNG/JPG/UI 资源、BGM/voice/movie 和插件扩展。AstraEMU 的目标是让 compat core 在独立进程内复现旧引擎行为，再把 trace、media block、TextCaptureEvent 和 snapshot 交给 Manager。EngineCore 不接收 TJS VM、旧插件对象或原生 GPU/audio handle。
...
| [runtime-core-design.md](runtime-core-design.md) | AstraEMU KrKr compat core 的最小模块划分 |
...
- IPC 服从 [AstraEMU IPC Contract](../../contracts/astraemu-ipc.md)。
