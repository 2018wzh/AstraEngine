...
KrKr provider session 持有旧 VM 和 KAG 状态机。Manager 只看规范化事件、presentation command、audio command、TextCaptureEvent、snapshot reference 和 diagnostics。
...
| save/load | `LegacySnapshotEnvelope` |
...
`jump`、`call`、`return` 不直接改 Manager 状态。它们先进入 KrKr session 的 scenario stack，再由 session 输出 `StateMachineTrace`。
...
Snapshot 由 provider session 生成 `LegacySnapshotEnvelope`。Astra runtime save 可以引用 envelope 的 opaque family section，但 Manager 不能解析旧 VM 内存。
