...
KrKr compat core 是 AstraEMU 的 family core。它运行在独立进程内，持有 TJS VM、KAG conductor、archive resolver、插件 facade 和旧状态机。Manager 只做窗口、输入、配置、provider selection、报告和 Astra Runtime bridge。
...
| `MediaBridge` | 输出 presentation/audio/movie command 和 shared media block |
| `SnapshotStore` | 保存 legacy VM snapshot，返回 `LegacyVmSnapshotRef` |

这已经是最小划分。不要再为每个 KrKr 插件建 public module；插件能力先统一进 `PluginFacade`。

## IPC

Core 支持 [AstraEMU IPC Contract](../../contracts/astraemu-ipc.md) 的命令：
...
3. 如果必须旧 DLL，限制在 KrKr compat core 进程内，并标记 native plugin requirement。
