...
Musica 尚未接入 [独立 Host](../../migrations/astraemu-independent-host.md)。旧通用 CLI 已删除；此前的 VFS mount、Luau private profile、Headless 和签名 provider 工作流不适用于当前 Host。

`astra-emu-musica-cli` 的源码作为 inactive 研究工具保留，包含 archive inventory、两阶段纯 Rust NRBF reader、GARbro scheme importer 和脚本/媒体 census。它不在主 workspace 中，不能直接沿用旧的 workspace 运行命令。保留源码是为了后续复用格式解析实现，不代表新 ABI 接入已经完成。
