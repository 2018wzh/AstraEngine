...
- [x] 严格读取原版签名、显式编码、单个有界 zlib 流和有序字段，拒绝截断、重复、非法编码及额外尾部数据。
- [x] 七个私有原版槽逐一匹配保存 PC 和消息 ID，未使用位置搜索或截断。
- [x] 为可表达的普通消息检查点建立显式状态投影；VM 候选准备失败时保留活动状态，不执行保存点之前的指令。
- [x] 对可表达的原版槽，从实际挂载 PAZ 恢复 897 条历史及当前消息 VM。
- [x] 接入 Family GPU 与解码/混音链，使用显式 NullAudio 完成一个原版存档导入并继续的片段。
- [ ] 完成该片段的可听音频验收。
- [x] 从 Musica 当前权威静态消息状态生成原版存档，在独立副本验证读回并继续。

增量回归覆盖10 项 eden 容器/位置/恢复测试、4 项脚本加载测试、58 项现有 VM 测试及 12 项 CLI 测试。可表达状态之外明确失败；未完成的实际流程保持待办，不以容器往返替代。原版文件只读，商业素材和测试槽只留在 ignored 私有目录。

## 历史实现记录（旧架构）

下表保留早期实现背景，不代表当前独立 Host 状态。
...
| 纯 Rust `MusicaPazDecryptProvider` | 已实现 | Blowfish、RC4 skip、archive XOR、zlib、movie transform；没有 Luau callback 或 fallback |
| Trusted Luau v2 private profile | 已实现 | data-only 一次注册、sandbox 与预算 tests；Musica Luau 不执行逐 entry 解密 |
...
| Musica deterministic VM state 与 control-flow slice | E2 slice | `set/setglobal/label/goto/if/wait/message`、BGM/SE stop、`playvoice *`、`transition`、无 stand `stage`、`chain/end`、budget、一次性文本 lease、连续 tick、尾链 VFS 切换和 postcard snapshot/restore；select、普通 voice、stand 与主要演出仍 blocking |
| Musica runtime provider / `cdylib` ABI | E2 slice | 签名动态 plugin 经通用 `--family musica --mount-profile` composition 启动；真实八包 Headless 到达第二条可见 message，共 373 tick、9 个 frame、6 个 checkpoint，音频 evidence、snapshot round-trip 与公共 review 流程通过 |
| Musica 演出、系统 UI、完整模拟 | 未完成 | 已验证黑场、竖排标题、CrossFade2、message panel 和首条日文正文；transition 动画、人物、UI、存储和路线仍开放 |
...
- [x] 从 `scr.paz` 与原程序候选确认入口文件 `test.sc`；多脚本时 CLI 要求完整稳定 URI `--entry musica:/scr/test.sc`，不接受裸文件名，也不隐式选择。
...
Family 已接入只读原版导入与静态消息设备恢复；VM v26 保存显式原版历史上下文，CLI 从核心槽重建受支持的原版 SAV。未知字段不透传，不可表达状态明确拒绝。AMUSSV05 在解码前拒绝旧槽并保留原文件。

本轮增量检查通过 14 项 eden codec/恢复/导出、11 项存储、2 项 CLI 输出保护，以及真实 GPU Family fixture 的导入、继续、保存、画布不匹配拒绝和带缩略图导出。两个 crate 的 all-target Clippy、格式和文档检查通过。

独立 Windows Sandbox 已完成一个真实静态消息片段的闭环：原版新建存档 → Musica Family 导入 → 正常输入推进三条正文 → 核心 F5 保存 → 导出 SAV 与同名 PNG → 原版载入并继续正文和转场。原版再次保存后，PC、消息 ID、900 条历史的全部字段、36 个原版变量及其资源字段相同；导出的 `eff_Method=end` 被原版省略，其含义仍是无活动效果。实际发现并修复了原版重新保存会把历史 `CT5` 归零、逻辑画布须为 1024×640、读取页需要同名 PNG 三处问题。

该结果仅覆盖本次《eden*》英文版加用户补丁的静态消息子集；其余六个输入槽含未支持历史类型，继续明确拒绝。未进行可听音频检查，NullAudio 不计作真实音频验收；不宣称其他游戏互通或全部演出兼容。原版输入未修改，输出与复存只位于独立测试目录。
