...
| [cosmic-text](https://docs.rs/crate/cosmic-text/latest) | workspace lockfile `0.18.2` | MIT OR Apache-2.0 | 日文 shaping、fallback、度量和换行；Musica 不实现私有字体排版器 |
| [Symphonia](https://docs.rs/symphonia/latest/symphonia/) | workspace lockfile `0.6.0` | MPL-2.0 | 音频 demux/decode；解码后的 owned PCM 进入统一 Kira `AudioServiceSession`，不保留 Musica 私有 mixer |
...
| 来源 | 访问日期 | 归属与可信度 | 允许用途 |
| --- | --- | --- | --- |
| [Kotaku gallery](https://kotaku.com/games/natsuzora-no-perseus/gallery) | 2026-07-21 | 页面标注图片来源为 Musica；辅助级 | 画面比例、消息框位置、人物占屏和色调检查 |
