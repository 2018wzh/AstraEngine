...
本页是旧架构移植记录。旧 CLI/Headless 命令已退出当前运行路径；Musica 源码保留为 inactive 研究材料，新 Host 接入尚未开始。
...
| 真实 Musica Headless 启动 | 本地样本 | 签名动态 plugin、八包 mount、373 fixed tick、9 实际呈现帧、6 checkpoint、snapshot round-trip、音频 artifact 和公共 review 流程通过；已到第二条可见 message |
