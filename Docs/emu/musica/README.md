...
Musica family 资料面向 PAZ + `.sc` 脚本游戏。核心和专用研究工具源码保留，但已退出活动 workspace，尚未接入独立 Family ABI。下文及关联页面中的旧 VFS、Headless 和动态 provider 结果属于历史研究；当前交付范围见 [独立 Host 重构](../../migrations/astraemu-independent-host.md)。
...
| [tooling.md](tooling.md) | 通用 VFS CLI、Musica 私有导入与研究工具 |
