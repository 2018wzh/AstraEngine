...
| [runtime-family-plugin.md](runtime-family-plugin.md) | AstraEMU Musica provider session 的模块拆分 |
...
Musica mount 只接受游戏目录内由 launch profile 指定的安全相对 `key_file`。`key.toml` 使用严格 schema `astra.emu.musica.keys.v1`，mount 时读取一次；VFS 随后从加密 source 流式解密，不建立明文 cache 或临时文件。旧 `astraemu.patch.luau`、private profile、GARbro importer 和 cache 字段都已硬删除。

当前 runtime 只接收日文原版内容变体 `natsuzora-no-perseus.original-ja`，并要求目录内存在原版入口 `perseus.exe`。`astra.emu.musica.locale.ja-jp.cp932.v1` 是严格的 CP932 转区/编码绑定：它解决原版字节在非日文宿主上的乱码，不提供翻译，也不加载汉化 exe、`.mys` 或其它本地化覆盖。汉化样本可以作为 ignored 研究资料，但不会进入挂载、脚本解析或消息呈现路径。
