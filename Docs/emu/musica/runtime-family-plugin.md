...
全屏切换通过 Family API v5 的类型化窗口命令交给 Manager 窗口线程处理；应用配置时提交一次，冷启动恢复已保存的全屏设置，取消不提交。无窗口的 Headless Host 明确拒绝窗口命令。字体选择与正文速度字段保留来源当前语义，不宣称新增字体或逐字渲染能力。真实游戏 Config、窗口切换与完整结局继续验收。
