...
显式 `launch_mode=title` 可进入来源的原生标题图片与菜单，支持鼠标/键盘焦点、开始游戏、读档、退出和剧情 `.end` 返回标题。标题变体取四条路线全局标记；开始游戏重新装载入口脚本、取消旧翻译、清理音频并保留当前配置、已读集合与全局变量。标题读档取消返回标题，成功读档保留 Title 启动方式。默认 `direct` 继续直接进入剧情；原生 Config 尚未接入，点击返回明确的 unavailable 诊断，标题模式还不能作为完整产品入口验收。VM v24 保存启动方式与鉴赏页状态，旧 v23 及更早明确拒绝。

Memories 沿用来源的 CG、Replay、BGM、Movie、Return 顺序，仅最终标题变体开放。CG 使用原生分页素材与缩略图；BGM 保留 47 首曲目、键盘分页及原生鼠标区域，复用会话混音器。回想与电影通过来源脚本表加载，结束返回标题，随后开始仍加载原始主剧情入口。页面使用共享 GPU Scene，停留期间不推进剧情；音乐鉴赏按来源规则处理停止和返回。Config 与真实游戏完整鉴赏流程仍待完成。

Config 模型与编辑事务已从来源迁入 MusicaConfigState、MusicaConfigControl 和 VM。open_config 记录标题或剧情等待作为返回位置，config_for_presentation 读取草稿，apply_config_control 应用、取消或返回试听/音量变更请求；非法编辑保留原草稿。当前自动阅读与角色语音使用同一配置，读档重建 VM 时整体保留当前设置。设置及草稿不进入剧情存档，Config 打开期间拒绝保存；成功读档清除草稿，失败则保留。原生 GPU 页面、控件输入、试听、全屏请求及配置文件持久化仍待接入，原生标题的 Config 入口继续明确报未接入。
