...
Musica family 通过 `LegacyRuntimeProvider` 接入 AstraEMU。Session 持有 PAZ archive set、`.sc` decoder、VM、presentation/audio mapper、external key diagnostics 和 snapshot state。文档只描述 planned target，不表示 runtime 已实现。
...
| 情况 | Diagnostic | 行为 |
| --- | --- | --- |
| 缺 private profile 或 key | `ASTRA_EMU_MUSICA_PRIVATE_PROFILE_*` | 阻止 mount |
