...
- 引擎侧改动由 `MusicaPatch.dll` + 补丁版可执行文件承担（推断为修改引擎的
...
| # | 差距 | 位置 | 现状 |
| --- | --- | --- | --- |
| 1 | 文本 NLS 只有 CP932 | `locale.rs` | `MusicaNls` 已预留 `gbk`/`utf8` 变体，挂载即硬错误；`MusicaLocaleHook` 只实现 `astra.emu.musica.locale.ja-jp.cp932.v1` |
| 2 | hook 调用点硬编码 | `script.rs:208/364`、`provider.rs:5458/11307` | `.sc` 解析、disassemble、资源目标解码直接调用 `MusicaLocaleHook::japanese_cp932()`，不从 profile 传递 |
...
`nls` 选项已经把"语言变体"建模为显式绑定；`MusicaPazDecryptor::new_with_locale`
...
- `MusicaLocaleHook` 从单实现改为按 NLS 变体构造：新增
  `astra.emu.musica.locale.zh-hans.gbk.v1` 与 `astra.emu.musica.locale.zh-hans.utf8.v1`
  hook id；`MusicaNls::is_currently_supported` 相应放开。
- 解码失败语义与 CP932 一致：`decode_without_replacement`，坏字节按
  `ASTRA_EMU_MUSICA_LOCALE_DECODE` 阻断，不做 replacement 猜测。
...
- `MusicaMountedVfs` 为每个角色建立两层 entry 视图：resolve 同 URI 时命中
...
- 实证：GARbro 仓库本体不含 key 值。Musica（引擎名 Musica）的 PAZ 密钥存于
...
- 本仓已有实现存量：硬化版两阶段 NRBF reader（634 行）与 CMVS importer
  ...
  FamilySupport，并按 `astra-emu-cmvs/src/scheme.rs` 的模式写 Musica 版
  importer。
- importer 形态：`astra-emu-musica-cli import-garbro-scheme --scheme-db
  <db> --title <title> --game-dir <root>`，把 `ArcKeys` 按档案名映射为
  八角色 scheme、`TypeKeys` 并入角色 scheme（现有 `MusicaPazDecryptor`
...
明确不做：不模拟 `MusicaPatch.dll` 的引擎内 hook；不提供运行时翻译服务或
