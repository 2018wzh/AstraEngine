...
`PackFile` 用于 `E:\Games\樱之诗春之雪` 的全部 12 个 `.arc`，也出现在 `E:\Games\サクラノ詩` 的少数 archive。
...
示例：`E:\Games\樱之诗春之雪\data01100.arc` 是 `PackFile`，entry count 为 5，data base 为 `0xB0`。entry `main` 的 relative offset 为 `0x0`，absolute offset 为 `0xB0`，raw size 为 726 bytes；entry `skp001` 的 relative offset 为 `0x2D6`，absolute offset 为 `0x386`，raw size 为 31,070 bytes。

## `BURIKO ARC20`

`BURIKO ARC20` 是现代 BGI/Ethornell 常见 archive。`E:\Games\素晴らしき日々15th` 的 53 个 `.arc` 均为此格式。
...
示例：`E:\Games\サクラノ詩\data01100.arc` 是 `BURIKO ARC20`，entry count 为 5，data base 为 `0x290`。entry `00_op_01` 的 absolute offset 为 `0x290`，raw size 为 34,372 bytes。`E:\Games\素晴らしき日々15th\data01701.arc` 的 tail fields 存在非零值，但 payload 仍以 `DSC FORMAT 1.00` 开头，reader 不应拒绝。
