...
BGI family 必须按 ADR 0012 接入：旧引擎状态机留在 family plugin 内，Manager 通过 RuntimeWorld、StateMachine action provider、trace、media mapper、`TextCaptureEvent` 和 snapshot section 观察它。BGI 规则不能反向污染 EngineCore 的 Actor/Component 边界。
...
| [runtime-core-design.md](runtime-core-design.md) | AstraEMU BGI family plugin 的内部模块和 provider 输出。 |
