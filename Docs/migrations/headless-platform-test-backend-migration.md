...
本页定义现有平台无关测试向统一 Headless Platform 后端迁移的实施路线。`S2-HEADLESS-CONTRACT-01` 的 `HostKind`、`HostLaunchProfile`、`HeadlessHostProfile`、schema 和 release rejection 已在 master 固化；`S2-HEADLESS-HOST-01` 已进入 `IN_PROGRESS`，其余媒体、输入、产物、CLI、测试收束、review 和 preflight 仍为 `SPEC_READY`。这些局部进展不能把 Stage 2 Headless、产品可玩性或真实平台验收标为完成。
...
Migration 11 的后续实现必须在新分支完成。`S2-HEADLESS-CONTRACT-01` 保持 `DONE`，`S2-HEADLESS-HOST-01` 保持 `IN_PROGRESS`，其余项保持 `SPEC_READY`。只有对应 Rust 类型、schema、crate、binary、workspace test migration、artifact、review 和 preflight report 都通过后，才能逐项改为 `DONE`。
