...
Host 默认打开系统音频设备。没有音频设备的测试环境可在设置中显式选择 `NullAudioDevice`：使用同一有界 PCM 队列，按采样时钟消费数据，并支持取消与关闭；不调用系统声卡，不输出声音。选择只在当前进程有效，游戏内显示测试标记。它用于游戏流程自动化，不能替代真实音频设备与听感测试；系统设备失败时仍须返回原始诊断。

保留现有 Slint 视觉与资料库操作，替换内部 host。资料库与设置采用新 schema，明确重建旧数据，不实现旧 schema 迁移。原生游戏存档不属于 Manager 数据。Family 动态库放在数据目录的 `cores/` 目录，Manager 冷启动按平台扩展和 `astra_emu_`/`libastra_emu_` 前缀自动发现，并校验 ABI 与必需 capability。多个 probe 命中时由用户选择，不能静默取第一项。

Library SQLite schema 4 分别保存资料库、外观、全局手柄映射、逐游戏映射和滤镜配置。外观设置支持跟随系统主题；手柄映射在启动游戏时确定，修改前先退出游戏。动态核心路径不进入 SQLite；旧 `plugin_installation` 表即使存在也不再读取或写入。旧 Host VFS、Luau patch 和不存在的翻译扩展配置页直接删除。

核心更新通过替换数据目录 `cores/` 中的文件并重启 Manager 完成；Manager 不提供运行期热加载或文件选择器安装路径。扫描中损坏、ABI 不匹配、descriptor/capability 错误和重复 plugin ID 都按文件显示诊断，其他唯一核心继续加载；重复 ID 的全部冲突文件均禁用。滤镜保存先编译并应用 GPU 配置，再写入数据库；写入失败恢复旧 GPU 配置。外部 HLSL 原文只存入本机设置数据库，不写入 package 或日志。
...
主任务负责设计、代码修复、审阅和集成；委派给 Luna 的任务使用独立 worktree。各 worktree 独享 target，遇到构建锁等待。构建并发按整机可用内存控制，多个工作树不同时运行重型构建。按普通软件开发方式运行测试，不建立新的 evidence/report 留存体系；临时游戏内容、截图、日志和编译探测产物完成后清理。代码通过相关检查后提交到重构分支，不自动合并或发布。未通过的检查与未完成的真实游戏流程如实记录为未完成。

Manager 的作品关联入口位于游戏库详情。Host 通过 `ManagerController::search_metadata(provider, query)` 调用现有 `MetadataProvider::search`，搜索结果经 `accept_match(candidate_id)` 显式选择后重新获取并保存；不提供手填 ID 的 UI 回调。网络许可、256 字符上限、单作品请求互斥和候选归属校验在 Host 执行。`refresh_metadata` 只更新已关联条目，旧设置页的手填 ID 与审核队列直接删除，不保留兼容入口。此接口只读外部元数据并修改本地关联，不发送游戏内容；发布条件仍沿用独立 Host 的许可与功能验收。
