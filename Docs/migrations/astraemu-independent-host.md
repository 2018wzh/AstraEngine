...
Library SQLite schema 3 分别保存资料库、外观、全局手柄映射、逐游戏映射、滤镜配置与已安装插件。外观设置支持跟随系统主题；手柄映射在启动游戏时确定，修改前先退出游戏。旧 Host VFS、Luau patch 和不存在的翻译扩展配置页直接删除。

插件安装由文件选择器显式选择本地动态库，启动时重新校验记录与 descriptor。滤镜保存先编译并应用 GPU 配置，再写入数据库；写入失败恢复旧 GPU 配置。外部 HLSL 原文只存入本机设置数据库，不写入 package 或日志。
