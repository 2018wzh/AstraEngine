...
Web target 的下一处真实编译阻断来自 host graph 把 Luau C++ VM 无条件拖入 `wasm32-unknown-unknown`。现已把 `astra-policy` 的纯 DTO/schema 与 mlua runtime feature 分开，并把 `VnPolicyState`、mutation/query/command/trace/snapshot contract 移出 Luau executor；`astra-vn-package` 与 `astra-vn-save` 在产品 package/save 路径只依赖 contract feature。`cargo tree -p astra-player-web -i mlua` 现在为空，Web 不再靠安装 C++ sysroot 掩盖 owner 错误。实际 wasm 编译已推进到下一处既有架构阻断：`abi_stable`/dynamic loader 仍被 in-process Web provider graph 无条件带入；在纯 in-process provider contract 与动态 ABI loader 分层前，Web E3 继续保持未完成。
