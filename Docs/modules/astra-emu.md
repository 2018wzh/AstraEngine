...
当前 contract 已 hard cut 到 family ABI v10：descriptor、instance、probe/open/step/VFS/surface/Hook/writable-file/shutdown 使用显式 `StableAbi` wire DTO。Family ABI 不再提供 snapshot/save/restore、text lease、session resource presentation 或 step budget；旧 binary、fingerprint 和 runtime snapshot 直接拒绝，不保留兼容入口。
...
`astra-emu-cli run` 是独立于 Manager 的 overlay-free 原生验收入口。它通过显式 `--family`、`--launch-profile` 和授权游戏目录创建 `AstraEmuRuntimeProvider` session，runtime entry 与启动模式只来自 launch profile，直接把 family frame 交给 Windows platform host，物理输入按 legacy 舞台宽高比路由。默认静音，显式 `--enable-audio` 才启用平台音频；无物理音频设备时可使用有界 Null output，但不计为物理音频证据。原生受控复跑可传入 `astra.user_input_sequence.v1` JSONL；存在 replay 时拒绝外部 gameplay input。`astra-emu-cli headless` 使用同一 provider lifecycle 和 launch profile，只消费序列化物理输入与固定时间控制，并输出脱敏 run report、PNG/WAV 与 artifact manifest。Headless E2 与 Release Sandbox 行为/视觉验收都不替代正式 Windows E3。
