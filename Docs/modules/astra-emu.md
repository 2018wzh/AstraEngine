...
当前 family contract 已硬切为 `astra.emu.family_abi.v8`；v5/v6/v7 binary、
...
当前 contract 已 hard cut 到 family ABI v8：descriptor、instance、probe/open/step/save/restore/resource read/VFS callback/shutdown 使用显式 `StableAbi` wire DTO，bulk payload 由 ABI-owned buffer 持有，retained ephemeral text 使用显式对齐和清除语义，v5/v6/v7 binary/manifest 直接拒绝。FVP runtime snapshot 使用独立的 v7 schema。下方 v4 range port 与 snapshot v5/v6 内容只保留为迁移历史，不能作为当前插件身份。
...
`astra-emu-cli run` 是独立于 Manager 的 overlay-free 原生验收入口。它通过显式 `--family`、`--mount-profile`、授权游戏目录和可选 entry 创建 `AstraEmuRuntimeProvider` session，直接把 family frame 交给 Windows platform host，物理输入按 legacy 舞台宽高比路由。默认静音，只用于排除 Slint/overlay 后比较核心视觉行为；显式 `--enable-audio` 才启用平台音频。原生受控复跑可显式传入同一份 `astra.user_input_sequence.v1` JSONL，并用 `--max-fixed-steps` 划定 soak 边界；它只走 RuntimeDriver 的物理输入映射，拒绝 `advance_ticks` 和 `await` 控制消息，checkpoint 只作为日志标记而不伪装成 Headless capture。`astra-emu-cli headless` 使用同一个 provider lifecycle 和 `astra-platform-headless` 执行 `astra.user_input_sequence.v1`。Headless 不接受 `advance`、`choose` 等产品语义捷径，只消费序列化物理输入与固定时间控制，并输出脱敏 `astra.emu.headless_run_report.v3`、PNG/WAV 与 artifact manifest。FVP Headless 的主路径把 semantic `ScenePacket` 交给 `wgpu_offscreen`；CPU rasterizer 仅保留为像素正确性 oracle，性能模式不得选用。性能证据必须同时提供 shared budget、report、Perfetto trace 和 trace manifest；它固定 1,200 warmup 加 72,000 measured semantic presentation，拒绝 Debug/dirty identity、CPU fallback、缺 timestamp query、非 120 Hz 和采样跳帧，并将两个 evidence hash 绑定进 v3 run report。旧 `--engine` 已删除，不提供 alias 或 runtime fallback。`--artifact-retention checkpoints` 是默认策略，只落盘具名 checkpoint PNG；所有提交帧仍进入 frame-stream hash 和视觉 trace。需要保存逐帧 PNG 做本地精确对照时必须显式选择 `all`。save/restore 后的首 tick 必须显式使用 `RestoreContinuation`，host-owned audio/movie state 先重置再由 family effect 重建，后续 tick 才回到 `Live`。
