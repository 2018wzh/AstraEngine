...
- Incremental playback：`IncrementalMediaDecoder` 和 `IncrementalMediaPlayback` 位于 `astra-media`，统一有界 read-ahead、PTS 选择、BGRA 校验、PCM canonical conversion、seek/cancel 和 telemetry。游标打开时验证完整 `MediaPlaybackConfig`，推进时限制 tick 间隔、视频/音频 packet 数、lead/lag 和迟到帧策略，并将显式丢帧计数交给 host。`FfmpegPlaybackDecoder::open_reader` 通过 custom AVIO 持有有界 `Read + Seek + Send` reader，FFmpeg 以 64 KiB callback buffer 直接 demux，不生成完整明文 spool；AVI 也走该路径。family adapter 只负责容器身份和显式 provider binding，FFmpeg feature 缺失时阻断，不保留手写 codec 或平台 fallback。
