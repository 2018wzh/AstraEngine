...
- Audio：`astra.audio_graph.v2` 提供事务性 voice/fade 生命周期和 fixed-delta continuation；persistent mixer 已收束到 Media，并为 Headless 每个 canonical tick 生成 800 stereo PCM frame 和 S16LE WAV。真实 Windows output 仍使用 bounded queue、WASAPI callback meter、错误传播和 close drain。
