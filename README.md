...
- `Engine/Programs/astra`：runtime evidence CLI，支持 `--version`、`doc-check`、`validate`、`import`、`cook`、`package`、`run --headless-smoke/--windowed-smoke`、`replay --compare` 和 `inspect`。当前输出 `foundation_core_gate`、module release gate binary hash、registered diagnostic-code gate、engine DLL SHA-256 evidence、Phase 6 Asset Pipeline evidence、media backend capability report、libpng image decode/RGBA texture smoke、image/font/audio cook metadata、NativeVN/TsuiNoSora Phase 4 Script/AstraVN evidence、`playable_vn` UI/system/save/load evidence、SDL/headless `window_present` frame evidence、binary `astra.package.manifest.v1` `.astrapkg` package manifest、zstd package payload table、package mount/payload read smoke、local DDC artifacts、DDC reuse/rebuild/corruption recovery evidence、package/cook/payload hash integrity diagnostics、package manifest hash/provider feature hash save-replay evidence 和 replay mismatch report。真实 renderer upload、font atlas execution、audio mixer playback、GPU filter execution 和 full Editor/AI release gate 仍是后续 production completion。
...
这些命令证明当前 binary `.astrapkg` package/replay/playable workflow，包括窗口创建、scripted input、UI/system 状态、media decode evidence、save/load、package manifest hash/provider feature hash 和 replay route hash。最终 UE-class acceptance 仍要求真实 media execution backend、Editor/creator workflow、AI/MCP 策略和长期压力/迁移测试等目标全部达成。
...
当前已有动态链接的 `Astra*` runtime/tool DLL targets、Phase 1 foundation runtime targets、Phase 2 Scene/Runtime foundation targets、Phase 3 Asset/Media/FilterGraph foundation targets、Phase 4 Script/AstraVN foundation targets、Phase 6 Asset Pipeline targets、media provider release-gate foundation evidence、mature media backend capability evidence、libpng image metadata decode evidence、image/font/audio cook metadata evidence、Native/Lua parity evidence、NativeVN 与 TsuiNoSora playable source asset sidecars、AssetRegistry/dependency graph、local DDC artifact execution/corruption recovery、binary `.astrapkg` zstd payload table、PackageReader random-access/chunked-read/mount evidence、Asset Release Gate evidence、cook/package integrity/replay/playable evidence 和 Catch2/CTest 测试。命令成功能证明当前 Foundation Core / Platform / Module / Property / Scene / Runtime / Media / Script / AstraVN、Phase 6 Asset Pipeline 与 playable evidence slice 的测试面通过，不能作为真实 media execution backend、完整 Script debugger/hot reload/Graph/Timeline、Editor 或完整 AI/creator release pipeline 已实现的前提；应以实际工作树和 CI 输出为准。当前继续复用成熟库和工具链：`yaml-cpp`、`nlohmann_json`、OpenSSL SHA-256、zstd、SDL3、libpng、libjpeg-turbo、libwebp、FreeType、HarfBuzz、miniaudio、Lua、`sol2`、Catch2 和 CTest，不为解析、JSON、哈希、压缩、图像 metadata decode、字体/文本 shaping/audio 基础能力探测、Lua 绑定或测试自造轮子。
