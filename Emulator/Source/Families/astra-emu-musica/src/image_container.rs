...
pub const MUSICA_IMAGE_DECODE_PROVIDER_ID: &str = "astra.decode.musica.image";

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MusicaImageFrameDescriptor {
    pub name: String,
    pub width: u32,
    pub height: u32,
    pub offset_x: i16,
...
#[derive(Debug, Clone)]
pub struct MusicaAniArchive {
    source: Arc<[u8]>,
    frames: Vec<MusicaImageFrameDescriptor>,
}

impl MusicaAniArchive {
    pub fn parse(source: impl Into<Arc<[u8]>>) -> Result<Self, LegacyCoreError> {
        ...
        for _ in 0..count {
            ...
            let name = MusicaLocaleHook::japanese_cp932()
                .decode(name_bytes)
                .map_err(|_| {
                    invalid(
                        "ASTRA_EMU_MUSICA_ANI_NAME",
            ...
            cursor = cursor
                ...
                .ok_or_else(|| invalid("ASTRA_EMU_MUSICA_ANI_BOUNDS", "ANI cursor overflowed"))?;
            ...
            let data_offset = cursor.checked_add(10).ok_or_else(|| {
                invalid("ASTRA_EMU_MUSICA_ANI_BOUNDS", "ANI data offset overflowed")
            })?;
            checked_slice(&source, data_offset, pixel_bytes)?;
            frames.push(MusicaImageFrameDescriptor {
                name,
                width,
                height,
                offset_x,
        ...
        if cursor != source.len() {
            return Err(invalid(
                "ASTRA_EMU_MUSICA_ANI_TRAILING_DATA",
    ...
    pub fn frames(&self) -> &[MusicaImageFrameDescriptor] {
        &self.frames
    }
    ...
    pub fn decode_frame(&self, index: usize) -> Result<RgbaImage, LegacyCoreError> {
        let frame = self.frames.get(index).ok_or_else(|| {
            invalid(
                "ASTRA_EMU_MUSICA_ANI_FRAME_INDEX",
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MusicaSqzFrameDescriptor {
    pub index: u32,
    pub data_offset: u64,
    pub stored_size: u64,
}
...
#[derive(Debug, Clone)]
pub struct MusicaSqzArchive {
    source: Arc<[u8]>,
    width: u32,
    height: u32,
    frames: Vec<MusicaSqzFrameDescriptor>,
}

impl MusicaSqzArchive {
    pub fn parse(source: impl Into<Arc<[u8]>>) -> Result<Self, LegacyCoreError> {
        let source = source.into();
        validate_container_size(source.len())?;
        if checked_slice(&source, 0, 4)? != b"SQZ1" {
            return Err(invalid(
                "ASTRA_EMU_MUSICA_SQZ_HEADER",
        ...
        let count = pair_count
            ...
            .ok_or_else(|| {
                invalid(
                    "ASTRA_EMU_MUSICA_SQZ_FRAME_COUNT",
                    "SQZ frame count is invalid",
                )
            })?;
        let index_end = 0x14usize
            .checked_add(count.checked_mul(8).ok_or_else(|| {
                invalid("ASTRA_EMU_MUSICA_SQZ_INDEX", "SQZ index size overflowed")
            })?)
            .ok_or_else(|| invalid("ASTRA_EMU_MUSICA_SQZ_INDEX", "SQZ index end overflowed"))?;
        ...
        for index in 0..count {
            ...
            frames.push(MusicaSqzFrameDescriptor {
                index: index as u32,
                data_offset: data_offset as u64,
    ...
    pub fn frames(&self) -> &[MusicaSqzFrameDescriptor] {
        &self.frames
    }
    ...
    pub fn decode_frame(&self, index: usize) -> Result<RgbaImage, LegacyCoreError> {
        let frame = self.frames.get(index).ok_or_else(|| {
            invalid(
                "ASTRA_EMU_MUSICA_SQZ_FRAME_INDEX",
        ...
        let limit = expected
            .checked_add(1)
            .ok_or_else(|| invalid("ASTRA_EMU_MUSICA_SQZ_OUTPUT", "SQZ output bound overflowed"))?;
        ...
        ZlibDecoder::new(encoded)
            ...
            .map_err(|_| {
                invalid(
                    "ASTRA_EMU_MUSICA_SQZ_ZLIB",
                    "SQZ frame zlib stream is invalid",
                )
            })?;
        if bgra.len() != expected {
            return Err(invalid(
                "ASTRA_EMU_MUSICA_SQZ_OUTPUT",
...
/// Explicit decode binding for Musica's family-owned ANI/SQZ image containers.
///
...
pub struct MusicaImageDecodeProvider;

impl MusicaImageDecodeProvider {
    pub fn capability(&self) -> DecodeCapability {
        DecodeCapability {
            provider_id: MUSICA_IMAGE_DECODE_PROVIDER_ID.to_owned(),
...
impl DecodeProvider for MusicaImageDecodeProvider {
    fn capability(&self) -> DecodeCapability {
        MusicaImageDecodeProvider::capability(self)
    }
    ...
    fn decode(&self, request: &DecodeRequest) -> Result<DecodeResult, MediaError> {
        if request.kind != DecodeKind::Image {
            return Err(MediaError::Diagnostics(vec![Diagnostic::blocking(
                "ASTRA_EMU_MUSICA_IMAGE_DECODE_KIND",
                "Musica image provider received a non-image request",
            )]));
        }
        let source = Arc::<[u8]>::from(request.bytes.as_ref());
        let (image, width, height) = match request.codec.as_str() {
            "ani" => {
                let archive = MusicaAniArchive::parse(source).map_err(image_decode_error)?;
            ...
            "sqz" => {
                let archive = MusicaSqzArchive::parse(source).map_err(image_decode_error)?;
            ...
            _ => {
                return Err(MediaError::Diagnostics(vec![Diagnostic::blocking(
                    "ASTRA_EMU_MUSICA_IMAGE_DECODE_CODEC",
                    "Musica image provider received an unsupported container codec",
                )]));
            }
        };
        ...
        if bytes.len() > MAX_PREVIEW_BYTES {
            return Err(MediaError::Diagnostics(vec![Diagnostic::blocking(
                "ASTRA_EMU_MUSICA_IMAGE_PREVIEW_LIMIT",
                "decoded Musica image exceeds the bounded preview output",
            )]));
        }
        Ok(DecodeResult {
            provider_id: MUSICA_IMAGE_DECODE_PROVIDER_ID.to_owned(),
...
fn checked_pixel_bytes(
    ...
    {
        return Err(invalid(
            "ASTRA_EMU_MUSICA_IMAGE_DESCRIPTOR",
    ...
    usize::try_from(pixels * u64::from(bits_per_pixel / 8))
        .map_err(|_| invalid("ASTRA_EMU_MUSICA_IMAGE_SIZE", "image byte size overflowed"))
}

fn validate_container_size(len: usize) -> Result<(), LegacyCoreError> {
    if len == 0 || len > MAX_CONTAINER_BYTES {
        return Err(invalid(
            "ASTRA_EMU_MUSICA_IMAGE_CONTAINER_SIZE",
...
fn checked_slice(source: &[u8], offset: usize, len: usize) -> Result<&[u8], LegacyCoreError> {
    let end = offset
        .checked_add(len)
        .ok_or_else(|| invalid("ASTRA_EMU_MUSICA_IMAGE_BOUNDS", "image range overflowed"))?;
    source
        .get(offset..end)
        .ok_or_else(|| invalid("ASTRA_EMU_MUSICA_IMAGE_BOUNDS", "image range is truncated"))
}
...
mod tests {
    ...
    fn image_decode_provider_exposes_a_bounded_first_frame() {
        ...
        let provider = MusicaImageDecodeProvider;
        let result = provider
            ...
            .unwrap();
        assert_eq!(result.provider_id, MUSICA_IMAGE_DECODE_PROVIDER_ID);
