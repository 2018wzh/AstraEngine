//! Musica AVI integration for the shared AstraMedia incremental decoder.
//!
//! Musica does not own a container parser or codec implementation. The family
...
pub const MUSICA_AVI_DECODE_PROVIDER_ID: &str = "astra.decode.musica.avi";
...
/// Explicit first-frame binding for Musica AVI previews.
///
...
pub struct MusicaAviDecodeProvider;

impl MusicaAviDecodeProvider {
    pub fn capability(&self) -> DecodeCapability {
        DecodeCapability {
            provider_id: MUSICA_AVI_DECODE_PROVIDER_ID.to_owned(),
...
impl DecodeProvider for MusicaAviDecodeProvider {
    fn capability(&self) -> DecodeCapability {
        MusicaAviDecodeProvider::capability(self)
    }

    fn decode(&self, request: &DecodeRequest) -> Result<DecodeResult, MediaError> {
        if request.kind != DecodeKind::Video {
            return Err(avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_KIND"));
        }
        if !request.codec.eq_ignore_ascii_case("avi") {
            return Err(avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_CODEC"));
        }
        validate_preview_input_len(request.bytes.len())?;
        if !is_avi_container_header(&request.bytes) {
            return Err(avi_preview_diagnostic(
                "ASTRA_EMU_MUSICA_AVI_PREVIEW_HEADER",
            ));
        }
        if !astra_media::ffmpeg_compiled() {
            return Err(avi_preview_diagnostic(
                "ASTRA_EMU_MUSICA_AVI_FFMPEG_UNAVAILABLE",
            ));
        }
        #[cfg(feature = "ffmpeg-vcpkg")]
        {
            let provider = FfmpegDecodeProvider::probe()
                .map_err(|error| avi_media_error("ASTRA_EMU_MUSICA_AVI_FFMPEG_PROBE", error))?;
            let result = provider
                .decode(request)
                .map_err(|error| avi_media_error("ASTRA_EMU_MUSICA_AVI_PREVIEW_DECODE", error))?;
            let DecodeOutput::CpuBuffer { bytes, format } = result.output else {
                return Err(avi_preview_diagnostic(
                    "ASTRA_EMU_MUSICA_AVI_PREVIEW_OUTPUT",
                ));
            };
            ...
            let expected_bytes = usize::try_from(width)
                ...
                .ok_or_else(|| avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_FRAME"))?;
            if bytes.len() != expected_bytes || bytes.len() > MAX_PREVIEW_FRAME_BYTES {
                return Err(avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_FRAME"));
            }
...
fn avi_preview_diagnostic(code: &'static str) -> MediaError {
    MediaError::Diagnostics(vec![astra_core::Diagnostic::blocking(
        code,
        "Musica AVI requires the explicitly bound AstraMedia FFmpeg provider",
    )])
}
...
fn validate_preview_input_len(len: usize) -> Result<(), MediaError> {
    if len == 0 || len > MAX_PREVIEW_INPUT_BYTES {
        return Err(avi_preview_diagnostic(
            "ASTRA_EMU_MUSICA_AVI_PREVIEW_INPUT_LIMIT",
        ));
    }
    Ok(())
}
...
fn parse_frame_format(format: &str) -> Result<(u32, u32), MediaError> {
    let dimensions = format
        ...
        .ok_or_else(|| avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_OUTPUT"))?;
    let (width, height) = dimensions
        .split_once('x')
        .ok_or_else(|| avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_OUTPUT"))?;
    let width = width
        .parse::<u32>()
        .map_err(|_| avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_OUTPUT"))?;
    let height = height
        .parse::<u32>()
        .map_err(|_| avi_preview_diagnostic("ASTRA_EMU_MUSICA_AVI_PREVIEW_OUTPUT"))?;
...
fn validate_video_dimensions(width: u32, height: u32) -> Result<(), &'static str> {
    if width == 0 || height == 0 || width > MAX_VIDEO_DIMENSION || height > MAX_VIDEO_DIMENSION {
        return Err("ASTRA_EMU_MUSICA_AVI_VIDEO_DIMENSIONS");
    }
    let pixels = u64::from(width)
        .checked_mul(u64::from(height))
        .and_then(|pixels| pixels.checked_mul(4))
        .ok_or("ASTRA_EMU_MUSICA_AVI_FRAME_BOUNDS")?;
    if pixels > MAX_PREVIEW_FRAME_BYTES as u64 {
        return Err("ASTRA_EMU_MUSICA_AVI_FRAME_BOUNDS");
    }
    Ok(())
}
...
fn avi_media_error(prefix: &'static str, error: MediaError) -> MediaError {
    ...
    tracing::error!(
        event = "astra_emu_musica_avi_provider_error",
        diagnostic_prefix = prefix,
        diagnostic_codes = %codes,
        "AstraMedia FFmpeg provider rejected the Musica AVI preview"
    );
...
mod tests {
    ...
    use super::{
        ...
        MusicaAviDecodeProvider, MAX_PREVIEW_INPUT_BYTES, MUSICA_AVI_DECODE_PROVIDER_ID,
    };

    #[test]
    fn container_identity_is_checked_before_provider_open() {
        let provider = MusicaAviDecodeProvider;
        ...
        assert_eq!(diagnostics[0].code, "ASTRA_EMU_MUSICA_AVI_PREVIEW_HEADER");
    ...
    fn preview_provider_is_explicit_and_bounded() {
        ...
        assert_eq!(capability.provider_id, MUSICA_AVI_DECODE_PROVIDER_ID);
    ...
    fn video_dimension_budget_rejects_invalid_frames() {
        ...
        assert_eq!(
            ...
            "ASTRA_EMU_MUSICA_AVI_VIDEO_DIMENSIONS"
        );
        assert_eq!(
            validate_video_dimensions(16_384, 16_384).unwrap_err(),
            "ASTRA_EMU_MUSICA_AVI_FRAME_BOUNDS"
        );
    }
}
