...
#[serde(deny_unknown_fields)]
struct MusicaKeyFile {
    schema: String,
    #[serde(default)]
    type_passwords: MusicaTypePasswords,
    archive_keys: BTreeMap<String, MusicaArchiveKeyFile>,
}

#[derive(Debug, Clone, Default, Deserialize)]
#[serde(deny_unknown_fields)]
struct MusicaTypePasswords {
    png: Option<String>,
    ogg: Option<String>,
    sc: Option<String>,
    avi: Option<String>,
...
#[derive(Debug, Clone, Deserialize)]
#[serde(deny_unknown_fields)]
struct MusicaArchiveKeyFile {
    index_key_hex: String,
    data_key_hex: String,
}

#[derive(Debug, Default)]
pub struct MusicaVfsFamilyFactory;

impl LegacyVfsFamilyFactory for MusicaVfsFamilyFactory {
    fn family_id(&self) -> &str {
        "musica"
    }

    fn family_options_schema_id(&self) -> &str {
        MUSICA_FAMILY_OPTIONS_SCHEMA
    }

    fn family_options_schema_hash(&self) -> Hash256 {
        Hash256::from_sha256(MUSICA_FAMILY_OPTIONS_SCHEMA.as_bytes())
    }
    ...
    fn mount(
        ...
        if context.family_config.schema_id != MUSICA_FAMILY_OPTIONS_SCHEMA
            || context.family_config.schema_hash != self.family_options_schema_hash()
            || context.prefix != "musica:/"
        {
            return Err(invalid(
                "ASTRA_EMU_MUSICA_MOUNT_PROFILE",
                "Musica mount identity or options schema is invalid",
            ));
        }
        let options: MusicaFamilyOptions = serde_json::from_slice(&context.family_config.payload)
            .map_err(|_| {
            invalid(
                "ASTRA_EMU_MUSICA_MOUNT_OPTIONS",
                "Musica family options are invalid",
            )
        })?;
        let nls = MusicaNls::parse(&options.nls).map_err(|_| {
            invalid(
        ...
        if !nls.is_currently_supported() {
            return Err(invalid(
                "ASTRA_EMU_MUSICA_NLS_UNSUPPORTED",
                "the selected Musica text encoding is reserved but not implemented",
            ));
        }
        ...
        let key_text = std::str::from_utf8(&key_bytes).map_err(|_| {
            invalid(
                "ASTRA_EMU_MUSICA_KEY_ENCODING",
                "Musica key file must be UTF-8 TOML",
            )
        })?;
        let key_file: MusicaKeyFile = toml::from_str(key_text).map_err(|_| {
            invalid(
                "ASTRA_EMU_MUSICA_KEY_PARSE",
                "Musica key file is not valid strict TOML",
            )
        })?;
        let schemes = key_file.into_schemes()?;
        let decryptor = Arc::new(MusicaPazDecryptor::new_with_locale(schemes, locale_hook)?);
        let configs = options
            .archive_roles
            .iter()
...
impl MusicaKeyFile {
    fn into_schemes(self) -> Result<BTreeMap<String, PazRoleScheme>, LegacyCoreError> {
        if self.schema != MUSICA_KEY_FILE_SCHEMA {
            return Err(invalid(
                "ASTRA_EMU_MUSICA_KEY_SCHEMA",
                "Musica key file schema is invalid",
            ));
        }
        let roles = self
            .archive_keys
        ...
        if roles != REQUIRED_ARCHIVE_ROLES.into_iter().collect() {
            return Err(invalid(
                ...
                "Musica key file does not contain exactly the required archive roles",
            ));
        }
        let type_passwords = [
        ...
        for password in type_passwords.values() {
            if MusicaLocaleHook::japanese_cp932().encode(password).is_err() {
                return Err(invalid(
                    "ASTRA_EMU_MUSICA_KEY_CP932",
                    "Musica type password cannot be encoded as CP932",
                ));
            }
        }
        self.archive_keys
...
fn decode_hex_key(value: &str, required: bool) -> Result<Vec<u8>, LegacyCoreError> {
    if !value.len().is_multiple_of(2) || value.bytes().any(|byte| !byte.is_ascii_hexdigit()) {
        return Err(invalid(
            "ASTRA_EMU_MUSICA_KEY_HEX",
            "Musica key file contains invalid even-length hex",
        ));
    }
    ...
    let bytes = hex::decode(value).map_err(|_| {
        invalid(
            "ASTRA_EMU_MUSICA_KEY_HEX",
            "Musica key file contains invalid hex",
        )
    })?;
    if !(4..=56).contains(&bytes.len()) {
        return Err(invalid(
            "ASTRA_EMU_MUSICA_KEY_LENGTH",
            "Musica Blowfish key length must be between 4 and 56 bytes",
        ));
    }
    Ok(bytes)
}

fn validate_options(options: &MusicaFamilyOptions) -> Result<(), LegacyCoreError> {
    let roles = options
        .archive_roles
        .iter()
        .map(String::as_str)
        ...
        || roles != REQUIRED_ARCHIVE_ROLES.into_iter().collect()
    {
        return Err(invalid(
            "ASTRA_EMU_MUSICA_MOUNT_OPTIONS",
            "Musica family options violate their contract",
        ));
    }
    Ok(())
}
...
fn validate_original_entrypoint(game_root: &std::path::Path) -> Result<(), LegacyCoreError> {
    ...
    if !metadata.file_type().is_file() || metadata.file_type().is_symlink() {
        return Err(invalid(
            "ASTRA_EMU_MUSICA_ORIGINAL_ENTRYPOINT",
            "the Musica entrypoint must be a regular non-symlink file",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn key_file() -> MusicaKeyFile {
        MusicaKeyFile {
            schema: MUSICA_KEY_FILE_SCHEMA.into(),
            type_passwords: MusicaTypePasswords {
                png: Some("画像".into()),
                ogg: None,
                sc: Some("script".into()),
    ...
    fn strict_toml_rejects_unknown_fields_and_duplicate_roles() {
        let unknown = "schema = 'astra.emu.musica.keys.v1'\nunknown = true\n[archive_keys]\n";
        assert!(toml::from_str::<MusicaKeyFile>(unknown).is_err());

        let duplicate = "schema = 'astra.emu.musica.keys.v1'\n[archive_keys.scr]\nindex_key_hex='00112233'\ndata_key_hex='44556677'\n[archive_keys.scr]\nindex_key_hex='00112233'\ndata_key_hex='44556677'\n";
        assert!(toml::from_str::<MusicaKeyFile>(duplicate).is_err());
    }
    ...
    fn family_options_reject_localized_variant_and_non_japanese_hook() {
        ...
        options.content_variant = MUSICA_ORIGINAL_VARIANT_ID.into();
        options.locale_hook = "astra.emu.musica.locale.gbk.v1".into();
        assert_eq!(
            validate_options(&options).unwrap_err().code(),
            "ASTRA_EMU_MUSICA_MOUNT_OPTIONS"
        );
    ...
    fn mount_blocks_reserved_nls_before_reading_private_game_files() {
        ...
        let context = LegacyVfsMountContext {
            ...
            family_config: LegacyOpaqueFamilyConfig {
                schema_id: MUSICA_FAMILY_OPTIONS_SCHEMA.into(),
                schema_hash: Hash256::from_sha256(MUSICA_FAMILY_OPTIONS_SCHEMA.as_bytes()),
