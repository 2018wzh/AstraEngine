...
use crate::{MusicaLocaleHook, MUSICA_SCRIPT_IR_SCHEMA};
...
pub enum ScControlFlowKind {
    ...
    /// A Musica `.select` command stores each option as `display:label`.
    ...
    pub fn observed_musica() -> Self {
        ...
        for opcode in [
            ...
            catalog
                ...
                .expect("static Musica opcode catalog is unique");
        }
...
pub enum ScParseError {
    #[error("ASTRA_EMU_MUSICA_SC_ENCODING at offset {0}")]
    Encoding(usize),
    #[error("ASTRA_EMU_MUSICA_SC_DUPLICATE_OPCODE: {0}")]
    DuplicateOpcode(String),
    #[error("ASTRA_EMU_MUSICA_SC_DUPLICATE_LABEL: {0}")]
    DuplicateLabel(String),
    #[error("ASTRA_EMU_MUSICA_SC_INVALID_TARGET: {0}")]
    InvalidTarget(String),
    #[error("ASTRA_EMU_MUSICA_SC_OPERAND_SCHEMA at offset {0}")]
    OperandSchema(usize),
    #[error("ASTRA_EMU_MUSICA_SC_SOURCE_INVARIANT at offset {0}")]
...
pub fn disassemble_sc(script: &ScScript) -> Result<String, ScParseError> {
    disassemble_sc_with_locale(script, MusicaLocaleHook::japanese_cp932())
}
...
mod tests {
    ...
    fn select_rejects_missing_label_separator() {
        ...
        assert_eq!(
            parse_sc(source, &ScOpcodeCatalog::observed_musica()).unwrap_err(),
    ...
    fn cp932_trail_backslash_survives_message_control_decoding() {
        let locale = MusicaLocaleHook::japanese_cp932();
        ...
        let markup = crate::parse_musica_message_markup(tokens.get(3).unwrap()).unwrap();
