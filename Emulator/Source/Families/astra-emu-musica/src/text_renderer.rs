...
pub(crate) struct MusicaTextRenderer {
...
impl MusicaTextRenderer {
    ...
    pub(crate) fn render(
        ...
        let renderer = match &mut self.renderer {
            ...
            Some(_) => return Err("ASTRA_EMU_MUSICA_TEXT_STAGE_CHANGED".into()),
            slot @ None => {
                let renderer = CpuRendererProvider
                    .create(RendererCreateRequest {
                        ...
                        profile: "astra.emu.musica.text.v1".into(),
                    })
                    .map_err(|_| "ASTRA_EMU_MUSICA_TEXT_RENDERER_CREATE".to_owned())?;
        ...
        renderer
            ...
            .map_err(|_| "ASTRA_EMU_MUSICA_TEXT_RENDER".to_owned())
    }
}
...
fn append_layout(
    ...
    let layout = provider
        ...
        .map_err(|_| "ASTRA_EMU_MUSICA_TEXT_LAYOUT".to_owned())?;
    if layout.diagnostics.iter().any(|diagnostic| {
        ...
        return Err("ASTRA_EMU_MUSICA_TEXT_LAYOUT_DIAGNOSTIC".into());
    }
    ...
    commands.extend(
        resources
            ...
            .map_err(|_| "ASTRA_EMU_MUSICA_TEXT_RESOURCE".to_owned())?,
    );
