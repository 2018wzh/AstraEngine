...
//!
//! Run explicitly with `--ignored` and `ASTRA_SIGLUS_TEST_GAME` pointing at a game directory (with
...
fn headless_boot_inputs_frames_audio_and_close() {
    let game = test_game().expect("ASTRA_SIGLUS_TEST_GAME must identify the authorized game");
