...
//! `ASTRA_SIGLUS_TEST_ROUTE=1`. This is a several-minute run: the Rewrite+
...
fn headless_route_reaches_dialogue_flow() {
    if std::env::var("ASTRA_SIGLUS_TEST_ROUTE").as_deref() != Ok("1") {
        eprintln!("ASTRA_SIGLUS_TEST_ROUTE not set; skipping");
        return;
    }
