...
to the `astra-hosted` branch of the
...
All local changes are inert without the `astra-hosted` Cargo feature on
`siglus_scene_vm`; the upstream binaries and platform shells build and
...
- Every vendored crate gains a `[lints]` section allowing rustc/clippy
  warnings; vendored upstream source is not held to this repository's lint
  gate.
