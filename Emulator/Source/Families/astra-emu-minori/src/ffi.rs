...
use crate::{provider::MinoriProvider, session::MinoriSession};

#[derive(Default)]
struct MinoriModule {
    provider: Mutex<MinoriProvider>,
    sessions: Mutex<BTreeMap<String, MinoriSession>>,
}
    ...
    fn advance(
        ...
        boundary(|| {
            ...
            let session = sessions
                ...
                .ok_or_else(|| {
                    FamilyError::invalid(
                        "ASTRA_EMU_MINORI_SESSION",
                        "the requested MINORI session does not exist",
                    )
                })?;
...
impl FrameVisitor for ConsumerVisitor<'_, '_> {
    fn accept(&mut self, frame: astra_emu_family_api::FrameView<'_>) -> FamilyResult<()> {
        self.consumer.accept(frame).into_result().map_err(|_| {
            FamilyError::invalid(
                "ASTRA_EMU_MINORI_FRAME_CONSUMER",
...
fn lock_error<T>(_error: std::sync::PoisonError<T>) -> FamilyError {
    FamilyError::invalid(
        "ASTRA_EMU_MINORI_LOCK",
        "the MINORI module state lock is poisoned",
    )
}
...
fn boundary<T>(action: impl FnOnce() -> FamilyResult<T>) -> RResult<T, FamilyError> {
    match catch_unwind(AssertUnwindSafe(action)) {
        ...
        Err(_) => Err(FamilyError::invalid(
            "ASTRA_EMU_MINORI_PANIC",
            "MINORI panicked at the dynamic family boundary",
        ))
        .into(),
    }
}
...
extern "C" fn construct_module() -> FamilyModuleBox {
    astra_emu_family_api::FamilyModule_TO::from_value(MinoriModule::default(), TD_Opaque)
}
