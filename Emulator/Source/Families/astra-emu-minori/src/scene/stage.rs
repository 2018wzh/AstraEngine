...
mod tests {
    ...
    fn native_suffix_selects_screen_and_rejects_overlay() {
        assert_eq!(
            foreground_blend("minori:/bg/light_SC.PNG").unwrap(),
            BlendMode::Screen
        );
        assert_eq!(
            foreground_blend("minori:/bg/light.png").unwrap(),
            BlendMode::Alpha
        );
        assert_eq!(
            foreground_blend("minori:/bg/light_ov.png")
                .unwrap_err()
                .code(),
            "ASTRA_EMU_MINORI_STAGE_BLEND"
        );
