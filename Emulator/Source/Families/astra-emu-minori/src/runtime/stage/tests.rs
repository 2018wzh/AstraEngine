...
fn vm(source: &[u8]) -> MinoriVm {
    MinoriVm::new(
        "minori:/scr/stage.sc".into(),
...
fn native_stage_sequence_and_stand_parameters_survive_save_and_restore() {
    ...
    let Some(MinoriVmEvent::Stage(stage)) = live.step(1).unwrap() else {
        panic!("missing stage event");
    };
    assert_eq!(
        stage.resource_sequence,
        vec![Some("minori:/bg/front.png".into()), None]
    );
    ...
    let decoded = MinoriVm::decode_native_save(&saved).unwrap();
...
fn comma_in_resource_name_is_not_reinterpreted_as_a_stand_parameter() {
    ...
    assert_eq!(stand.resource_uri, "minori:/st/actor.png,1");
...
fn restore_rejects_invalid_stage_before_changing_live_state() {
    ...
    for variant in 0..4 {
        ...
        match variant {
            ...
            2 => stage.resource_sequence = vec![Some("minori:/bg/../private.png".into())],
            _ => invalid.schema = "astra.emu.minori.runtime_state.v7".into(),
        }
        let bytes = postcard::to_allocvec(&invalid).unwrap();
        assert!(MinoriVm::decode_native_save(&bytes).is_err());
