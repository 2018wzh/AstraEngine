...
pub(super) fn execute_select(
    ...
    state.message = None;
    state.wait = Some(MinoriWaitState::Input {
        token_id: format!("minori.choice.{}", state.instruction_count),
    });
    Ok(Some(MinoriVmEvent::Choice))
}

fn options(command: &ScCommand) -> Result<Vec<(String, String)>, MinoriRuntimeError> {
    ...
    tokens
        ...
        .zip(targets)
        .map(|(token, target)| {
            let (display, parsed) = token.split_once(':').ok_or(MinoriRuntimeError::Operand)?;
...
) -> Result<Vec<(String, String)>, MinoriRuntimeError> {
    let choice = state.choice.as_ref().ok_or(MinoriRuntimeError::State)?;
    let command = script
        .lines
        .iter()
...
pub(super) fn validate_choice(
    ...
    if let Some(choice) = &state.choice {
        ...
        if choice
            ...
            || !matches!(state.wait, Some(MinoriWaitState::Input { .. }))
        {
            return Err(MinoriRuntimeError::State);
        }
    }
...
impl MinoriVm {
    pub fn focus_choice(&mut self, index: u32) -> Result<bool, MinoriRuntimeError> {
    ...
    pub fn choice_display(&self) -> Result<Option<(Vec<String>, u32)>, MinoriRuntimeError> {
        validate_choice(&self.script, &self.state)?;
        self.state
            .choice
            .as_ref()
    ...
    pub fn move_choice(&mut self, direction: i32) -> Result<(), MinoriRuntimeError> {
        ...
        let choice = self
            ...
            .as_mut()
            .ok_or(MinoriRuntimeError::State)?;
        let current = choice.selected_index.ok_or(MinoriRuntimeError::State)? as i32;
    ...
    pub fn commit_choice(&mut self) -> Result<(), MinoriRuntimeError> {
        ...
        let pc = *self.labels.get(target).ok_or(MinoriRuntimeError::Label)?;
