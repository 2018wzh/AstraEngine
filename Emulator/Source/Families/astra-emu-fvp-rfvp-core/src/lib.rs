...
// This crate is a pinned MPL-2.0 compatibility derivative. These lints are
// retained only where changing upstream names or cfg-specific dormant code
// would change its observable surface or make future parity audits opaque.
#![allow(
    dead_code,
...
// Clippy findings in the imported revision are tracked as parity debt. The
...
pub mod rendering;
#[cfg(all(not(feature = "no_std"), not(feature = "host-command-audio")))]
pub mod rfvp_audio;
#[cfg(any(feature = "no_std", feature = "host-command-audio"))]
