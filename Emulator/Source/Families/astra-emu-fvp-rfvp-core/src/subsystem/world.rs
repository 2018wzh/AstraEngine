...
use crate::subsystem::components::syscalls::sound::{
    AudioLoad, AudioPlay, AudioSilentOn, AudioState, AudioStop, AudioType, AudioVol, SoundLoad,
    SoundMasterVol, SoundPlay, SoundSilentOn, SoundStop, SoundType, SoundTypeVol, SoundVol,
};
...
use crate::subsystem::components::syscalls::utils::{
    BreakPoint, FloatToInt, IntToText, Rand, SysAtSkipName, SysProjFolder,
};
...
use super::resources::input_manager::{InputManager, KeyCode};
...
pub struct SyscallJournalEntry {
    pub name: String,
...
pub struct GameData {
    ...
    runtime_vfs: Option<Arc<dyn RuntimeVfs>>,
    syscall_journal: Vec<SyscallJournalEntry>,
    time: Time,
...
impl GameData {
    ...
    pub unsafe fn init_default_in_place(dst: *mut GameData) {
        ...
        ptr::addr_of_mut!((*dst).runtime_vfs).write(None);
        ptr::addr_of_mut!((*dst).syscall_journal).write(Vec::new());
...
impl GameData {
    ...
    pub fn vfs_load_file(&self, path: &str) -> anyhow::Result<Vec<u8>> {
        if let Some(vfs) = &self.runtime_vfs {
            return vfs.read_file(path);
        }
        self.vfs.read_file(path)
    }

    pub fn set_runtime_vfs(&mut self, vfs: Arc<dyn RuntimeVfs>) {
        self.runtime_vfs = Some(vfs);
    }

    pub fn take_syscall_journal(&mut self) -> Vec<SyscallJournalEntry> {
        core::mem::take(&mut self.syscall_journal)
    }

    pub fn inject_keycode(&mut self, keycode: KeyCode, pressed: bool, repeat: bool) {
        if pressed {
            self.inputs_manager.notify_keycode_down(keycode, repeat);
        } else {
            self.inputs_manager.notify_keycode_up(keycode);
        }
    }

    pub fn inject_pointer_position(&mut self, x: i32, y: i32, in_screen: bool) {
        self.inputs_manager.notify_mouse_move(x, y);
...
impl VmSyscall for GameData {
    fn do_syscall(&mut self, name: &str, args: Vec<Variant>) -> anyhow::Result<Variant> {
        let syscall_table = SYSCALL_TBL.borrow();
        let syscall = syscall_table
            .get(name)
            .ok_or_else(|| anyhow::anyhow!("RFVP_UNKNOWN_SYSCALL: {name}"))?;
        let journal_args = args.clone();
        let result = syscall.call(self, args)?;
        self.syscall_journal.push(SyscallJournalEntry {
            name: name.to_owned(),
            args: journal_args,
            result: result.clone(),
        });
...
lazy_static::lazy_static! {
    static ref SYSCALL_TBL : AtomicRefCell<StableHashMap<String, Box<dyn Syscaller + 'static + Send + Sync>>> = {
        ...
        // Register the extracted catalog with fail-fast handlers. Explicit implementations below
        // replace these entries; any remaining gap is a blocking coverage failure at runtime.
...
pub fn syscall_coverage() -> (usize, Vec<&'static str>) {
    let table = SYSCALL_TBL.borrow();
    let release_specs = SYSCALL_SPECS
        .iter()
        .filter(|spec| spec.name != "BREAKPOINT")
        .collect::<Vec<_>>();
    let mut missing = release_specs
        .iter()
        .filter_map(|spec| {
            table
                .get(spec.name)
                .filter(|handler| !handler.is_implemented())
                .map(|_| spec.name)
        })
        .collect::<Vec<_>>();
    missing.sort_unstable();
    (release_specs.len(), missing)
}

#[cfg(test)]
mod syscall_coverage_tests {
    ...
    #[test]
    fn catalog_coverage_is_classified_without_soft_fail_handlers() {
        let (total, missing) = super::syscall_coverage();
        assert_eq!(total, 148);
        assert!(
            missing.is_empty(),
            "missing release syscall handlers: {missing:?}"
        );
    }

    #[test]
    fn every_release_syscall_has_a_panic_free_observable_neutral_probe() {
        let table = super::SYSCALL_TBL.borrow();
        for spec in SYSCALL_SPECS
            .iter()
            .filter(|spec| spec.name != "BREAKPOINT")
        {
            let handler = table
                .get(spec.name)
                .unwrap_or_else(|| panic!("release syscall is not registered: {}", spec.name));
            let argc = usize::try_from(spec.argc)
                .unwrap_or_else(|_| panic!("release syscall has variable argc: {}", spec.name));
            let mut game_data = super::GameData::default();
            let outcome = catch_unwind(AssertUnwindSafe(|| {
                handler.call(&mut game_data, vec![Variant::Nil; argc])
            }));
            let result =
                outcome.unwrap_or_else(|_| panic!("release syscall panicked: {}", spec.name));
            if let Err(error) = result {
                let diagnostic = error.to_string();
                assert!(
                    !diagnostic.contains("RFVP_SYSCALL_NOT_IMPLEMENTED"),
                    "release syscall has no observable implementation: {}",
                    spec.name
                );
                assert!(
                    !diagnostic.trim().is_empty(),
                    "release syscall returned an unclassified empty error: {}",
                    spec.name
                );
            }
        }
