    ...
    pub fn apply_snapshot_v1(
        ...
        for t in &snap.textures {
            ...
            if id < self.textures.len() {
                self.textures[id].apply_snapshot_v1(t, vfs).map_err(|_| {
                    let code = match t.load_kind {
                        GraphBuffLoadKind::Texture => "RFVP_SNAPSHOT_TEXTURE_RESTORE_TEXTURE",
                        GraphBuffLoadKind::Mask => "RFVP_SNAPSHOT_TEXTURE_RESTORE_MASK",
                        GraphBuffLoadKind::GaijiGlyph => "RFVP_SNAPSHOT_TEXTURE_RESTORE_GAIJI",
                        GraphBuffLoadKind::RawRgba => "RFVP_SNAPSHOT_TEXTURE_RESTORE_RAW_RGBA",
                        GraphBuffLoadKind::Unknown => "RFVP_SNAPSHOT_TEXTURE_RESTORE_UNKNOWN",
                    };
                    anyhow::anyhow!(code)
                })?;
            }
        ...
        self.text_manager.apply_snapshot_v1(&snap.text_manager);
        {
            ...
            pm.apply_snapshot_v1(&snap.parts_manager, vfs)
                .map_err(|_| anyhow::anyhow!("RFVP_SNAPSHOT_PARTS_RESTORE"))?;
        }
        self.gaiji_manager
            .apply_snapshot_v1(&snap.gaiji_manager, vfs)
            .map_err(|_| anyhow::anyhow!("RFVP_SNAPSHOT_GAIJI_RESTORE"))?;
        ...
        self.dissolve_mask_graph
            ...
            .map_err(|_| anyhow::anyhow!("RFVP_SNAPSHOT_DISSOLVE_MASK_RESTORE"))?;
