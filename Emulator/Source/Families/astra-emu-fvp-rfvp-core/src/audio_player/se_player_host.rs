...
impl SePlayer {
    ...
    pub fn load_named(&mut self, slot: i32, name: impl Into<String>, _se: Vec<u8>) -> Result<()> {
        let name = name.into();
        let slot = checked_slot(slot)?;
        self.audio_manager.push_command(AudioCommand::LoadResource {
            id: Self::id(slot),
    ...
    pub fn apply_snapshot_v1(&mut self, snap: &SePlayerSnapshotV1, _vfs: &Vfs) -> Result<()> {
        ...
        for slot in &snap.slots {
            ...
            if let Some(path) = slot.path.as_ref() {
                self.load_named(i as i32, path.clone(), Vec::new())?;
            }
            if slot.playing {
                self.play(
                    i as i32,
...
mod tests {
    ...
    #[test]
    fn snapshot_restore_reemits_resource_before_play_without_raw_vfs_read() {
        let audio_manager = Arc::new(AudioManager::new());
        let mut player = SePlayer::new(Arc::clone(&audio_manager));
        let vfs = Vfs::default();
        let snapshot = SePlayerSnapshotV1 {
            version: 1,
            slots: vec![SeSlotSnapshotV1 {
                slot: 17,
                path: Some("voice/restore-only.ogg".into()),
                sound_type: Some(1),
                volume: 0.5,
                muted: false,
                playing: true,
        ...
        player.apply_snapshot_v1(&snapshot, &vfs).unwrap();
        ...
        assert!(matches!(
            commands.as_slice(),
            [
                AudioCommand::LoadResource { uri, .. },
                AudioCommand::Play { .. }
            ] if uri == "voice/restore-only.ogg"
        ));
    }
}
