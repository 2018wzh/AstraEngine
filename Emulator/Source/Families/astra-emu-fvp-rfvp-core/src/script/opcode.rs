...
mod tests {
    ...
    #[test]
    fn numeric_and_mnemonic_tables_cover_the_complete_fvp_instruction_set() {
        for raw in 0_i32..=0x27 {
            let opcode = Opcode::try_from(raw).expect("all defined FVP opcodes must decode");
            assert_eq!(opcode as i32, raw);

            let mnemonic = opcode.to_string();
            assert_eq!(Opcode::try_from(mnemonic.as_str()), Ok(opcode));
        }

        for invalid in [-1, 0x28, i32::MAX] {
            assert!(Opcode::try_from(invalid).is_err());
        }

        // Preserve the observable rfvp correction for the two historically
        // swapped Rust variant names without changing their numeric ABI.
        assert_eq!(Opcode::SetLE as i32, 0x25);
        assert_eq!(Opcode::SetLE.to_string(), "set_ge");
        assert_eq!(Opcode::SetGE as i32, 0x27);
        assert_eq!(Opcode::SetGE.to_string(), "set_le");
    }
}
