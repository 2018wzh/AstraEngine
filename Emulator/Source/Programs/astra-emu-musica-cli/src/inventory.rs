...
use astra_emu_musica::REQUIRED_ARCHIVE_ROLES;
use serde::Serialize;
...
pub fn scan_archive_inventory(
    ...
    let root = game_root
        .canonicalize()
        .map_err(|_| "ASTRA_EMU_MUSICA_INVENTORY_ROOT")?;
    if !root.is_dir() {
        return Err("ASTRA_EMU_MUSICA_INVENTORY_ROOT".into());
    }
    let mut pending = vec![root];
    let mut scanned = 0usize;
    let mut roles = BTreeMap::<String, Vec<Part>>::new();
    while let Some(directory) = pending.pop() {
        for entry in fs::read_dir(directory).map_err(|_| "ASTRA_EMU_MUSICA_INVENTORY_READ")? {
            let entry = entry.map_err(|_| "ASTRA_EMU_MUSICA_INVENTORY_READ")?;
            let kind = entry
                .file_type()
                .map_err(|_| "ASTRA_EMU_MUSICA_INVENTORY_METADATA")?;
            if kind.is_symlink() {
                return Err("ASTRA_EMU_MUSICA_INVENTORY_SYMLINK".into());
            }
            if kind.is_dir() {
            ...
            scanned = scanned
                .checked_add(1)
                .ok_or("ASTRA_EMU_MUSICA_INVENTORY_FILE_LIMIT")?;
            if scanned > MAX_SCANNED_FILES {
                return Err("ASTRA_EMU_MUSICA_INVENTORY_FILE_LIMIT".into());
            }
            ...
            let byte_size = entry
                ...
                .len();
            if byte_size == 0 {
                return Err("ASTRA_EMU_MUSICA_INVENTORY_EMPTY".into());
            }
            let parts = roles.entry(role).or_default();
            if parts.iter().any(|existing| existing.index == part) {
                return Err("ASTRA_EMU_MUSICA_INVENTORY_DUPLICATE_PART".into());
            }
            parts.push(Part {
                index: part,
                byte_size,
    ...
    if roles.is_empty() {
        return Err("ASTRA_EMU_MUSICA_INVENTORY_MISSING".into());
    }
    ...
    for (role, mut parts) in roles {
        ...
        for (expected, part) in parts.iter().enumerate() {
            if usize::from(part.index) != expected {
                return Err("ASTRA_EMU_MUSICA_INVENTORY_PART_SEQUENCE".into());
            }
        ...
        let count =
            u64::try_from(parts.len()).map_err(|_| "ASTRA_EMU_MUSICA_INVENTORY_FILE_LIMIT")?;
        let bytes = parts.iter().try_fold(0u64, |total, part| {
            total
                .checked_add(part.byte_size)
                .ok_or("ASTRA_EMU_MUSICA_INVENTORY_SIZE")
        })?;
    ...
    Ok(ArchiveInventoryReport {
        schema: "astra.emu.musica.archive_inventory.v1",
        logical_archive_count: u64::try_from(archives.len())
            .map_err(|_| "ASTRA_EMU_MUSICA_INVENTORY_FILE_LIMIT")?,
        physical_file_count,
        physical_byte_count,
...
fn parse_archive_name(
    ...
    let name = name.to_str().ok_or("ASTRA_EMU_MUSICA_INVENTORY_FILENAME")?;
    ...
    {
        return Err("ASTRA_EMU_MUSICA_INVENTORY_ROLE".into());
    }
    ...
    fn missing_part_and_duplicate_case_variant_are_blocking() {
        ...
        assert_eq!(
            scan_archive_inventory(missing.path())
                .unwrap_err()
                .to_string(),
            "ASTRA_EMU_MUSICA_INVENTORY_PART_SEQUENCE"
        );
        ...
        assert_eq!(
            scan_archive_inventory(duplicate.path())
                .unwrap_err()
                .to_string(),
            "ASTRA_EMU_MUSICA_INVENTORY_DUPLICATE_PART"
        );
    }
}
