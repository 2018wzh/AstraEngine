...
#[derive(Debug, Parser)]
#[command(name = "astra-emu-musica-cli")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if result.is_err() {
        tracing::error!(
            event = "astra.emu.musica_cli.failed",
            diagnostic_code = "ASTRA_EMU_MUSICA_CLI_FAILED",
            action
        );
    } else {
        tracing::info!(event = "astra.emu.musica_cli.completed", action);
    }
    result
}
...
fn mount_musica(
    ...
    Ok(mount_family_vfs(
        ...
        game_dir,
        profile,
        vec![Arc::new(MusicaVfsFamilyFactory)],
    )?)
}
...
fn census(
    ...
    println!(
        ...
        serde_json::to_string(&serde_json::json!({
            "schema": "astra.emu.musica.sc_census.v5",
...
fn census_audio_resources(
    ...
    for entry in &manifest.entries {
        let prefix = format!("musica:/{}/", entry.source_id);
        let relative = entry
            .uri
            .strip_prefix(&prefix)
            .ok_or("ASTRA_EMU_MUSICA_CENSUS_URI_ROLE")?;
        entries
...
fn census_media(
        ...
        .filter(|entry| matches!(entry.source_id.as_str(), "bg" | "bgm"))
    {
        if entry.decoded_size > MAX_ENTRY_BYTES {
            return Err("ASTRA_EMU_MUSICA_MEDIA_ENTRY_LIMIT".into());
        }
        ...
        if bytes.len() as u64 != entry.decoded_size {
            return Err("ASTRA_EMU_MUSICA_MEDIA_ENTRY_SIZE".into());
        }
        decoded_bytes = decoded_bytes
            .checked_add(entry.decoded_size)
            .ok_or("ASTRA_EMU_MUSICA_MEDIA_TOTAL_SIZE")?;
        let extension = entry
            .uri
            .rsplit_once('.')
        ...
        match extension.as_str() {
            "png" => {
                let image = image::load_from_memory_with_format(&bytes, image::ImageFormat::Png)
                    .map_err(|_| "ASTRA_EMU_MUSICA_MEDIA_PNG")?;
                record_dimensions(
                    image.width(),
                    image.height(),
                    &mut decoded_pixels,
            ...
            "ani" => {
                let archive = MusicaAniArchive::parse(Arc::<[u8]>::from(bytes))?;
            ...
            "sqz" => {
                let archive = MusicaSqzArchive::parse(Arc::<[u8]>::from(bytes))?;
                for index in 0..archive.frames().len() {
                    let frame = archive.decode_frame(index)?;
                    record_dimensions(
                        frame.width(),
...
fn census_movies(
    ...
    let vfs = mount_musica(game_dir, profile)?;
    let report = serde_json::to_string(&serde_json::json!({
        "schema": "astra.emu.musica.movie_census.v1",
...
fn collect_movie_container_census(
        ...
        .filter(|entry| entry.source_id == "mov")
    {
        if entry.decoded_size == 0 {
            return Err("ASTRA_EMU_MUSICA_MOVIE_EMPTY".into());
        }
        movie.entry_count = movie
            .entry_count
            .checked_add(1)
            .ok_or("ASTRA_EMU_MUSICA_MOVIE_COUNT")?;
        movie.decoded_bytes = movie
            .decoded_bytes
            .checked_add(entry.decoded_size)
            .ok_or("ASTRA_EMU_MUSICA_MOVIE_TOTAL_SIZE")?;
...
fn classify_movie_entry(
    ...
    while offset < scan_limit {
        ...
        if bytes.len() as u64 != length {
            return Err("ASTRA_EMU_MUSICA_MOVIE_PROBE_SHORT".into());
        }
        ...
        offset = offset
            .checked_add(length)
            .ok_or("ASTRA_EMU_MUSICA_MOVIE_SCAN_OFFSET")?;
        if tail_length > MOVIE_SIGNATURE_TAIL_BYTES {
            return Err("ASTRA_EMU_MUSICA_MOVIE_SCAN_STATE".into());
        }
    }
    Ok("unrecognized")
}
...
fn write_private_report(
    ...
    if path.exists() {
        return Err("ASTRA_EMU_MUSICA_REPORT_EXISTS".into());
    }
    let parent = path.parent().ok_or("ASTRA_EMU_MUSICA_REPORT_PATH")?;
    if !parent.is_dir() {
        return Err("ASTRA_EMU_MUSICA_REPORT_PARENT".into());
    }
    let file_name = path.file_name().ok_or("ASTRA_EMU_MUSICA_REPORT_PATH")?;
    let temporary = parent.join(format!(".{}.tmp", file_name.to_string_lossy()));
    if temporary.exists() {
        return Err("ASTRA_EMU_MUSICA_REPORT_TEMP_EXISTS".into());
    }
...
fn record_dimensions(
    ...
    *decoded_pixels = decoded_pixels
        ...
        .ok_or("ASTRA_EMU_MUSICA_MEDIA_PIXEL_TOTAL")?;
