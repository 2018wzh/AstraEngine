...
namespace TJS {
    ...
    public:
        ...
        static void Compile(tjs_int numparam, tTJSVariant **param,
                            tTJSNI_RegExp *_this);

        static bool Match(OnigRegion *region, const ttstr &target,
                          tTJSNI_RegExp *_this);

        static bool Exec(OnigRegion *region, const ttstr &target,
                         tTJSNI_RegExp *_this);

        static iTJSDispatch2 *GetResultArray(bool matched,
                                             const tjs_char *target,
                                             tTJSNI_RegExp *_this,
                                             const OnigRegion *region);

        static iTJSDispatch2 *GetResultArray(bool matched, const ttstr &target,
                                             tTJSNI_RegExp *_this,
                                             const OnigRegion *region) {
