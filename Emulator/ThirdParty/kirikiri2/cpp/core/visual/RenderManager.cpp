...
static void *TVPAllocBitmapBits(tjs_uint size, tjs_uint width,
    ...
    {
        tjs_uint8 *ptr;
        size += sizeof(tTVPLayerBitmapMemoryRecord);
        uint32_t pagesize = (size + 4095) & ~4095;
        ptr = (tjs_uint8 *)malloc(pagesize + 8192);
        uint8_t *p = (uint8_t *)(((intptr_t)ptr + 4095) & ~4095);
        tTVPLayerBitmapMemoryRecord *hdr =
            (tTVPLayerBitmapMemoryRecord *)(p + pagesize - size);
        DWORD old;
    ...
    *(tjs_uint32 *)(ptr + size) = ~record->sentinel_backup2;
    // Stored sentinels are nagated, to avoid that the sentinel
    // backups in tTVPLayerBitmapMemoryRecord becomes the same value
    // as the sentinels. This trick will make the detection of the
    // memory corruption easier. Because on some occasions, running
    // memory writing will write the same values at first sentinel and
    // the tTVPLayerBitmapMemoryRecord.
...
void tTVPBitmap::Allocate(tjs_uint width, tjs_uint height, tjs_uint bpp)
{
	...
	{
		free(BitmapInfo), BitmapInfo = nullptr;
		throw;
	}
}
//---------------------------------------------------------------------------
...
public:
    ...
    GetAdapterTexture(cocos2d::Texture2D *origTex) override {
        ...
        // Hosted mode has no cocos adapter; forward the original.
        return origTex;
#else
...
static const tjs_uint8
    __empty_line[EMPTY_LINE_BYTES + 32] = {}; // at most 2048 pixels per line
static const tjs_uint8 *_empty_line =
    (const tjs_uint8 *)(((intptr_t)(&__empty_line) + 15) & ~15);

class tTVPSoftwareTexture2D_compress : public tTVPSoftwareTexture2D_static,
                                       public tTVPContinuousEventCallbackIntf {
protected:
    tjs_int PixelFrameLife = 0;

    tTVPSoftwareTexture2D_compress(int pitch, unsigned int w, unsigned int h,
                                   TVPTextureFormat::e format) :
...
public:
    const void *GetPixelData() override {
        if(!BmpData) {
            ...
            tjs_int w = Width *
                (GetFormat() == TVPTextureFormat::RGBA ? sizeof(tjs_uint32)
                                                       : sizeof(tjs_uint8));
...
public:
    tTVPSoftwareTexture2D_half(tTVPBitmap *bmp, const void *pixel, int pitch,
    ...
    static iTVPTexture2D *Create(tTVPBitmap *bmp, const void *pixel, int pitch,
                                 unsigned int w, unsigned int h,
                                 TVPTextureFormat::e format) {
...
public:
    tTVPSoftwareTexture2D_lz4(int pitch, unsigned int w, unsigned int h,
                              TVPTextureFormat::e format) :
        tTVPSoftwareTexture2D_compress(pitch, w, h, format) {}
    ...
    void Init(tTVPBitmap *bmp, const void *pixel, unsigned int w,
              unsigned int h) {
        ...
        std::vector<char> tmp;
        tmp.resize(blkSize + w);
        tjs_uint y = 0;
        for(; y < h - BlockH + 1; y += BlockH, src += blkSize) {
            tjs_uint dstSize =
                LZ4_compress_default(src, &tmp.front(), blkSize, blkSize + w);
            Block blk = { new char[dstSize], dstSize, BlockH };
        ...
        if(h > y) {
            ...
            tjs_uint dstSize =
                LZ4_compress_default(src, &tmp.front(), blkSize, blkSize + w);
            Block blk = { new char[dstSize], dstSize, h };
            memcpy(blk.Data, &tmp.front(), dstSize);
            CompressedBlock.emplace_back(blk);
            DataSize += dstSize;
        }
    ...
    static iTVPTexture2D *Create(tTVPBitmap *bmp, const void *pixel, int pitch,
                                 unsigned int w, unsigned int h,
                                 TVPTextureFormat::e format) {
        tTVPSoftwareTexture2D_lz4 *tex =
            new tTVPSoftwareTexture2D_lz4(pitch, w, h, format);
...
public:
    tTVPSoftwareTexture2D_lz4_tlg5(int pitch, unsigned int w, unsigned int h,
                                   TVPTextureFormat::e format) :
        tTVPSoftwareTexture2D_lz4(pitch, w, h, format) {}

    void CompressBlock(const char *src, tjs_uint w, tjs_uint h, char *tmp,
                       char *tranbuf) {
    ...
    static iTVPTexture2D *Create(tTVPBitmap *bmp, const void *pixel, int pitch,
                                 ...
                                 TVPTextureFormat::e format) {
        if(format == TVPTextureFormat::RGBA) {
            tTVPSoftwareTexture2D_lz4_tlg5 *tex =
                new tTVPSoftwareTexture2D_lz4_tlg5(pitch, w, h, format);
            tex->Init(bmp, pixel, w, h);
            return tex;
        }
        return tTVPSoftwareTexture2D_lz4::Create(bmp, pixel, pitch, w, h,
                                                 format);
    }
...
class tTVPSoftwareTexture2D : public tTVPSoftwareTexture2D_static {
    tTVPSoftwareTexture2D(tTVPBitmap *bmp) :
        tTVPSoftwareTexture2D_static(
            bmp->GetBits(), bmp->GetPitch(), bmp->GetWidth(), bmp->GetHeight(),
...
#endif
            if(pitch) {
                Update(tex->GetScanLineForRead(0), tex->GetFormat(), pitch,
...
template <typename TSrc, typename TDst,
          void (*&FuncWithOpa)(TDst *, const TSrc *, tjs_int, tjs_uint32,
                               tjs_int),
...
class tTVPRenderMethod_FillWithColor
...
public:
    void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        TVPExecThreadTask(taskNum, [=](int i) {
            ...
            y0 = h * i / taskNum;
            y1 = h * (i + 1) / taskNum;
            this->PartialCopy(_tar, rctar.left, rctar.top + y0, _src,
                              rcsrc.left, rcsrc.top + y0, w, y1 - y0,
                              backwardCopy);
            // 			PartialParam _param;
    ...
    virtual void PartialCopy(iTVPTexture2D *dst, tjs_int dx, tjs_int dy,
...
public:
    void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        if(rcsrc.top > rcsrc.bottom) {
            ...
            // 			tjs_uint8 *src =
            // (tjs_uint8*)_src->GetScanLineForRead(rcsrc.top - 1) +
            // rcsrc.left
            // * pixelsize; 			tjs_uint8 *dst =
            // (tjs_uint8*)_tar->GetScanLineForRead(rctar.top) +
            // rctar.left * pixelsize; 			tjs_int spitch =
            // -_src->GetPitch(), dpitch = _tar->GetPitch();
            // tjs_int wbytes = w * pixelsize; 			while (h--) {
            // memcpy(dst,
            // src, wbytes); 				src += spitch; dst +=
            // dpitch;
            // 			}
            ...
            // rcsrc.right * pixelsize; 			tjs_uint8 *dst =
            // (tjs_uint8*)_tar->GetScanLineForRead(rctar.top) +
            // rctar.left * pixelsize; 			tjs_int spitch =
            // _src->GetPitch(), dpitch = _tar->GetPitch();
            tjs_int wbytes = w * pixelsize;
            ...
            if(pixelsize == 4) { // 32bpp
                ...
                // 					memcpy(dst, src,
                // wbytes);
                // TVPReverse32((tjs_uint32*)dst, w); src += spitch;
                // dst += dpitch;
                // 				}
            } else { // 8bpp
                for(int y = 0; y < h; ++y) {
                ...
                // 				while (h--) {
                // 					memcpy(dst, src,
                // wbytes); 					TVPReverse8(dst, w);
                // src += spitch; dst += dpitch;
                // 				}
            }
        } else {
            // 			tjs_int spitch = _src->GetPitch(), dpitch =
            // _tar->GetPitch(); 			tjs_uint8 *dest, *src;
            tjs_int h = rcsrc.bottom - rcsrc.top;
            tjs_int w = rcsrc.right - rcsrc.left;
            assert(w == rctar.get_width() && h == rctar.get_height());
            bool backwardCopy = (_tar == _src && rctar.top > rcsrc.top);
            ...
            tjs_int taskNum =
                _src == _tar ? 1 : GetAdaptiveThreadNum(w * h, 66);
            TVPExecThreadTask(taskNum, [=](int i) {
...
public:
    void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        tjs_int taskNum = GetAdaptiveThreadNum(w * h, THREAD_FACTOR);
        TVPExecThreadTask(
            taskNum, [this, _src, _tar, sx, sy, dx, dy, w, h, taskNum](int i) {
                tjs_int y0, y1;
                y0 = h * i / taskNum;
                y1 = h * (i + 1) / taskNum;
                this->PartialFill(_tar, _src, sx, sy + y0, dx, dy + y0, w,
                                  y1 - y0);
            });
    }
...
template <int THREAD_FACTOR,
          void (*&Func)(tjs_uint32 *, const tjs_uint32 *, tjs_int)>
class tTVPRenderMethod_Blt
    ...
    void PartialFill(iTVPTexture2D *dst, iTVPTexture2D *src, tjs_int sx,
        ...
        // 		while (height--)
        // 		{
        // 			Func((tjs_uint32*)dst, (const tjs_uint32*)src,
        // width); 			src += spitch; dst += dpitch;
        // 		}
    }
};

template <int THREAD_FACTOR,
          void (*&Func)(tjs_uint32 *, const tjs_uint32 *, const tjs_uint32 *,
                        tjs_int, tjs_int)>
...
public:
    void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        // 		src = (tjs_uint8*)_src->GetScanLineForRead(rcsrc.top)
        ...
        TVPExecThreadTask(taskNum, [&](int i) {
            ...
            y0 = h * i / taskNum;
            y1 = h * (i + 1) / taskNum;
            this->PartialProc(_tar, rctar.left, rctar.top + y0, _src,
                              rcsrc.left, rcsrc.top + y0, _dst, rcdst.left,
    ...
    virtual void PartialProc(iTVPTexture2D *tar, tjs_int tx, tjs_int ty,
        ...
        // 		tjs_uint8 *dst = p->dest, *src = p->src, *tar =
        // p->tar; 		tjs_int width = p->w; 		tjs_int height = p->h;

        // 		while (height--)
        // 		{
        // 			Func((tjs_uint32*)tar, (const tjs_uint32*)src,
        // (const tjs_uint32*)dst,
        // width, opa); 			src += spitch; dst += dpitch; tar
        // += tpitch;
        // 		}
    }
};
...
template <int THREAD_FACTOR,
          ...
          void (*&FuncSwitch)(tjs_uint32 *, const tjs_uint32 *,
...
public:
    void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        TVPExecThreadTask(taskNum, [&](int i) {
            ...
            this->PartialProc(_tar, rctar.left, rctar.top + y0, _src,
                              rcsrc.left, rcsrc.top + y0, _dst, rcdst.left,
                              rcdst.top + y0, _rule, rcrule.left,
    ...
    virtual void PartialProc(iTVPTexture2D *tar, tjs_int tx, tjs_int ty,
                             ...
                             tjs_int w, tjs_int h) {
        // 		tjs_uint8 *dst = p->dest, *src = p->src, *tar =
        // p->tar, *rule = p->rule; 		tjs_int width = p->w;
        // tjs_int height = p->h;
        ...
        if(vague >= 512) {
            ...
            // 			while (height--)
            // 			{
            // 				Func((tjs_uint32*)tar, (const
            // tjs_uint32*)src, (const tjs_uint32*)dst, rule,
            // BlendTable, width); 				src
            // += spitch; dst += dpitch; tar += tpitch; rule +=
            // rpitch;
            // 			}
        } else {
            ...
            // 			while (height--)
            // 			{
            // 				FuncSwitch((tjs_uint32*)tar, (const
            // tjs_uint32*)src, (const tjs_uint32*)dst, rule,
            // BlendTable, width, src1lv, src2lv); 				src +=
            // spitch; dst += dpitch; tar
            // += tpitch; rule += rpitch;
            // 			}
        }
    }
...
template <
    int THREAD_FACTOR, void (*&Func)(tjs_uint32 *, const tjs_uint32 *, tjs_int),
    void (*&FuncWithOpa)(tjs_uint32 *, const tjs_uint32 *, tjs_int, tjs_int)>
class tTVPRenderMethod_BltAndOpa
    : public tTVPRenderMethod_BaseBlt<tjs_uint32, THREAD_FACTOR> {
    typedef tTVPRenderMethod_BaseBlt<tjs_uint32, THREAD_FACTOR> inherit;
...
template <int THREAD_FACTOR,
          void (*&FuncWithOpa)(tjs_uint32 *, const tjs_uint32 *, tjs_int,
                               tjs_int)>
class tTVPRenderMethod_BltWithOpa
    : public tTVPRenderMethod_BaseBlt<tjs_uint32, THREAD_FACTOR> {
    typedef tTVPRenderMethod_BaseBlt<tjs_uint32, THREAD_FACTOR> inherit;
...
template <int THREAD_FACTOR,
          void (*&FuncWithOpa)(tjs_uint32 *, const tjs_uint32 *,
                               const tjs_uint32 *, tjs_int, tjs_int)>
class tTVPRenderMethod_BltWithOpa_SD
    : public tTVPRenderMethod_BaseBlt<tjs_uint32, THREAD_FACTOR> {
    typedef tTVPRenderMethod_BaseBlt<tjs_uint32, THREAD_FACTOR> inherit;
    tjs_int opa;
...
// some blur operation template functions to select algorithm by base
// integer type
...
template <>
void TVPAddSubVertSum<tTVPARGB<tjs_uint16>, tjs_uint16>(
    tjs_uint16 *dest, const tjs_uint32 *addline, const tjs_uint32 *subline,
    tjs_int len) {
    TVPAddSubVertSum16(dest, addline, subline, len);
}
template <>
void TVPAddSubVertSum<tTVPARGB_AA<tjs_uint16>, tjs_uint16>(
    tjs_uint16 *dest, const tjs_uint32 *addline, const tjs_uint32 *subline,
    tjs_int len) {
    TVPAddSubVertSum16_d(dest, addline, subline, len);
}
template <>
void TVPAddSubVertSum<tTVPARGB<tjs_uint32>, tjs_uint32>(
    tjs_uint32 *dest, const tjs_uint32 *addline, const tjs_uint32 *subline,
    tjs_int len) {
    TVPAddSubVertSum32(dest, addline, subline, len);
}
template <>
void TVPAddSubVertSum<tTVPARGB_AA<tjs_uint32>, tjs_uint32>(
    tjs_uint32 *dest, const tjs_uint32 *addline, const tjs_uint32 *subline,
    tjs_int len) {
...
template <>
void TVPDoBoxBlurAvg<tTVPARGB<tjs_uint16>, tjs_uint16>(tjs_uint32 *dest,
                                                       tjs_uint16 *sum,
                                                       const tjs_uint16 *add,
                                                       const tjs_uint16 *sub,
                                                       tjs_int n, tjs_int len) {
    TVPDoBoxBlurAvg16(dest, sum, add, sub, n, len);
}
template <>
void TVPDoBoxBlurAvg<tTVPARGB_AA<tjs_uint16>, tjs_uint16>(
...
template <>
void TVPDoBoxBlurAvg<tTVPARGB<tjs_uint32>, tjs_uint32>(tjs_uint32 *dest,
                                                       tjs_uint32 *sum,
                                                       const tjs_uint32 *add,
                                                       const tjs_uint32 *sub,
                                                       tjs_int n, tjs_int len) {
    TVPDoBoxBlurAvg32(dest, sum, add, sub, n, len);
}
template <>
void TVPDoBoxBlurAvg<tTVPARGB_AA<tjs_uint32>, tjs_uint32>(
...
struct TDoBoxBlurLoop {
    ...
    static void DoBoxBlurLoop(const tTVPRect &rect, const tTVPRect &area,
                              tjs_int width /*of texture*/,
                              tjs_int height /*of texture*/, tjs_uint32 *line0,
                              tjs_int pitch) {
        // Box-Blur template function used by
        // tTVPBaseBitmap::DoBoxBlur family. Based on contributed blur
        // code by yun, say thanks to him.
        ...
        try {
            // allocate buffers
            vert_sum = (tARGB *)TJSAlignedAlloc(
                sizeof(tARGB) *
                    (vert_sum_right_limit - vert_sum_left_limit + 1 + 1),
                4); // use 128bit aligned allocation
            ...
            // initialize vert_sum
            {
                ...
                for(tjs_int y = v_init_start; y <= v_init_end; y++) {
                    ...
                    tARGB *vs = vert_sum;
                    for(int x = vert_sum_left_limit; x <= vert_sum_right_limit;
                        x++)
                        *(vs++) += add_line[x];
                }
            }
            ...
            tjs_int h_init_start =
                rect.left + area.left; // this always be the same
                                       // value as vert_sum_left_limit
            if(h_init_start < 0)
                h_init_start = 0;
            ...
            tjs_int right_frac_len = rect.right + area.right >= width
                ? rect.right + area.right - width + 1
                : 0;
            tjs_int center_len =
                rect.right - rect.left - left_frac_len - right_frac_len;
            ...
            for(tjs_int y = rect.top; y < rect.bottom; y++) {
                ...
                if(dest_buf_free == 0) {
                    ...
                    memcpy(rect.left + line0 + pitch * (y - dest_buf_size),
                ...
                if(center_len > 0) {
                    ...
                    TVPDoBoxBlurAvg<tARGB>(
                        dp, &sum.packed,
                        &(vert_sum + x + area.right + 1 - vert_sum_left_limit)
                             ->packed,
                        &(vert_sum + x + area.left - vert_sum_left_limit)
                             ->packed,
                        horz_sum_count * vert_sum_count, center_len);
                    dp += center_len;
                }
                x = center_lim;
                ...
                if(y != rect.bottom - 1) {
                    ...
                    const tjs_uint32 *add_line;
                    sub_line = y + area.top < 0
                        ? nullptr
                        : (line0 + (y + area.top) * pitch);
                    add_line = y + area.bottom + 1 >= height
                        ? nullptr
                        : (line0 + (y + area.bottom + 1) * pitch);

                    if(sub_line && add_line) {
                        // both sub_line and add_line are available
                        // uses function in tvpgl
                        TVPAddSubVertSum<tARGB>(
                            &vert_sum->packed, add_line + vert_sum_left_limit,
                        ...
                        tARGB *vs = vert_sum;
                        for(int x = vert_sum_left_limit;
                            x <= vert_sum_right_limit; x++)
                            *vs -= sub_line[x], vs++;
                        vert_sum_count--;
                    } else if(add_line) {
            ...
            while(dest_buf_free < dest_buf_size) {
                memcpy(rect.left + line0 +
                           (rect.bottom - (dest_buf_size - dest_buf_free)) *
                               pitch,
                       &dest_buf[dest_buf_wp][0],
                       (rect.right - rect.left) * sizeof(tjs_uint32));
            ...
            if(dest_buf_size) {
                for(tjs_int i = 0; i < dest_buf_size; i++) {
                    //	_log_ptr("free  bufN", &dest_buf[i][0],
                    //(rect.right -
                    // rect.left)
                    //* 4);
...
public:
    int EnumParameterID(const char *name) override {
        ...
        if(!strcmp(name, "area_top"))
            return 1;
        if(!strcmp(name, "area_right"))
            return 2;
        if(!strcmp(name, "area_bottom"))
            return 3;
        return -1;
    }
    void SetParameterInt(int id, int v) override {
    ...
    void DoRender(iTVPTexture2D *tar, const tTVPRect &rctar, iTVPTexture2D *dst,
        ...
        // 			(area.right - area.left + 1) * (area.bottom -
        // area.top
        // + 1);
        ...
        // pitchBytes); else if (area_size < (1L << 24))
        // Func32::DoBoxBlurLoop(rctar, area, _tar->GetWidth(),
        // _tar->GetHeight(), (tjs_uint32*)line, pitchBytes); else
        // 			TVPThrowExceptionMessage(TVPBoxBlurAreaMustBeSmallerThan16Million);
    }
};

iTVPRenderMethod *iTVPRenderManager::GetRenderMethod(const char *name,
                                                     tjs_uint32 *hint) {
    tjs_uint32 hash;
    if(hint && *hint) {
        hash = *hint;
...
struct tRenderMethodCache {
    struct MethodInfo {
        ...
        void Init(iTVPRenderManager *mgr, const char *nameNormal,
                  const char *nameHDA, const char *nameOpa,
                  const char *nameHDAOpa) {
            if(nameNormal)
                Normal = mgr->GetRenderMethod(nameNormal);
            if(nameHDA)
                HDA = mgr->GetRenderMethod(nameHDA);
            if(nameOpa) {
    ...
    tRenderMethodCache(iTVPRenderManager *mgr) {
        Info[bmCopy].Init(mgr, "Copy", "CopyColor", "ConstAlphaBlend",
                          "ConstAlphaBlend");
        Info[bmCopyOnAlpha].Init(mgr, "CopyOpaqueImage", "CopyColor",
                                 "ConstAlphaBlend_d", "ConstAlphaBlend_d");
        Info[bmAlpha].Init(mgr, nullptr, nullptr, "AlphaBlend", "AlphaBlend");
        Info[bmAlphaOnAlpha].Init(mgr, nullptr, nullptr, "AlphaBlend_d",
                                  "AlphaBlend_d");
        ...
        Info[bmDodge].Init(mgr, nullptr, nullptr, "ColorDodgeBlend",
                           "ColorDodgeBlend");
        Info[bmDarken].Init(mgr, nullptr, nullptr, "DarkenBlend",
                            "DarkenBlend");
        Info[bmLighten].Init(mgr, nullptr, nullptr, "LightenBlend",
                             "LightenBlend");
        Info[bmScreen].Init(mgr, nullptr, nullptr, "ScreenBlend",
                            "ScreenBlend");
        Info[bmAddAlpha].Init(mgr, nullptr, nullptr, "AdditiveAlphaBlend",
                              "AdditiveAlphaBlend");
        Info[bmAddAlphaOnAddAlpha].Init(mgr, nullptr, nullptr,
                                        "AdditiveAlphaBlend_a",
                                        "AdditiveAlphaBlend_a");
        // Info[bmAddAlphaOnAlpha].Init(mgr, nullptr, nullptr,
        // nullptr, nullptr);
        Info[bmAlphaOnAddAlpha].Init(mgr, nullptr, nullptr, "AlphaBlend_a",
                                     "AlphaBlend_a");
        Info[bmCopyOnAddAlpha].Init(mgr, "CopyOpaqueImage", nullptr,
                                    "ConstAlphaBlend_a", "ConstAlphaBlend_a");
        Info[bmPsNormal].Init(mgr, nullptr, nullptr, "PsAlphaBlend",
                              "PsAlphaBlend");
        Info[bmPsAdditive].Init(mgr, nullptr, nullptr, "PsAddBlend",
                                "PsAddBlend");
        Info[bmPsSubtractive].Init(mgr, nullptr, nullptr, "PsSubBlend",
                                   "PsSubBlend");
        Info[bmPsMultiplicative].Init(mgr, nullptr, nullptr, "PsMulBlend",
                                      "PsMulBlend");
        Info[bmPsScreen].Init(mgr, nullptr, nullptr, "PsScreenBlend",
                              "PsScreenBlend");
        Info[bmPsOverlay].Init(mgr, nullptr, nullptr, "PsOverlayBlend",
                               "PsOverlayBlend");
        Info[bmPsHardLight].Init(mgr, nullptr, nullptr, "PsHardLightBlend",
                                 "PsHardLightBlend");
        Info[bmPsSoftLight].Init(mgr, nullptr, nullptr, "PsSoftLightBlend",
                                 "PsSoftLightBlend");
        Info[bmPsColorDodge].Init(mgr, nullptr, nullptr, "PsColorDodgeBlend",
                                  "PsColorDodgeBlend");
        Info[bmPsColorDodge5].Init(mgr, nullptr, nullptr, "PsColorDodge5Blend",
                                   "PsColorDodge5Blend");
        Info[bmPsColorBurn].Init(mgr, nullptr, nullptr, "PsColorBurnBlend",
                                 "PsColorBurnBlend");
        Info[bmPsLighten].Init(mgr, nullptr, nullptr, "PsLightenBlend",
                               "PsLightenBlend");
        Info[bmPsDarken].Init(mgr, nullptr, nullptr, "PsDarkenBlend",
                              "PsDarkenBlend");
        Info[bmPsDifference].Init(mgr, nullptr, nullptr, "PsDiffBlend",
                                  "PsDiffBlend");
        Info[bmPsDifference5].Init(mgr, nullptr, nullptr, "PsDiff5Blend",
                                   "PsDiff5Blend");
        Info[bmPsExclusion].Init(mgr, nullptr, nullptr, "PsExclusionBlend",
                                 "PsExclusionBlend");
    }
...
iTVPRenderMethod *
iTVPRenderManager::GetRenderMethod(tjs_int opa, bool hda,
                                   int /*tTVPBBBltMethod*/ method) {
    switch(method) {
        case bmCopy:
            // constant ratio alpha blendng
        case bmCopyOnAlpha:
            // constant ratio alpha blending (assuming source is
            // opaque) with consideration of destination alpha
        case bmCopyOnAddAlpha:
            // constant ratio alpha blending (assuming source is
            // opaque) with consideration of destination additive
            // alpha
            if(opa == 255) {
                return hda ? RenderMethodCache->Info[method].HDA
                           : RenderMethodCache->Info[method].Normal;
            } else {
        ...
        case bmAdd:
            // additive blending ( this does not consider distination
            // alpha )
        case bmSub:
            // subtractive blending ( this does not consider
            // distination alpha )
        case bmDodge:
            // color dodge mode ( this does not consider distination
            // alpha )
        case bmDarken:
        ...
        case bmLighten:
            ...
            // )
        case bmScreen:
            // screen multiplicative mode ( this does not consider
            // distination alpha
            // )
        case bmAddAlpha:
            // Additive Alpha
        ...
        case bmMul:
            // multiplicative blending ( this does not consider
            // distination alpha )
...
iTVPRenderMethod *
iTVPRenderManager::GetOrCompileRenderMethod(const char *name, uint32_t *hint,
...
class tTVPSoftwareRenderManager : public iTVPRenderManager {
    ...
    iTVPTexture2D *getTempTexture(unsigned int w, unsigned int h) {
        if(!tempTexture || tempTexture->GetWidth() < w ||
           tempTexture->GetHeight() < h) {
            if(tempTexture)
                tempTexture->Release();
            tempTexture =
                CreateTexture2D(nullptr, 0, w, h, TVPTextureFormat::RGBA);
        }
        return tempTexture;
    }
...
public:
    void Register_1() {
        // ---------- ApplyColorMap ----------
        // 		{
        // 			static tTVPRenderMethod_ColorOpacity<tjs_uint8,
        // tjs_uint32, TVPApplyColorMap_o, TVPApplyColorMap> method;
        // 			RegisterRenderMethod("ApplyColorMap", &method);
        // 		}
        {
            static tTVPRenderMethod_ColorOpacity<tjs_uint8, tjs_uint32,
        ...
        {
            static tTVPRenderMethod_ColorOpacity<
                tjs_uint8, tjs_uint32, TVPApplyColorMap_do, TVPApplyColorMap_d>
                method;
            RegisterRenderMethod("ApplyColorMap_d", &method);
        }
        {
            static tTVPRenderMethod_ColorOpacity<
                tjs_uint8, tjs_uint32, TVPApplyColorMap_ao, TVPApplyColorMap_a>
                method;
        ...
        {
            static tTVPRenderMethod_FillWithColor<tjs_uint32, 115, TVPFillColor>
                method;
            RegisterRenderMethod("FillColor", &method);
        }
        {
            static tTVPRenderMethod_FillWithOpacity<tjs_uint32, 84, tjs_uint32,
                                                    TVPFillMask>
                method;
        ...
        {
            static tTVPRenderMethod_FillWithColorOpa<tjs_uint32, 55,
                                                     TVPConstColorAlphaBlend>
                method;
            RegisterRenderMethod("ConstColorAlphaBlend", &method);
        }
        {
            static tTVPRenderMethod_FillWithColorOpa<tjs_uint32, 25,
                                                     TVPConstColorAlphaBlend_d>
                method;
            RegisterRenderMethod("ConstColorAlphaBlend_d", &method);
        }
        {
            static tTVPRenderMethod_FillWithColorOpa<tjs_uint32, 147,
                                                     TVPConstColorAlphaBlend_a>
                method;
            RegisterRenderMethod("ConstColorAlphaBlend_a", &method);
        }
        {
            static tTVPRenderMethod_FillWithOpacity<tjs_uint32, 50, tjs_int,
        ...
        {
            static tTVPRenderMethod_TransBlt<59, TVPConstAlphaBlend_SD_a>
                method;
            RegisterRenderMethod("ConstAlphaBlend_SD_a", &method);
        }
        {
            static tTVPRenderMethod_TransBlt<59, TVPConstAlphaBlend_SD_d>
                method;
            RegisterRenderMethod("ConstAlphaBlend_SD_d", &method);
        }
        {
            static tTVPRenderMethod_BltWithOpa_SD<59, TVPConstAlphaBlend_SD>
                method;
        ...
        // --------- UnivTransBlend ----------
        {
            static tTVPRenderMethod_UnivTransBlt<
                46, TVPInitUnivTransBlendTable_d, TVPUnivTransBlend_d,
                TVPUnivTransBlend_switch_d>
                method;
            RegisterRenderMethod("UnivTransBlend_d", &method);
        }
        {
            static tTVPRenderMethod_UnivTransBlt<
                46, TVPInitUnivTransBlendTable_a, TVPUnivTransBlend_a,
                TVPUnivTransBlend_switch_a>
                method;
            RegisterRenderMethod("UnivTransBlend_a", &method);
        }
        {
            static tTVPRenderMethod_UnivTransBlt<46, TVPInitUnivTransBlendTable,
                                                 TVPUnivTransBlend,
    ...
    void Register_2() {
        // TVPxxxBlend, TVPxxxBlend_HDA, TVPxxxBlend_o,
        // TVPxxxBlend_HDA_o
#define REGISER_BLEND_4(THREAD_FACTOR, NAME)                                   \
    {                                                                          \
        static tTVPRenderMethod_BltAndOpa<THREAD_FACTOR, TVP##NAME##Blend_HDA, \
                                          TVP##NAME##Blend_HDA_o>              \
            method;                                                            \
        RegisterRenderMethod(#NAME "Blend", &method);                          \
    }
#define REGISER_BLEND_NON_HDA_4(THREAD_FACTOR, NAME)                           \
    {                                                                          \
        static tTVPRenderMethod_BltAndOpa<THREAD_FACTOR, TVP##NAME##Blend,     \
                                          TVP##NAME##Blend_o>                  \
            method;                                                            \
        RegisterRenderMethod(#NAME "Blend", &method);                          \
    }
#define REGISER_BLEND_WITH_HDA_4(THREAD_FACTOR, NAME)                          \
    {                                                                          \
        ...
        {
            static tTVPRenderMethod_BltAndOpa<52, TVPAlphaBlend_d,
                                              TVPAlphaBlend_do>
                method;
            RegisterRenderMethod("AlphaBlend_d", &method);
        }
        {
            static tTVPRenderMethod_BltAndOpa<52, TVPAlphaBlend_a,
                                              TVPAlphaBlend_ao>
                method;
    ...
    void Register_3() {
        ...
        {
            static tTVPRenderMethod_BltAndOpa<52, TVPAdditiveAlphaBlend_a,
    ...
    tTVPSoftwareRenderManager() :
        ...
        if(compTexMethod == "halfline")
            _createStaticTexture2D = tTVPSoftwareTexture2D_half::Create;
        else if(compTexMethod == "lz4")
        ...
        {
            static tTVPRenderMethod_ApplySelf<TVPConvertAlphaToAdditiveAlpha>
                method;
            RegisterRenderMethod("AlphaToAdditiveAlpha", &method);
        }
        {
            static tTVPRenderMethod_ApplySelf<TVPConvertAdditiveAlphaToAlpha>
                method;
        ...
        {
            static tTVPRenderMethod_DoBoxBlur<
                TDoBoxBlurLoop<tTVPARGB<tjs_uint16>>,
                TDoBoxBlurLoop<tTVPARGB<tjs_uint32>>>
                method;
            RegisterRenderMethod("BoxBlur", &method);
        }
        {
            static tTVPRenderMethod_DoBoxBlur<
                TDoBoxBlurLoop<tTVPARGB_AA<tjs_uint16>>,
                TDoBoxBlurLoop<tTVPARGB_AA<tjs_uint32>>>
                method;
...
#undef REGISER_BLEND_4
    ...
    CreateTexture2D(const void *pixel, int pitch, unsigned int w,
        ...
        if(pixel) { // create static pixel reference texture
            return /*_createStaticTexture2D*/ tTVPSoftwareTexture2D::Create(
                nullptr, pixel, pitch, w, h, format);
        } else {
    ...
    iTVPTexture2D *CreateTexture2D(tTVPBitmap *bmp) override {
        if(bmp->GetHeight() <= 16) {
            ...
            return _createStaticTexture2D(bmp, bmp->GetBits(), bmp->GetPitch(),
    ...
    bool GetTextureStat(iTVPTexture2D *texture, uint64_t &vmemsize) override {
        ...
        vmemsize =
            static_cast<iTVPSoftwareTexture2D *>(texture)->GetBitmapSize();
        return true;
    }
    ...
    void OperateRect(iTVPRenderMethod *method, iTVPTexture2D *tar,
                     tTVPRect rctar, iTVPTexture2D *src, tTVPRect rcsrc) {
        int sw = rcsrc.get_width(), sh = rcsrc.get_height(),
            dw = rctar.get_width(), dh = rctar.get_height();
        ...
        if(sw > 0 && sh > 0 && (sw != dw || sh != dh)) {
            ...
            if(cr.right < rctar.right) {
                rcsrc.right -= (float)rcsrc.get_width() / rctar.get_width() *
            ...
            if(cr.bottom < rctar.bottom) {
                rcsrc.bottom -= (float)rcsrc.get_height() / rctar.get_height() *
...
// #endif
            ...
            // assert(StretchType < sizeof(swsFlags) /
            // sizeof(swsFlags[0]));
            img_convert_ctx = sws_getCachedContext(
                img_convert_ctx, sw, sh, AV_PIX_FMT_RGBA, dw, dh,
                AV_PIX_FMT_RGBA, swsFlags[StretchType], nullptr, nullptr,
                nullptr);
    ...
    void OperateRect(iTVPRenderMethod *method, iTVPTexture2D *tar,
        ...
        if(check) {
            for(int i = 0; i < textures.size(); ++i) {
                ...
                unsigned int fmt = tex->GetFormat() == TVPTextureFormat::RGBA
                    ? CV_8UC4
                    : CV_8UC1;
            ...
            unsigned int fmt =
                tar->GetFormat() == TVPTextureFormat::RGBA ? CV_8UC4 : CV_8UC1;
            _tar = cv::Mat(tar->GetHeight(), tar->GetWidth(), fmt,
                           (void *)tar->GetPixelData(), tar->GetPitch());
            _tar.type();
        }
#endif
        ...
        for(int i = 0; i < textures.size(); ++i) {
            textures[i].first->GetScanLineForRead(
                0); // prepare pixel data for compressed texture
        }
        ...
        switch(textures.size()) {
            case 0: // fill tar
                ((tTVPRenderMethod_Software *)method)
                    ->DoRender(tar, rctar, nullptr, rctar, reftar, rctar,
                               nullptr, rctar);
                break;
            case 1: // src -> tar
                OperateRect(method, tar, rctar, textures[0].first,
                            textures[0].second);
                break;
            case 2: // src x dst -> tar
                ((tTVPRenderMethod_Software *)method)
                    ->DoRender(tar, rctar, textures[1].first,
                               textures[1].second, textures[0].first,
                               textures[0].second, nullptr, rctar);
                break;
            case 3: // src x dst x rule->tar
                ((tTVPRenderMethod_Software *)method)
                    ->DoRender(tar, rctar, textures[1].first,
                               textures[1].second, textures[0].first,
                               textures[0].second, textures[2].first,
                               textures[2].second);
                break;
        }
    }
    ...
    static bool CheckQuad(const tTVPPointD *pt) {
        if(pt[1].x == pt[3].x && pt[1].y == pt[3].y && pt[2].x == pt[4].x &&
           pt[2].y == pt[4].y)
            return true;
        return false;
    }
    static bool CheckTextureQuad(const tRenderTexQuadArray &textures) {
        for(int i = 0; i < textures.size(); ++i) {
            if(!CheckQuad(textures[i].second))
                return false;
        }
        return true;
    }
...
#define TVP_DoBilinearAffineLoop_ARGS                                          \
    ...
    typedef std::function<void(tjs_int, tjs_int, tjs_uint8 *, tjs_int, tjs_int,
                               const tjs_uint8 *, tjs_int, tjs_int, tjs_int,
                               const tTVPRect &, const tTVPRect &)>
        TAffuncFunc;

    std::unordered_map<iTVPRenderMethod *,
                       std::function<TAffuncFunc(tTVPRenderMethod_Software *)>>
        _stretchMethodFactory;
...
#define TVP_BILINEAR_FORCE_COND false
            _stretchMethodFactory[GetRenderMethod("Copy")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                if(TVP_BILINEAR_FORCE_COND ||
                   StretchType >= stFastLinear) { // bilinear interpolation
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                              tjs_int l, tjs_int len, const tjs_uint8 *src,
                              tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                    ...
                    // 				} else { // !hda
                    // 					affineloop = [](tjs_int sxs,
                    // tjs_int sys, tjs_uint8 *dest, tjs_int l,
                    // tjs_int len, const tjs_uint8 *src, tjs_int
                    // srcpitch, tjs_int sxl, tjs_int syl,
                    // const tTVPRect & srccliprect, const tTVPRect &
                    // srcrect){ TVPDoAffineLoop(
            ...
            _stretchMethodFactory[GetRenderMethod("ConstAlphaBlend")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa =
                    static_cast<
                        tTVPRenderMethod_BltWithOpa<59, TVPConstAlphaBlend> *>(
                        method)
                        ->opa;
                if(TVP_BILINEAR_FORCE_COND ||
                   StretchType >= stFastLinear) { // bilinear interpolation
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                                 tjs_int l, tjs_int len, const tjs_uint8 *src,
                                 tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                 const tTVPRect &srccliprect,
                                 const tTVPRect &srcrect) {
                        TVPDoBilinearAffineLoop(
                            ...
                            tTVPInterpLinTransConstAlphaBlendFunctionObject(
                                opa),
                            TVP_DoBilinearAffineLoop_ARGS);
                    };
                } else {
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                        ...
                        TVPDoAffineLoop(
                            tTVPStretchWithOpacityFunctionObject(
                                TVPStretchConstAlphaBlend_HDA, opa),
                            tTVPAffineWithOpacityFunctionObject(
                                TVPLinTransConstAlphaBlend_HDA, opa),
                            TVP_DoAffineLoop_ARGS);
                    };
                }
            };
            _stretchMethodFactory[GetRenderMethod("CopyOpaqueImage")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l,
                          tjs_int len, const tjs_uint8 *src, tjs_int srcpitch,
                          tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                          const tTVPRect &srcrect) {
                    TVPDoAffineLoop(
            ...
            _stretchMethodFactory[GetRenderMethod("ConstAlphaBlend_d")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa = static_cast<tTVPRenderMethod_BltWithOpa<
                    59, TVPConstAlphaBlend_d> *>(method)
                                  ->opa;
                return
                    [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l,
                          tjs_int len, const tjs_uint8 *src, tjs_int srcpitch,
                          tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                          const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(
                                            TVPStretchConstAlphaBlend_d, opa),
                                        tTVPAffineWithOpacityFunctionObject(
                                            TVPLinTransConstAlphaBlend_d, opa),
                                        TVP_DoAffineLoop_ARGS);
                    };
            };
            _stretchMethodFactory[GetRenderMethod("AlphaBlend")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa = static_cast<tTVPRenderMethod_BltAndOpa<
                    52, TVPAlphaBlend_HDA, TVPAlphaBlend_HDA_o> *>(method)
                                  ->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                              tjs_int l, tjs_int len, const tjs_uint8 *src,
                    ...
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                                 ...
                                 const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(
                                            TVPStretchAlphaBlend_HDA_o, opa),
                                        tTVPAffineWithOpacityFunctionObject(
                                            TVPLinTransAlphaBlend_HDA_o, opa),
                                        TVP_DoAffineLoop_ARGS);
                    };
                }
            };
            _stretchMethodFactory[GetRenderMethod("AlphaBlend_d")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa = static_cast<tTVPRenderMethod_BltAndOpa<
                    52, TVPAlphaBlend_d, TVPAlphaBlend_do> *>(method)
                                  ->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                              tjs_int l, tjs_int len, const tjs_uint8 *src,
                    ...
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                                 ...
                                 const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(
                                            TVPStretchAlphaBlend_do, opa),
                                        tTVPAffineWithOpacityFunctionObject(
                                            TVPLinTransAlphaBlend_do, opa),
                                        TVP_DoAffineLoop_ARGS);
                    };
                }
            };
            _stretchMethodFactory[GetRenderMethod("AdditiveAlphaBlend")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa = static_cast<tTVPRenderMethod_BltAndOpa<
                    52, TVPAdditiveAlphaBlend_HDA, TVPAdditiveAlphaBlend_HDA_o>
                                              *>(method)
                                  ->opa;
                if(opa == 255) {
                    if(TVP_BILINEAR_FORCE_COND || StretchType >= stFastLinear) {
                        ...
                        return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                                  ...
                                  const tTVPRect &srcrect) {
                            TVPDoAffineLoop(
                                tTVPStretchFunctionObject(
                                    TVPStretchAdditiveAlphaBlend_HDA),
                                tTVPAffineFunctionObject(
                                    TVPLinTransAdditiveAlphaBlend_HDA),
                                TVP_DoAffineLoop_ARGS);
                        };
                    }
                } else {
                    if(TVP_BILINEAR_FORCE_COND || StretchType >= stFastLinear) {
                        return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                                     tjs_int l, tjs_int len,
                                     const tjs_uint8 *src, tjs_int srcpitch,
                                     tjs_int sxl, tjs_int syl,
                                     const tTVPRect &srccliprect,
                                     const tTVPRect &srcrect) {
                            TVPDoBilinearAffineLoop(
                                tTVPInterpStretchAdditiveAlphaBlend_oFunctionObject(
                                    opa),
                                tTVPInterpLinTransAdditiveAlphaBlend_oFunctionObject(
                                    opa),
                                TVP_DoBilinearAffineLoop_ARGS);
                        };
                    } else {
                        return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                            ...
                            TVPDoAffineLoop(
                                tTVPStretchWithOpacityFunctionObject(
                                    TVPStretchAdditiveAlphaBlend_HDA_o, opa),
                                tTVPAffineWithOpacityFunctionObject(
                                    TVPLinTransAdditiveAlphaBlend_HDA_o, opa),
                                TVP_DoAffineLoop_ARGS);
                        };
                    }
                }
            };
            _stretchMethodFactory[GetRenderMethod("AdditiveAlphaBlend_a")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa = static_cast<tTVPRenderMethod_BltAndOpa<
                    52, TVPAdditiveAlphaBlend_a, TVPAdditiveAlphaBlend_ao> *>(
                                  method)
                                  ->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                    ...
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                        ...
                        TVPDoAffineLoop(
                            tTVPStretchWithOpacityFunctionObject(
                                TVPStretchAlphaBlend_ao, opa),
                            tTVPAffineWithOpacityFunctionObject(
                                TVPLinTransAdditiveAlphaBlend_ao, opa),
                            TVP_DoAffineLoop_ARGS);
                    };
                }
            };
            _stretchMethodFactory[GetRenderMethod("AlphaBlend_a")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa = static_cast<tTVPRenderMethod_BltAndOpa<
                    52, TVPAlphaBlend_a, TVPAlphaBlend_ao> *>(method)
                                  ->opa;
                if(opa == 255) {
                    return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                              tjs_int l, tjs_int len, const tjs_uint8 *src,
                    ...
                    return [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                        ...
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(
                                            TVPStretchAlphaBlend_ao, opa),
                                        tTVPAffineWithOpacityFunctionObject(
                                            TVPLinTransAlphaBlend_ao, opa),
                                        TVP_DoAffineLoop_ARGS);
                    };
                }
            };
            _stretchMethodFactory[GetRenderMethod("ConstAlphaBlend_a")] =
                [this](tTVPRenderMethod_Software *method) -> TAffuncFunc {
                tjs_int opa = static_cast<tTVPRenderMethod_BltWithOpa<
                    59, TVPConstAlphaBlend_a> *>(method)
                                  ->opa;
                return
                    [opa](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l,
                          tjs_int len, const tjs_uint8 *src, tjs_int srcpitch,
                          ...
                          const tTVPRect &srcrect) {
                        TVPDoAffineLoop(tTVPStretchWithOpacityFunctionObject(
                                            TVPStretchConstAlphaBlend_a, opa),
                                        tTVPAffineWithOpacityFunctionObject(
                                            TVPLinTransConstAlphaBlend_a, opa),
                                        TVP_DoAffineLoop_ARGS);
                    };
            };
        }
        auto it = _stretchMethodFactory.find(method);
        if(_stretchMethodFactory.end() != it)
            return it->second(method);
        return [](tjs_int sxs, tjs_int sys, tjs_uint8 *dest, tjs_int l,
                  tjs_int len, const tjs_uint8 *src, tjs_int srcpitch,
                  tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
                  const tTVPRect &srcrect) {
            TVPDoBilinearAffineLoop(tTVPInterpStretchCopyFunctionObject(),
                                    tTVPInterpLinTransCopyFunctionObject(),
                                    TVP_DoBilinearAffineLoop_ARGS);
        };
    }
    ...
    void OperateTriangles(iTVPRenderMethod *method, int nTriangles,
        ...
        if(nTriangles == 2 && checkQuad && checkTextureQuad) {
            ...
            bool isDstRect = isDoubleEqual(dstpt[0].y, dstpt[1].y) &&
            ...
            while(isSrcRect && isDstRect) {
                tTVPRect dstrect(dstpt[0].x, dstpt[0].y, dstpt[5].x,
                                 dstpt[5].y);
                tTVPRect refrect(srcpt[0].x, srcpt[0].y, srcpt[5].x,
                                 srcpt[5].y);
                ...
                if(rcdest.left > dstrect.left) {
                    refrect.left +=
                        (float)rw / dw * (rcdest.left - dstrect.left);
                }
                if(rcdest.right < dstrect.right) {
                    refrect.right -=
                        (float)rw / dw * (dstrect.right - rcdest.right);
                }
            ...
            if(taskNum > nTriangles)
                taskNum = nTriangles;
            TVPExecThreadTask(taskNum, [&](int n) {
                int begin = nTriangles * n / taskNum,
                    end = nTriangles * (n + 1) / taskNum;
                for(int i = begin; i < end; ++i) {
                    ...
                    InternalAffineBlt(rcclip, rc, rc, src, dst, dstpt + 3 * i,
                                      !nrot, affineloop);
                }
            });
        }
    ...
    public:
        tTVPBilinearStretchFunctionObject(tTVPBilinearStretchFunction func) :
            Func(func) {
            ;
        }
...
#define TVP_DEFINE_BILINEAR_STRETCH_WITH_OPACITY_FUNCTION(func, one)           \
    ...
    TVP_DEFINE_BILINEAR_STRETCH_FUNCTION(TVPInterpStretchCopy, *dest = color);

    TVP_DEFINE_BILINEAR_STRETCH_WITH_OPACITY_FUNCTION(
        TVPInterpStretchConstAlphaBlend,
        *dest = TVPBlendARGB(*dest, color, Opacity));

    TVP_DEFINE_BILINEAR_STRETCH_FUNCTION(TVPInterpStretchAdditiveAlphaBlend,
    ...
    public:
        tTVPBilinearAffineFunctionObject(tTVPBilinearAffineFunction func) :
            Func(func) {
            ;
        }
        void operator()(tjs_uint32 *dest, tjs_int len, const tjs_uint32 *src,
...
#define TVP_DEFINE_BILINEAR_AFFINE_WITH_OPACITY_FUNCTION(func, one)            \
    ...
    TVP_DEFINE_BILINEAR_AFFINE_FUNCTION(TVPInterpLinTransCopy, *dest = color);

    TVP_DEFINE_BILINEAR_AFFINE_WITH_OPACITY_FUNCTION(
        TVPInterpLinTransConstAlphaBlend,
        *dest = TVPBlendARGB(*dest, color, Opacity));

    TVP_DEFINE_BILINEAR_AFFINE_FUNCTION(TVPInterpLinTransAdditiveAlphaBlend,
                                        *dest = TVPAddAlphaBlend_n_a(*dest,
                                                                     color));
    ...
    static void TVPDoBilinearAffineLoop(
        tFuncStretch stretch, tFuncAffine affine, tjs_int sxs, tjs_int sys,
        tjs_uint8 *dest, tjs_int l, tjs_int len, const tjs_uint8 *src,
        tjs_int srcpitch, tjs_int sxl, tjs_int syl, const tTVPRect &srccliprect,
        const tTVPRect &srcrect) {
        ...
        while(len_remain > 0) {
            sx = spx >> 16;
            sy = spy >> 16;
            if(sx >= srcrect.left && sx < srcrect.right && sy >= srcrect.top &&
               sy < srcrect.bottom)
                break;
            dp--;
            spx -= sxs;
...
#define FIX_SX_SY                                                              \
    ...
    if(sy >= srccliprect.bottom)                                               \
        ...
        while(len_remain > 0) {
            ...
            affine.DoOnePixel(dp,
                              TVPBlendARGB(TVPBlendARGB(c00, c01, blend_x),
        ...
        if(len_remain > 0) {
            ...
            if(sys == 0) {
                ...
                stretch(dp, len_remain, (const tjs_uint32 *)l1,
                        (const tjs_uint32 *)l2, (spy & 0xffff) >> 8, spx, sxs);
            } else {
                // do affine
                affine(dp, len_remain, (tjs_uint32 *)src, spx, spy, sxs, sys,
                       srcpitch);
            }
        }
    }
    template <typename tFuncStretch, typename tFuncAffine>
    static void TVPDoAffineLoop(tFuncStretch stretch, tFuncAffine affine,
                                tjs_int sxs, tjs_int sys, tjs_uint8 *dest,
                                tjs_int l, tjs_int len, const tjs_uint8 *src,
                                tjs_int srcpitch, tjs_int sxl, tjs_int syl,
                                const tTVPRect &srcrect) {
        tjs_int len_remain = len;
    ...
    int InternalAffineBlt(/*CopyBitmapParam *param,*/ tTVPRect destrect,
                          tTVPRect refrect, tTVPRect srcrect,
                          iTVPTexture2D *_src, iTVPTexture2D *dst,
                          const tTVPPointD *points_in, bool rot,
        ...
        if(refrect.left >= refrect.right || refrect.top >= refrect.bottom)
            return 1;
        if(refrect.left < 0 || refrect.top < 0 || refrect.right > sw ||
           refrect.bottom > sh)
            TVPThrowExceptionMessage(TVPOutOfRectangle);

        // multiply source rectangle points by 65536 (16.16
        // fixed-point) note that each pixel has actually 1.0 x 1.0
        // size eg. a pixel at 0,0 may have (-0.5, -0.5) - (0.5, 0.5)
        // area
        ...
        for(tjs_int i = 0; i < 3; i++)
            points[i].x = (int)(points_in[i].x * 65536),
            points[i].y = (int)(points_in[i].y * 65536);
        ...
        if(destrect.top < 0)
            destrect.top = 0;
        if(destrect.right > dw)
            destrect.right = dw;
        if(destrect.bottom > dh)
            destrect.bottom = dh;

        if(destrect.left >= destrect.right || destrect.top >= destrect.bottom)
            return 1; // not drawable

        // vertex points
        tjs_int points_x[3];
        tjs_int points_y[3];

        // check each vertex and find
        // most-top/most-bottom/most-left/most-right points
        ...
        // - upper-right (p1)
        if(leftlimit > points_x[1])
            leftlimit = points_x[1];
        if(rightlimit < points_x[1])
            rightlimit = points_x[1];
        if(scanlinestart > points_y[1])
            scanlinestart = points_y[1];
        if(scanlineend < points_y[1])
            scanlineend = points_y[1];

        // - bottom-right (p2)
        if(leftlimit > points_x[2])
            leftlimit = points_x[2];
        if(rightlimit < points_x[2])
            rightlimit = points_x[2];
        if(scanlinestart > points_y[2])
            scanlinestart = points_y[2];
        if(scanlineend < points_y[2])
            scanlineend = points_y[2];

        // rough check destrect intersections
        if(floor_16(leftlimit) >= destrect.right)
            return 0;
        if(floor_16(rightlimit) < destrect.left)
            return 0;
        if(floor_16(scanlinestart) >= destrect.bottom)
            return 0;
        if(floor_16(scanlineend) < destrect.top)
            return 0;
        ...
        if(destrect.top > yc)
            yc = destrect.top;
        if(destrect.bottom <= yclim)
            yclim = destrect.bottom - 1;
        ...
        if(mode & stRefNoClip)
            srccliprect.left = 0, srccliprect.top = 0, srccliprect.right = sw,
            srccliprect.bottom =
                sh; // no clip; all the bitmap will be the source
        else
            srccliprect = srcrect; // clip; the source is limited to
        ...
        for(; yc <= yclim; yc += 65536, dest += destpitch) {
            ...
            if(code < 6) {
                for(code = 0; code < 3; code++) {
                    ...
                    tjs_int ip1 = (code + 1) % 3;
                    if(points_y[ip0] <= yl && points_y[ip1] > yl) {
                        where = div_16(yl - points_y[ip0],
                                       points_y[ip1] - points_y[ip0]);
                        break;
                    } else if(points_y[ip0] > yl && points_y[ip1] <= yl) {
                        where = div_16(points_y[ip0] - yl,
                                       points_y[ip0] - points_y[ip1]);
                        break;
                    }
                }
            }
            ...
            if(rot) {
                switch(line_code0) {
                    case 0:
                        ll = mul_16(points_x[1] - points_x[0], where0) +
                            points_x[0];
                        sxl = mul_16(refrect.right - refrect.left, where0) +
                            refrect.left;
                        syl = refrect.top;
                        break;
                    case 1:
                        ll = mul_16(points_x[2] - points_x[1], where0) +
                            points_x[1];
                        sxl = mul_16(refrect.left - refrect.right, where0) +
                            refrect.right;
                        syl = mul_16(refrect.bottom - refrect.top, where0) +
                            refrect.top;
                        break;
                    case 2:
                        ll = mul_16(points_x[0] - points_x[2], where0) +
                            points_x[2];
                        sxl = refrect.left;
                        syl = mul_16(refrect.top - refrect.bottom, where0) +
                            refrect.bottom;
                        break;
                }
                switch(line_code1) {
                    case 0:
                        rr = mul_16(points_x[1] - points_x[0], where1) +
                            points_x[0];
                        sxr = mul_16(refrect.right - refrect.left, where1) +
                            refrect.left;
                        syr = refrect.top;
                        break;
                    case 1:
                        rr = mul_16(points_x[2] - points_x[1], where1) +
                            points_x[1];
                        sxr = mul_16(refrect.left - refrect.right, where1) +
                            refrect.right;
                        syr = mul_16(refrect.bottom - refrect.top, where1) +
                            refrect.top;
                        break;
                    case 2:
                        rr = mul_16(points_x[0] - points_x[2], where1) +
                            points_x[2];
                        sxr = refrect.left;
                        syr = mul_16(refrect.top - refrect.bottom, where1) +
                            refrect.bottom;
                        break;
                }
            } else {
            ...
            // 0x8000000 were choosed to avoid special divition
            // behavior around zero
            l = ((tjs_uint32)(ll + 0x80000000ul - 1) / 65536) -
                (0x80000000ul / 65536) + 1;
            sxl += mul_16(65535 - ((tjs_uint32)(ll + 0x80000000ul - 1) % 65536),
                          sxs); // adjust source start point x
            syl += mul_16(65535 - ((tjs_uint32)(ll + 0x80000000ul - 1) % 65536),
                          sys); // adjust source start point y

            r = ((tjs_uint32)(rr + 0x80000000ul - 1) / 65536) -
                (0x80000000ul / 65536) +
                1; // note that at this point r is *NOT* inclusive
            ...
            // - transfer
            if(len > 0) {
                // fill left and right of the line if 'clear' is
                // specified 				if (clear)
                // 				{
                // 					tjs_int clen;
                //
                // 					int ll = l + 1;
                // 					if (ll > destrect.right)
                // ll
                // =
                // destrect.right; 					clen = ll
                // -
                // destrect.left; 					if (clen >
                // 0) (hda ? TVPFillColor :
                // TVPFillARGB)((tjs_uint32*)dest + destrect.left,
                // clen, clearcolor);
                // int rr = r - 1; 					if (rr <
                // destrect.left) rr =
                // destrect.left; 					clen =
                ...
                // - rr; 					if (clen > 0)
                // (hda ? TVPFillColor :
                // TVPFillARGB)((tjs_uint32*)dest + rr, clen,
                // clearcolor);
                // 				}
        ...
        // 				do
        // 				(hda ? TVPFillColor :
        // TVPFillARGB)((tjs_uint32*)p
        // +
        // destrect.left, 					destrect.right -
        // destrect.left, clearcolor), 					p +=
        // destpitch; 				while (--h);
        // 			}
        //
    ...
    void OperatePerspective(iTVPRenderMethod *method, int nQuads,
        ...
        for(int i = 0; i < nQuads; ++i) {
            ...
            if(isSrcRect)
                do {
                    tTVPRect dstrect(dstpt[0].x, dstpt[0].y, dstpt[2].x,
                                     dstpt[2].y);
                    tTVPRect refrect(srcpt[0].x, srcpt[0].y, srcpt[2].x,
                                     srcpt[2].y);
                    if(dstrect.left > dstrect.right ||
                       dstrect.top > dstrect.bottom ||
                    ...
                    tTVPRect rcdest;
                    if(TVPIntersectRect(&rcdest, cr, dstrect)) {
                        tjs_int dw = dstrect.get_width(),
                                dh = dstrect.get_height();
                        tjs_int rw = refrect.get_width(),
                                rh = refrect.get_height();
                        ...
                        if(rcdest.top > dstrect.top) {
                            refrect.top +=
                                (float)rh / dh * (rcdest.top - dstrect.top);
                        }
                        if(rcdest.bottom < dstrect.bottom) {
                            refrect.bottom -= (float)rh / dh *
                                (dstrect.bottom - rcdest.bottom);
                        }
                        OperateRect(method, target, rcdest, src, refrect);
                        processed = true;
                    }
            ...
            if(!processed) {
                ...
                cv::Point2f pts_dst[] = { cv::Point2f(dstpt[0].x - rcclip.left,
                                                      dstpt[0].y - rcclip.top),
                                          cv::Point2f(dstpt[1].x - rcclip.left,
                                                      dstpt[1].y - rcclip.top),
                                          cv::Point2f(dstpt[3].x - rcclip.left,
                                                      dstpt[3].y - rcclip.top),
                                          cv::Point2f(dstpt[2].x - rcclip.left,
                                                      dstpt[2].y -
                                                          rcclip.top) };
