...
void VideoPresentOverlay::ClearNode() {
    ...
    // Hosted sessions have no scene graph; the root stays null for the whole
    // session and every frame is dropped after the queue drain below.
#else
...
void VideoPresentOverlay::PresentPicture(float dt) {
    ...
    FrameMove();
#ifdef KRKR2_ASTRA_HOSTED
    // Hosted sessions drop decoded frames after the drain above; movie
    // timing and audio keep running without a scene graph.
    (void)pic;
#else
    if(!pic.rgba) {
        return;
...
#ifdef KRKR2_ASTRA_HOSTED
    // No scene graph and no per-frame presenter: the picture ring is left to
    // overwrite itself (AddPicture never blocks), audio and movie-length
    // semantics keep running, and decoded frames are dropped.
#else
...
#else
void VideoPresentOverlay2::SetRootNode(cocos2d::Node * /*node*/) {
    // No scene graph in hosted sessions; the node is ignored.
    ClearNode();
}
#endif
