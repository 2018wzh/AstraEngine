...
#include "../../environ/astra/astra_krkr_host.h"
#endif
...
// Astra hosted fill entry: the family plugin's audio worker pulls mixed PCM
// instead of an OS audio callback.
#if defined(KRKR2_ASTRA_HOSTED)
extern "C" void astra_krkr_hosted_fill_audio(int16_t *samples,
                                             uint32_t frame_count) {
    if(TVPAudioRenderer) {
        TVPAudioRenderer->FillBuffer(reinterpret_cast<uint8_t *>(samples),
                                      int(frame_count) * ASTRA_KRKR_CHANNELS *
                                          int(sizeof(int16_t)));
    } else {
        memset(samples, 0,
               size_t(frame_count) * ASTRA_KRKR_CHANNELS * sizeof(int16_t));
    }
}
#endif
...
#if defined(KRKR2_ASTRA_HOSTED)
    class tTVPAudioRendererHosted : public iTVPAudioRenderer {
    public:
        bool Init() override {
            InitMixer();
            _frame_size = ASTRA_KRKR_CHANNELS * int(sizeof(int16_t));
            SetupMixer();
            return true;
        }
    };
    renderer = new tTVPAudioRendererHosted;
    if(renderer->Init())
        return renderer;
    delete renderer;
    return nullptr;
