...
namespace {
    ...
    inline void mirrorForceVisibleGeometry_guess(
        ...
        tjs_int32 coordIndex = 0;
        (void)coord.SetValue(coordIndex, coordX, TJS_MEMBERENSURE);
        ...
        tjs_int32 matrixIndex = 0;
        (void)matrix.SetValue(matrixIndex, m11, TJS_MEMBERENSURE);
    ...
    inline void applyGroundCorrection_guess(
        ...
        // Non-constant index lvalues keep MSVC from reading the converted
        // zero as a null-pointer constant for the key-based overload.
        tjs_int32 axisIndex = 0;
        node.accumulated.posX = corrected.getRealValue(axisIndex++, 0.0);
        node.accumulated.posY = corrected.getRealValue(axisIndex++, 0.0);
        node.accumulated.posZ = corrected.getRealValue(axisIndex, 0.0);
    }
} // anonymous namespace
