// The hosted provider vendors an upstream MPL-2.0 tree. Keep its warning
// policy independent from the AstraEngine workspace lint gate; provider and
// adapter code are linted by the owning crate.
#![allow(warnings)]
