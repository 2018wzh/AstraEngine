...
impl BgmPlayer {
    ...
    /// Reconciles the logical player bit with the host mixer after a voice
    /// naturally reaches EOF.  The hosted core owns the query and only passes
    /// a bounded, already validated slot.
    pub(crate) fn set_playing_from_host(&mut self, slot: usize, playing: bool) {
        if slot < BGM_SLOT_COUNT {
            self.playing[slot] = playing;
        }
    }
