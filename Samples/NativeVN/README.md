...
Status: redistributable playable demo with automated scripted QA and a manual QA checklist.
...
build\Bin\astra.exe test build/Saved/Packages/NativeVN.astrapkg --plan Samples/NativeVN/Tests/player/nativevn_player.yaml --backend sdl --auto-close --json
...
`AstraGame` QA is the main automated QA entry. Player plans declare `case_id`, `persona`, `objective`, `steps`, and JSON Pointer `assertions`; failed assertions or malformed runtime events fail the command.

The older scripted files remain valid for lower-level QA runs. Each scripted file may declare `case_id`, `persona`, `objective`, and `expects`; `AstraGame` launcher writes the result to `playable_vn.player_qa`.
