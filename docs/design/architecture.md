...
flowchart TD
    ...
    ExternalAgent["External Agent / MCP Host"] --> MCP["MCP Integration"]
    MCP --> Editor
    ...
    RuntimeAI["Optional Runtime AI"] --> RuntimeServices
```
...
外部 Agent 通过 MCP Integration 访问 Editor/Developer 工具链。MCP 读取 Text-First 源数据、调用验证和构建工具，并在 trusted session 中直接写入 workspace/project 内文本源文件。

动态模块通过 ModuleManager 加载，通过 ExtensionRegistry 注册 RuntimeCommand source、Runtime Services 扩展、资产管线、Editor UI、MCP tool、AI Provider 和兼容模块能力。插件不直接访问 Runtime ECS、Renderer2D、AudioCore 或 Editor 内部对象。
...
Level 3C: Optional Runtime AI
运行时闲聊、反应、旁白变体、受约束生成

Level 4: Editor / Tools
Content Browser、Scene Editor、Story Graph、Agent Workbench、Compatibility Inspector、MCP Integration
```
...
Editor -> Runtime
Game -> Runtime
AI Plugins -> AIRuntime / Runtime Services
MCP Integration -> Editor / Developer Tools
Runtime 不依赖 Editor
...
Runtime Services 是 Astra Runtime、Runtime AI、Editor Preview、Headless Test 和动态兼容模块共享的服务层。
...
- Astra DSL、Story Graph、可选 AI hook 和兼容模块的脚本/timeline 适配器可以生成 RuntimeCommand。
...
CommandBuffer
RuntimeCommand、Astra Runtime、兼容模块和受约束 Runtime AI 写入 World 的延迟变更队列。
```
...
Astra DSL、Story Graph、Astra Runtime 内的 Agent hook 和兼容模块的 runtime adapter 可以生成 RuntimeCommand。RuntimeCommand 是进入 Runtime Services 的稳定意图协议和可选日志格式。
...
RuntimeCommand 是 Astra Runtime 内部意图与 Runtime Services 之间的稳定协议。兼容模块可以在运行时读取外部脚本或 timeline 并生成瞬时 RuntimeCommand，但不把外部项目转换为 Astra canonical source。
...
- `permissions`：文件、网络、MCP、AI、runtime packaging 等权限。
- `platforms`：支持的平台和架构。
...
- MCP resource、tool 和 prompt provider。
...
- MCP/Developer 模块默认不进入 packaged runtime。
...
MCP Integration 是 Editor/Developer 插件能力，用于让外部 Agent 访问 AstraEngine 工具链。
...
- MCP server 默认禁用。
- 用户显式启动 trusted session。
- Packaged runtime 默认不包含 MCP server。
- 受信 MCP session 可直接写 workspace/project 内文本源文件。
- 每个 mutating tool 必须记录 Operation Log。
- MCP 不暴露 EnTT/ECS 内部类型、明文密钥、未授权外部路径或 editor UI 对象。

MCP 与 AI Provider 的区别：

- AI Provider 负责生成内容。
- MCP 负责暴露项目上下文和工具调用。
- 当外部 AI Agent 通过 MCP 写入内容时，MCP 记录 Operation Log；AI 相关内容仍可进入 AI Audit Log。
...
- MCP trusted direct write 的 Operation Log 是否足以支持审计和回滚辅助。
- ModuleManager、AstraModule C ABI、ExtensionRegistry 和 PluginDescriptor schema 是否能覆盖 Runtime、Editor、MCP、Cook、AI Provider 和 Compatibility Module 扩展。
- VN Property System 是否足以支撑属性面板、schema 生成、MCP 字段编辑、序列化和插件配置，而不需要完整 UObject 体系。
- Runtime AI 的确定性回放是否需要保存完整输出，还是保存 prompt hash + output hash 即可。
