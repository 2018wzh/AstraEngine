...
- AI 协作式创作与运行时受控内容生成。
- 独立 `AstraEmu Toolkit` 形式的旧 VN 引擎模拟、兼容运行与现代化表现。

非目标：

- 不以复杂 3D 动作、FPS、高实时网络竞技或大型开放世界为主目标。
- 不引入完整 UE `UObject`、UHT、GC 或跨 ABI C++ Actor 继承体系。
- 不让 AI、Live2D、Lua 或旧 VM 污染 Core；Lua 可以作为 Script/AstraVN 能力，但不进入 Core 依赖。
...
- EngineCore 最终完成必须在 Windows、Linux、macOS、iOS、Android 和 Web 上支撑 NativeVN 与 TsuiNoSora 完整游玩。
...
- Lua AstraVN source schema：Lua-authored VN/system script state、source map、debug symbol、hot reload policy、save/replay snapshot。
...
- AstraEmu local case report：commercial case title、engine family、local root hash、package patch hash、coverage、commands、diagnostics。
...
- `legacy-compatibility-contract.md` 固化 `AstraEmu Toolkit` 边界、package patch script、本地商业 case report 和 Artemis/KrKr/BGI family gate；它基于 AstraEngine 架构实现，但不进入 NativeVN 创作流程。
...
AstraGame   -> Core, Platform, Module, Asset, Media, Runtime, Script, AstraVN
...
AstraEmu    -> Core, Platform, Module, Asset, Media, Runtime, Script, AstraVN (standalone toolkit)
```

禁止依赖：

- Core 不依赖 Platform、Runtime、VN、AI、MCP、Editor、Compat、SDL、Lua、renderer、audio。
- Lua 归属 Script/AstraVN provider/runtime，不允许成为 Core dependency。
- Runtime 不依赖 Editor UI、Editor widget、MCP server implementation 或 AI provider implementation。
- AstraGame 是显式恢复的新 production launch/session 边界；本次用户授权覆盖历史 `do not revive AstraGame` 约束，但仍禁止恢复 `VNRuntimeServices` 和 `Bootstrap`。
...
- AstraEmu 不得作为 NativeVN runtime parity 或 Editor 创作流程的前置依赖。
...
- `script-and-presentation.md`：ScriptRuntimeHost、Native DSL、Lua-as-AstraVN-source、Graph/Timeline、PresentationCommand、AstraVN。
...
`AstraEmu Toolkit` 是独立扩展交付物，不是 `AstraEngine` 创作层的一部分：
...
├─ API Mapper
├─ TextCapture / Translation Bridge
├─ Enhancement Profile
└─ Runtime Inspector
```

它复用稳定的 Runtime、Asset、Media、Script 和 AstraVN visible semantics，但不作为 native runtime
达到 production parity 的前置条件，也不能反向污染 Core 或 NativeVN 制作流程。
...
脚本运行时只能通过 Script API、RuntimeEvent 和 Presentation API 影响世界。旧 VM 不进入 AstraEngine authoring；它由独立 AstraEmu Compat Core 保存 VM 状态，并通过 API Mapper 输出事件。
...
旧游戏现代化在 AstraEmu Toolkit 中尽量 layer-aware：背景、角色、UI、文本、最终画面分别处理。文本优先重新排版，不做截图式超分。
...
- AstraEmu toolkit module：Compat Core、content probe、TextCapture、translation bridge、enhancement profile。
...
flowchart TB
  subgraph Input["Input Runtimes"]
    ...
    Artemis["AstraEmu Artemis Core\nsystem.ini / IET / ASB / AST / Lua"]
    KrKr["AstraEmu KrKr Core\nKAG / TJS / XP3"]
    BGI["AstraEmu BGI Core\nscenario / system VM / archives"]
  end
...
AstraVN 原生项目和 AstraEmu Artemis/KrKr/BGI cores 共享 Runtime、Presentation、Media 和 AstraVN visible
semantics；输入脚本语言、legacy VM 控制流、family-specific host API 和外部资产 resolver
保持在 AstraEmu Compat Core 内。AstraEmu 必须在稳定 native runtime 之上实现，不能作为
Core、Runtime、Asset、Media 或 NativeVN 制作流程的早期达标前置项。

Artemis、KrKr、BGI core 的真实案例、v1 路线、package patch policy 和 `foreign-*:/` mount policy 在
`compatibility-layer.md` 中定义；AstraVN 与 AstraEmu runtime 的共享/不共享边界在
...
14. Expansion deliverable：AstraEmu Toolkit。
