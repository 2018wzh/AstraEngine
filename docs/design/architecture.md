...
- `legacy-compatibility-contract.md` 固化 `AstraEmu Toolkit` 边界；它基于 AstraEngine 架构实现，但不进入 NativeVN 创作流程。
...
AstraVN 原生项目和 AstraEmu Artemis Core 共享 Runtime、Presentation、Media 和 AstraVN visible
semantics；输入脚本语言、Artemis VM 控制流、`e:*` 宿主 API 和外部资产 resolver
保持在 AstraEmu Compat Core 内。AstraEmu 必须在稳定 native runtime 之上实现，不能作为
Core、Runtime、Asset、Media 或 NativeVN 制作流程的早期达标前置项。

Artemis core 的真实案例、v1 路线和 `foreign-artemis:/` policy 在
