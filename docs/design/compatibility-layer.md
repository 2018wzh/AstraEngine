...
- 通过用户提供的 package patch script 描述原始发行数据的 reader、index、decode、offset/table、resource map 和 diagnostic rule。
...
- 不服务 Editor 创作工作流。
- 不修改 foreign source。
- 不破解、解密或绕过 DRM / 商业保护；受保护内容只能输出 blocking diagnostic。
...
- 受保护或加密数据只报告 unsupported / protected diagnostic。
...
- Patch script 是用户提供的结构描述和适配层，不是内置单作硬编码。
- Patch script 可以描述 reader、index、decode、offset/table、resource map、filename normalization 和 diagnostic rule。
- Patch script 必须 sandbox，必须记录 hash，必须进入本地验收报告。
- Patch script 不能绕过 DRM、破解商业保护或移除访问控制；需要这类行为的 payload 只能报告 blocking diagnostic。
- Patch script 输出通过 read-only `foreign-*:/` VFS 或 PackageReader-style adapter 暴露给 Compat Core。

可提交本地验收报告使用：

```yaml
schema: astra.emu.local_case_report.v1
case_title: Sakura no Uta 10th Anniversary Edition
engine_family: artemis
edition: original_release_local
local_root_hash: sha256:...
core_id: astra.emu.artemis.core
package_patch_set:
  id: local.artemis.sakura_no_uta_10th
  hash: sha256:...
commands:
  probe: astraemu probe --case local-artemis-sakura-no-uta-10th --json
  run: astraemu run --case local-artemis-sakura-no-uta-10th --report
coverage:
  required_total: 0
  required_covered: 0
  uncovered_required: []
...
报告可以提交到仓库或审计包；商业源文件、解包 payload、私有绝对路径和未授权截图不能提交。

## 5. Family v1 Targets

首批真实目标不是 mock core，而是三类引擎 family 使用原始发行数据的本地完整可玩验收：
...
每个 family 的完成门槛是 100% full-content-flow coverage：

- 主线、分支、结局和选择。
- 全文本、ruby/control tag、voice、BGM、SE。
- CG、背景、角色 sprite、转场、movie。
- 系统菜单、config、save/load、backlog、replay。
- 所有 required tag/API/asset 引用要么 covered，要么以 blocking diagnostic 阻止验收。

### 5.1 Artemis v1

Artemis v1 面向原始发行数据和用户提供的 package patch set。安装态布局识别：
...
- Probe：识别 `system.ini`、`system/first.iet`、`system/*.asb`、`script/*.ast`、package candidates 和 media roots。
- Package patch：支持 `.pfs`、`.pfs.000`、`.pfs.721`、movie data、字体、脚本、index table、resource map。
- Index：索引 `.iet`、`.asb`、`.ast`、`.ipt`、`.sli`、`.tbl`。
- Lua host：提供 Artemis `e` 对象的最小 API surface。
- Tag coverage：覆盖 `bg`、`fg`、`text`、`msg`、`vo`、`se`、`bgm`、`select`、`excall`、`wait`、`extrans`、`msgoff`、`quake`、`ruby`、`eval`、`movie` 以及目标作品 required tag/API。
...
### 5.2 KrKr / KAG / TJS / XP3 v1

KrKr v1 面向 `Senren Banka` 原始发行数据：

- Probe：识别 `.xp3` package candidates、KAG scenario、TJS/system script、plugin、media root、font、save/config 数据。
- Package patch：描述 XP3 index/resource access、payload mapping、filename normalization 和 protected-content diagnostic。
- KAG executor：覆盖 text、label、jump、call、macro、choice、variable、layer、background、character sprite、transition、wait、voice、BGM、SE、movie。
- TJS host：提供目标作品 required system/UI/save/config/plugin call 的最小安全 adapter。
- Coverage report：统计 KAG tag、TJS API、asset reference、route/endings、system menu、backlog 和 save/load 覆盖。

### 5.3 BGI / Ethornell v1

BGI v1 面向 `Subarashiki Hibi 15th Anniversary Edition` 原始发行数据：

- Probe：识别 BGI/Ethornell archive candidates、scenario script、system script、media root、font、save/config 数据。
- Package patch：描述 archive index/resource access、payload mapping、filename normalization 和 protected-content diagnostic。
- Script model：保留 scenario story layer 与 system script / VM layer 分离，不把 BGI control flow 写入 AstraVN source language。
- Executor：覆盖 text、choice、variable、route flow、system menu、backlog、config、background/CG、character sprite、transition、voice、BGM、SE、movie、save/load 和 endings。
- Coverage report：统计 scenario/system call、asset reference、route/endings、system UI、save/load/replay 覆盖。
...
- `AstraEmu` 可扫描本地原始发行数据并选择 Artemis、KrKr 或 BGI Compat Core。
- `Sakura no Uta 10th Anniversary Edition`、`Senren Banka`、`Subarashiki Hibi 15th Anniversary Edition` 均有 `astra.emu.local_case_report.v1`。
- 三个本地 case 的 full-content-flow coverage 均为 100%，未覆盖 required item 为 0。
- Package patch script 可处理目标 case 所需 reader/index/decode/resource map，但不绕过 DRM 或商业保护。
