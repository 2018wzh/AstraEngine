...
| Real media backend | Phase 7 provider/decode/timeline/filter evidence plus SDL/headless RGBA image, bgfx/Skia provider smoke, HarfBuzz/FreeType glyph primitive present evidence from package payloads, CPU RGBA FilterGraph execution, and DriverDiffReport from runtime release-gate evidence; native audio perceptual diff and Editor visual diff viewers remain broader hardening targets |
