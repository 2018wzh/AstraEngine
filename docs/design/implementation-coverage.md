...
| Script / Presentation | `script-and-presentation.md` | ScriptRuntimeHost, Script API, DSL IR, Lua-as-AstraVN-source, PresentationCommand, AstraVN | TODO 12, 13 | ScriptParity, NativeVN |
...
| AstraEmu Toolkit | `compatibility-layer.md`, `legacy-compatibility-contract.md` | AstraEmuManager, CompatRuntimeProvider, Content Probe, package patch script, local case report, TextCapture, save-state | TODO 20 | AstraEmuToolkit |
...
| `legacy-compatibility-contract.md` | `AstraEmuManager`, `ICompatRuntimeProvider`, `CompatPackagePatchResult`, `CompatCoverageReport`, `LegacyVmSnapshot`, `TextCaptureEvent`, `astra.emu.local_case_report.v1` | AstraEmuToolkit |
...
| TsuiNoSora modern port | `Samples/TsuiNoSora/Tools` local converter, user-supplied source root, untracked generated AstraVN `Content`, coverage report, all-platform package/full-play evidence after conversion |
...
| Script debug and snapshot | ScriptParity debugger, Lua-as-AstraVN-source parity and save/replay reports |
...
| AstraEmu remains standalone | AstraEmuToolkit does not participate in NativeVN creation workflow; package patch scripts and local commercial reports remain toolkit-only |
