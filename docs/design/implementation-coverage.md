...
| Real media backend | Phase 7 provider/decode/timeline/filter evidence plus SDL/headless RGBA image, optional bgfx/Skia provider smoke, and HarfBuzz/FreeType glyph primitive present evidence from package payloads in NativeVN/TsuiNoSora reports; true platform surface binding and per-driver visual/audio diff remain the broader hardening target |
