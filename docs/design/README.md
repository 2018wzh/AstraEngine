...
`README` -> `goals` -> `architecture` -> `implementation-coverage` -> `foundation-core-platform-property` -> `runtime-core` -> `actor-component-ecs-hybrid` -> `asset-pipeline` -> `media-runtime` -> `script-and-presentation` -> `extension-and-module-system` -> `content-and-assets` -> `editor-and-pipeline` -> `editor-ui-ai-collaboration-prototype` -> `ai-collaboration` -> `mcp-integration` -> `tools-release-observability` -> `samples-and-test-matrix` -> `roadmap` -> `compatibility-layer`。
