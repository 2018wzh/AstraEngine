...
定位：高度可定制化、模块化的 2D 引擎。视觉小说是第一垂直模块，同时支持传统 VN、AI 协作和运行时受控 AI 内容。旧 VN 引擎模拟器和旧作现代化收敛为独立 `AstraEmu Toolkit`，是 native runtime production parity 之后的扩展交付物，不进入 NativeVN 创作流程。

## 1. 核心定位

AstraEngine 不是 AAA 通用 3D 引擎，也不是单一 VN 播放器。它的目标是提供一套轻量但完整的 2D 叙事与演出引擎基础。EngineCore 的最终玩家 runtime 目标覆盖 Windows、Linux、macOS、iOS、Android 和 Web，并以 NativeVN 与 TsuiNoSora 完整游玩报告作为完成证据：
...
- ScriptRuntimeHost 目标上支持 Astra Native Script、Lua-as-AstraVN-source 和自定义脚本运行时；旧 VN VM 由独立 `AstraEmu Toolkit` 的 Compat Core 承载。
- FilterGraph 是风格化演出、后处理和旧游戏现代化的统一管线。
- AI 只输出结构化 Intent，由 Validator、ControlPolicy 和 Director 仲裁后转为事件。
- Legacy VN 支持以独立 `AstraEmu Toolkit` 实现，不要求导入为 Astra 源项目；它复用稳定 runtime/media/module 边界，通过用户提供的 package patch script 适配原始发行数据，不绕过 DRM 或商业保护，也不参与 NativeVN 制作流程。
...
| [player-automation-testing.md](player-automation-testing.md) | `AstraGame` QA、player test plan、RuntimeEvent 注入、JSON Pointer assertions 和 NativeVN QA evidence |
| [samples-and-test-matrix.md](samples-and-test-matrix.md) | NativeVN、RuntimeStress、PackageLaunch、ScriptParity、MediaBackend、AIIntentSafety、CreatorWorkflow、CustomizationPlugin、AstraEmuToolkit |
| [implementation-coverage.md](implementation-coverage.md) | 设计覆盖矩阵，映射系统、公共契约、TODO、验收证据和非目标 |
| [enginecore-production-checklist.md](enginecore-production-checklist.md) | EngineCore/runtime/tooling 目标达成 checklist，记录全平台 NativeVN/TsuiNoSora 完整游玩 gate、Lua-as-AstraVN-source、代码证据和缺口 |
| [editor-production-checklist.md](editor-production-checklist.md) | Editor/Creator workflow 目标达成 checklist，记录真实 Qt Editor、PIE/debug/package/review/AI 工作流和当前缺口 |
| [astraemu-production-checklist.md](astraemu-production-checklist.md) | AstraEmu standalone expansion checklist，记录 package patch、本地商业 case report、Artemis/KrKr/BGI 100% coverage 和边界状态 |
| [compatibility-layer.md](compatibility-layer.md) | `AstraEmu Toolkit` 独立运行器、Compat Core、TextCapture、翻译和现代化 overlay |
...
| [legacy-compatibility-contract.md](legacy-compatibility-contract.md) | AstraEmuManager、Compat Core、Content Probe、TextCapture 和 save-state 边界 |
...
- Runtime 不依赖 Editor。
- EngineCore 最终 runtime 完成必须有 Windows、Linux、macOS、iOS、Android、Web 上 NativeVN 与 TsuiNoSora 完整游玩证据。
