...
| [ai-collaboration.md](ai-collaboration.md) | AI Agent 协作体系、Boundary Manager、Review Queue、Diff/Patch、Provenance、运行时 AI 约束 |
...
| [mcp-integration.md](mcp-integration.md) | MCP server、resources、tools、prompts、trusted session、operation log 和完整工具链接口 |
...
| [../adr](../adr) | 架构决策记录，包括动态模块优先、编辑器 UI、Renderer2D 后端、Runtime ECS、MCP Integration、Text-First Source Data 和兼容扩展策略 |
...
系统以 Astra Runtime 和稳定 Runtime Services 为中心。Astra DSL、Story Graph、Compatibility Modules、Runtime AI、Editor Preview、Headless Test 都通过同一套 Stage、Dialogue、Choice、Audio、Asset、Input、Save、Localization 服务运行。Runtime Services 内部采用 Bevy 风格 ECS World 组织运行时状态和系统调度，但对外仍暴露稳定服务 facade 和 Astra RuntimeCommand 协议。
...
AstraEngine 只专注视觉小说，但在 VN 领域内追求 UE 级可定制化。AI Provider、TTS Provider、Live2D、Spine、外部引擎兼容模块、资产验证器、Cook Processor、Editor 面板、MCP 工具和 Story Graph 节点都应通过动态模块扩展。源码级模块只用于引擎核心、实验性底层能力或尚未稳定 ABI 的内部代码。
...
MCP 是 Editor/Developer 插件能力，不进入默认 packaged runtime。受信 MCP session 可以直接写入 workspace/project 内文本源文件，但必须记录 Operation Log；MCP 不暴露 EnTT/ECS 内部状态、明文密钥或未授权外部路径。
...
- MCP Integration 的资源、工具和 Operation Log 设计。
...
- Runtime 不依赖 Editor。
- AI 不直接修改 Canonical Project，必须通过 Patch 或 Review Queue。
- Cooked Build 默认确定性，可复现。
- Runtime AI 默认关闭，只有项目策略显式开启时才可打入发布包。
...
- MCP trusted direct write 只允许写 workspace/project 内文本源文件，并必须记录 Operation Log。
- 动态模块必须通过 ModuleManager、ExtensionRegistry 和权限声明接入，不得绕过 Runtime Services、Boundary Manager 或 Release Gate。
- Packaged runtime 只包含启用且 runtime-safe 的动态模块；Editor、Developer、MCP 调试模块默认不进入发布包。
...
- AI Provider 接口是否进入 Runtime Core，还是只存在于 AI 插件层。
- MCP 是否需要 packaged runtime 支持。
