...
- ScriptRuntimeHost 目标上支持 Astra Native Script、Lua 和自定义脚本运行时；旧 VN VM 由独立 `AstraEmu Toolkit` 的 Compat Core 承载。
- FilterGraph 是风格化演出、后处理和旧游戏现代化的统一管线。
- AI 只输出结构化 Intent，由 Validator、ControlPolicy 和 Director 仲裁后转为事件。
- Legacy VN 支持以独立 `AstraEmu Toolkit` 实现，不要求反编译或导入为 Astra 源项目；它复用稳定 runtime/media/module 边界，但不参与 NativeVN 制作流程。
...
| [enginecore-production-checklist.md](enginecore-production-checklist.md) | EngineCore/runtime/tooling 生产完备度 checklist，逐项记录设计、代码证据、验证命令和缺口 |
| [editor-production-checklist.md](editor-production-checklist.md) | Editor/Creator workflow 生产完备度 checklist，记录当前未实现项和可复用 runtime 前置条件 |
| [astraemu-production-checklist.md](astraemu-production-checklist.md) | AstraEmu standalone expansion checklist，记录 Compat Core、probe、TextCapture、translation、Artemis 和边界状态 |
