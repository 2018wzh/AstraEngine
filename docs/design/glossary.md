...
独立的审计模块，负责统一记录 MCP tool side effect 和内容生成来源。它通常拆分为 Operation Log 与 Generation Audit Log 两类事件流，但对外提供统一查询和发布前统计入口。
...
Runtime ECS 中用于延迟写入 World 的命令队列。RuntimeCommand、Astra Runtime、兼容模块和 Runtime Generation Orchestrator 应通过 CommandBuffer 或服务 facade 进入 ECS，而不是直接暴露 EnTT registry。
...
AstraEngine 默认项目级扩展形态。动态模块由 ModuleManager 加载，通过 AstraModule C ABI 和 ExtensionRegistry 注册能力，可用于 Runtime、Editor、Developer、MCP、Runtime Generation、Agent Audit、Asset Pipeline、AI Provider 和 Compatibility 扩展。
...
动态模块注册扩展点的中心注册表。它接收 RuntimeCommand source、CompatibilityAdapter、VFS mount provider、ForeignAssetResolver、Service extension、ECS system pack、脚本函数、Story Graph 节点、AssetValidator、CookProcessor、Editor panel、MCP host/provider、Runtime Generation Orchestrator、Agent Audit sink 和 AI provider 等注册。
...
Model Context Protocol。AstraEngine 将 MCP 重定义为统一 Agent 能力协议层，用于向开发阶段和运行时阶段的 Agent 暴露 resources、tools、prompts、sessions 和权限边界。它通过 Editor MCP Host 和 Runtime MCP Host 分别承载开发阶段协作与运行时受控生成。
...
运行时侧 MCP host。它只暴露 runtime-safe resources、tools 和 prompts，服务运行时受控内容生成，不允许 project source direct write，也不能获得 Editor trusted session 语义。
...
MCP mutating tool 的追加式操作日志。记录 session、actor、tool、输入摘要、受影响路径、校验结果和输出摘要，用于审计、回滚定位和 Release Gate 诊断。统一 MCP 化后，它由 Agent Audit 模块管理，并与 Generation Audit Log 分开。
...
发布前质量门禁。用于阻止未审核 AI 内容、缺失资产、脚本错误、本地化缺失、Runtime MCP / Generation 策略冲突等问题进入发布包。
...
旧术语。统一重构后更准确的表述是 Runtime MCP Host + Runtime Generation Orchestrator + Provider Modules。默认关闭，只能在项目策略和发布模式允许时启用。

### Runtime Generation Orchestrator

运行时生成编排模块。它消费 Runtime MCP session，调用 Context Builder、Boundary Manager、Provider Modules 和 fallback 逻辑，把最终结果转成 RuntimeCommand，并向 Agent Audit 报告来源信息。

### RuntimeCommand

Astra DSL、Story Graph、Astra Runtime 内部 Agent hook、Runtime Generation Orchestrator 和 compatibility adapter 到 Runtime Services 的命令协议。

### RuntimeCommand Source

可在运行时产生 RuntimeCommand 的扩展点。Astra DSL、Story Graph、Runtime Generation Orchestrator 和兼容模块都可以作为 RuntimeCommand source。
...
稳定运行时服务层，包括 Stage、Dialogue、Choice、Audio、Asset、Input、Save、Localization。Astra Runtime、兼容模块、Runtime MCP Host、Runtime Generation Orchestrator、Editor Preview 和 Headless Test 共享该层。内部使用 Runtime ECS World 组织状态和系统调度。
