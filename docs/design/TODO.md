...
NativeVN Phase 8 full playable demo 已实现 `astra cook/package/run --backend headless/run --backend sdl/replay --compare/inspect`
...
Runtime-only release gate、trace/crash evidence、sectioned save descriptors、Runtime tick result DTO、Director arbitration DTO 和 Media driver diff evidence 已补齐；Editor、AI/MCP、Legacy、Lua-as-AstraVN-source、完整全平台 NativeVN/TsuiNoSora 游玩、AstraEmu Artemis/KrKr/BGI compatibility 和 Editor-facing debug/visual diff tools 仍按下列 TODO 阶段推进，不能因 runtime-only evidence slice 被声明为 full engine/editor complete。
...
Phase 1/6/7 注记：当前已建立 dynamic-only `Astra*` runtime/tool DLL targets、示例插件、Phase 1 tests、构建选项边界、`AstraTools` 和 `astra` runtime evidence CLI。`validate`、`import`、`cook`、`package`、`run --backend headless`、`replay --compare`、`inspect`、`doc-check` 已覆盖 Phase 0/1 文档、Foundation validation、`foundation_core_gate`、engine/plugin binary hash evidence、Phase 7 media provider/decode/timeline/filter evidence 和 NativeVN binary `.astrapkg` package/replay evidence；full runtime release gate、trace/crash bundle 和 per-driver visual/audio diff 仍归后续 Tools/Samples 阶段。

- [x] Build：定义新顶层目标命名规则：`Astra<Module>`、`Astra<Tool>`、`Astra<Sample>`、`Astra<TestSuite>`。
...
- [x] Build：建立 CLI 工具入口：`astra`，包含 validate、cook、package、run、replay、inspect、doc-check。（当前覆盖 foundation/runtime evidence workflow；production release tooling 后续完成。）
- [x] Build：建立 sample project 目录规范：`Samples/NativeVN`, `Samples/RuntimeStress`, `Samples/PackageLaunch`。
- [x] Test：建立测试分类和命名：unit、integration、headless、validation、stress、compat、release-gate。
...
Phase 2 注记：当前已实现 foundation `AstraScene`：`ActorWorld`、stable Actor/Component DTO、`ComponentDescriptor`、generation-safe `ActorHandle`、spawn/activate/deactivate/destroy、foundation component descriptors、JSON component data、world snapshot/restore、missing actor reference diagnostics、headless local ECS pack 和 private EnTT-backed storage。EnTT 是 private implementation detail，不进入 public ABI、save ID、Editor/MCP contract 或 authoring model。完整 prefab/variant、deferred destroy、preview attach、component migration hardening 和 production inspector/debugger 仍属后续 completion。
...
Phase 2 注记：当前已实现 foundation Runtime module：`RuntimeWorld`、frame/fixed-step counters、RuntimeEvent DTO、immediate/queued/deferred EventBus、trace、actor-bound state-machine transition、ControlPolicy allow/queue/reject foundation、Director state、foundation save/load、RuntimeReplay DTO 和 deterministic stable hash validation。完整 scheduler/coroutine、guard/action API、priority/subscription lifetime、presentation extraction、production replay mismatch 定位和 full Director arbitration 仍属后续 completion。

- [x] Runtime：定义 deterministic tick model：fixed update、variable presentation update、frame index、event sequence。（Runtime-only completion 覆盖 `RuntimeTickInput`、`RuntimeFrameResult`、frame/event sequence、scheduler completion 和 stable hashes；Editor debugger presentation viewers 后续完成。）
- [x] Runtime：实现 RuntimeEvent：type id、category、source/target、sequence、timestamp policy、payload schema。
- [x] Runtime：实现 EventBus：immediate、queued、deferred、priority、subscription lifetime、trace hook。（Phase 5 覆盖 priority ordering、subscription lifetime 和 target-aware dispatch。）
...
- [x] Runtime：实现 ControlPolicy：owner、channel、lock、interrupt、queue、reject、priority inheritance。（Phase 5 覆盖 interrupt policy 和 priority inheritance foundation。）
- [x] Runtime：实现 Director：global phase、timeline lock、choice lock、AI permission window、conflict arbitration.（Runtime-only completion 覆盖 `DirectorArbitrationRequest/Result`、queue/allow decision 和 arbitration log；AI MCP host 后续完成。）
- [x] Runtime：实现 task/coroutine scheduler：cancellable task、wait event、wait time、wait asset、save state。（Phase 5 使用 serializable task DTO，不暴露 C++ coroutine frame。）
- [x] Runtime：实现 replay recorder/player：input/event/script/presentation hash comparison。（Phase 5 覆盖 replay checkpoints 和 hash mismatch localization；深度 UI diff 后续完成。）
- [x] Runtime：实现 RuntimeWorld facade，组合 Scene、EventBus、StateMachine、Director、Scheduler。
...
- [x] Runtime：按 `docs/design/runtime-production-contract.md` 实现 `RuntimeTickInput`、`RuntimeFrameResult`、serializable scheduler、Director arbitration 和 Actor lifecycle production contract。（Runtime-only evidence 已覆盖 tick result、scheduler wake、deferred destroy flush、Director arbitration 和 RuntimeStress；Editor preview tooling 后续完成。）
...
Phase 2 注记：当前已实现 foundation save/load/replay：`astra.runtime.snapshot.v1` `VersionedDocument`、`astra.runtime.replay.v1` DTO、frame/fixed-step/event sequence、seed、world/actor/component/state-machine/blackboard/control-policy/director snapshot、runtime event trace 和 deterministic stable hash validation。生产级 save header、module/schema version matrix、script/timeline/filter/resource/AI/module extension state、migration/integrity/compression 和 replay mismatch report 仍属后续 completion。

- [x] Save：定义 save container：header、engine version、project version、module versions、schema versions。（Phase 5 实现 `astra.runtime.save_container.v2` JSON section manifest。）
- [x] Save：保存 World、Scene、Actor、Component、StateMachine、Blackboard、ControlPolicy、Director。
- [x] Save：保存 ScriptRuntime state、Timeline state、FilterProfile state、resource overrides、random seed。（Phase 5 保存 JSON section placeholders 和 random seed；provider payload 深化后续完成。）
- [x] Save：保存 AI committed output 和 runtime intent audit reference。（Phase 5 保存 committed-output section placeholder；Runtime AI provider 后续填充。）
- [x] Save：支持 module extension state，Legacy 仅作为 expansion extension state。（Phase 5 保存 module extension section placeholder。）
- [ ] Save：实现 migration：snapshot schema migration、component migration、module state migration。
- [x] Save：实现 integrity：hash、compression option、diagnostics、partial failure policy。（Phase 5 覆盖 section hash 和 optional zstd compression；provider-specific partial recovery 后续扩展。）
- [x] Replay：记录 input、runtime events、script decisions、choice selections、committed AI output。（Phase 5 DTO 覆盖 input/script/choice/AI streams 和 checkpoints。）
- [x] Replay：实现 deterministic comparison：state hash、event hash、presentation hash。
- [x] Save/Replay：按 `docs/design/save-replay-production-contract.md` 实现 section provider、migration edge、replay stream 和 mismatch localization contract。（Runtime-only completion 覆盖 section descriptors、migration edge DTO、ReplayStream DTO 和 mismatch localization evidence；external module `ISaveSectionProvider` plugin registration 后续扩展。）
...
Phase 6/7 注记：当前已实现 `AstraAsset` production Asset Pipeline slice：asset URI/ID 解析、VFS mount、sidecar DTO/validation、registry scan、dependency diagnostics、import preset/project template/review item DTO、watch invalidation plumbing、`ImportRequest`/`IAssetImporter`、`CookRequest`/`ICookProcessor`、built-in image/audio/font/text/filter/script/timeline importers and cook processors、local DDC write/reuse/corruption recovery、binary `.astrapkg` writer/reader、zstd payload compression、manifest/payload hash integrity diagnostics、random-access/chunked payload read、read-only package mount policy、hot reload rollback DTO、Asset Release Gate blockers，以及 NativeVN package-only save/replay package hash 和 Phase 7 media provider evidence。
...
- [x] Asset：实现 VFS：directory mount、package mount、read-only mount、mount priority、diagnostics。（Phase 3 foundation 覆盖 directory mount/resolve/read-only/priority，并通过 `PackageReader::MountPackage()` 输出 package mount DTO；VFS 内联 package payload resolver 后续完成。）
...
- [x] Asset：实现 importer framework：image、audio、font、text、filter profile、script、timeline。（Phase 6 built-in importer descriptors and generic importer cover production source/sidecar import; richer per-format authoring UX is Phase 9。）
- [x] Asset：实现 cooker framework：source hash、settings hash、derived data key、output manifest。（Phase 6 uses SHA-256 source/sidecar hashes, provider feature hash and DDC keys through `CookAssetRegistry`。）
- [x] Asset：实现 DerivedDataCache：local cache、clean policy、versioning、corruption recovery。（Phase 6 covers local cache, versioned keys and corruption recovery evidence; policy cleanup can deepen during hardening。）
- [x] Asset：实现 package manifest：asset table、dependency table、module table、hash、release profile。（Phase 6 覆盖 cook artifact table、dependency evidence、module table、binary zstd payload table、package hash/profile 和 provider feature hash。）
- [x] Asset：实现 package reader：streaming read、random access、diagnostics、mount.（Phase 6 覆盖 binary `.astrapkg` manifest/header/hash/payload integrity diagnostics、random-access read、chunked read、text read 和 read-only mount DTO。）
- [x] Asset：实现 hot reload：asset/script/filter/timeline invalidation and rollback。（Phase 6 covers source/sidecar invalidation planning and rollback DTO; provider resource switching deepens in Phase 7/10。）
...
- [x] Asset：按 `docs/design/asset-pipeline.md` 实现 Importer、Cooker、DDC、Package Manifest、Hot Reload 和 Asset Release Gate contract。（Phase 6 production Asset Pipeline slice；Phase 7 media execution evidence consumes package payloads。）
- [x] Asset：按 `docs/design/asset-package-production-contract.md` 实现 `ImportRequest`、`CookRequest`、`CookArtifactDescriptor`、`DdcKey`、`PackagePayloadRef`、`PackageMountPolicy` 和 hot reload rollback。
- [x] Docs：编写 Asset Authoring Guide、Importer/Cooker Guide、Package Format Reference。（Phase 6 manual/design pages cover binary `.astrapkg`、DDC、release gate and hot reload semantics.)
...
Phase 3/7 注记：当前已实现 `AstraMedia` foundation and Phase 7 backend evidence slice：PresentationCommand、RenderGraph/text/audio/filter DTO、FilterProfile validation/application、Renderer2D/TextLayout/Audio foundation provider descriptors、Image/Audio/Video decode provider slots、Timeline 和 FilterGraph provider slots、mature backend capability probe（SDL3、libpng、libjpeg-turbo、libwebp、FreeType、HarfBuzz、miniaudio、FFmpeg）、PNG/JPEG/WebP image metadata/RGBA decode API、decoded CPU texture buffer import、glyph-run/atlas token capture、logical miniaudio mixer/bus capture、video decode extension-point diagnostics、Timeline camera/audio/filter state、media release-gate production validation、HeadlessRenderer2D deterministic capture/hash 和 Phase 7 CLI/sample evidence。PCM sample decode、GPU shader execution、video frame decode、native GPU/audio device verification 和 per-driver visual/audio diff hardening remain later work。
...
- [x] Media：实现 image decode/cook path：PNG/JPEG/WebP policy、texture format、atlas option。（Phase 7 adds decoded CPU texture buffer import and production provider evidence；platform-specific GPU texture residency remains private backend detail。）
...
- [x] Media：实现 video extension point，不阻塞 core runtime acceptance。（Phase 7 adds `astra.video_decode` slot, FFmpeg provider descriptor, inspect/decode-frame API, and deterministic placeholder fallback where no redistributable video fixture exists。）
...
- [x] Media：实现 Timeline/Animation：keyframes、events、camera, easing、save/replay state。（Phase 7 adds canonical timeline schema/API and NativeVN timeline evidence for camera/audio/filter state。）
...
- [x] Media：按 `docs/design/hardware-media-decode.md` 实现独立 `astra.image_decode`、`astra.audio_decode`、`astra.video_decode` provider slot、capability negotiation、fallback 和 release gate。
- [x] Media：按 `docs/design/media-backend-production-contract.md` 实现 Renderer2D/TextLayout/Audio/Timeline/FilterGraph provider 执行 contract。（Current CI execution uses deterministic headless hashes and logical captures; native driver visual/audio diff remains hardening。）
...
Phase 8 注记：当前已实现 `AstraScript` completion slice：`ScriptRuntimeHost`、PEGTL-backed production `.astra` parser、统一 command schema registry、command manifest、AST/StateGraph/Narrative/Effect IR、source map、debug-symbol DTO、debug step、hot reload compatibility/rollback report、Lua extension schema sandbox、`ScriptSnapshot v2`、`ScriptEventBridge` 和 NativeVN Phase 8 evidence。Lua-as-AstraVN-source 仍需 source map、debug symbols、hot reload、save/replay 和 ScriptParity evidence；真实 Live2D/Emote SDK、Editor graph/timeline UI 和 AI/MCP 仍属后续阶段。
...
- [x] Script：实现 Lua runtime foundation：extension schema sandbox、host API binding 和 forbidden API blocking。（当前是 Lua foundation evidence，不代表 Lua-as-AstraVN-source 完成。）
- [ ] Script：实现 Lua-as-AstraVN-source：Lua-authored VN/system script、source map、debug symbols、hot reload report、snapshot/save/replay state 和 release-gate packaged eligibility。
...
- [x] Script：按 `docs/design/script-and-presentation.md` 实现 ScriptRuntimeHost、Native DSL、Graph/Timeline foundation 和 PresentationCommand contract。（Phase 8 覆盖 embedded graph/timeline IR 和 Lua extension schema；Lua-as-AstraVN-source 与 Editor UI 后续。）
...
- Native Script 和 Lua-as-AstraVN-source 可驱动同一 VN scene，并产生同等 runtime events。
...
- [x] AstraVN：定义预置 Actor：Scene、StoryDirector、DialogueSystem、ChoiceSystem、AudioSystem、FilterSystem、Character、Camera。（Phase 4 foundation 覆盖固定 `NativeVN` validation preset。）
...
- [ ] Editor：按 `docs/design/editor-runtime-creator-contract.md` 实现 `EditorRuntimeSession`、`InspectRequest`、`DebugCommand`、`PreviewOverlay` 和 `SourcePatchProposal` contract。
...
- [ ] Customization：按 `docs/design/provider-contracts.md` 实现 provider descriptor、capability negotiation、selection policy、shutdown contract 和 permission audit。
...
- [ ] AI：按 `docs/design/ai-mcp-safety-contract.md` 实现 `AIIntent`、`IntentValidationResult`、`CommittedAIOutput`、`GenerationAuditRecord` 和 `ReviewQueueItem` contract。
...
Phase 1/3/4/5/6/7/NativeVN evidence 注记：`AstraTools` 和 `astra` 已实现 `validate`、`import`、`cook`、`package`、`run --backend headless`、`replay --compare`、`inspect`、`doc-check`，并使用 CLI11、yaml-cpp、nlohmann_json、spdlog、OpenSSL SHA-256、zstd、Lua、sol2 和 CTest/Catch2 提供证据。Core structured logging 已实现 `astra.log.event.v1`、async rotating JSONL、console sink、memory capture、recent-log ring、diagnostic mirroring 和 CLI log flags。Phase 7 sample validate/headless validation 会输出 Asset/Media/FilterGraph hash、media backend capability report、image decode validation、media provider release-gate evidence、media provider/decode/timeline/filter evidence、binary package evidence；NativeVN evidence slice 会输出 source asset sidecars、Script/AstraVN headless evidence、cook/package manifests、local DDC artifact evidence、PackageReader payload/mount validation、package manifest hash/provider feature hash save-replay evidence、engine/plugin DLL hash evidence 和 golden replay comparison。下列条目描述的是完整 UE-class Tools/Release Gate/Observability 目标；trace/crash bundle 和 deeper per-driver UI/audio replay diff 仍未完成。
...
- [x] Tools：实现 `AstraGame` launcher：launch cooked package、headless run、scripted input。（当前覆盖 NativeVN package headless/windowed scripted playable evidence；production launch harness 后续完成。）
- [x] Tools：实现 `AstraGame` QA：player automation test plan、player action、explicit RuntimeEvent injection、JSON Pointer assertions 和 `astra.test.player_report.v1`。（当前覆盖 NativeVN Tools slice；真实设备输入、Editor PIE、AI/MCP、perceptual diff 和 long soak 后续完成。）
- [x] Tools：实现 `AstraGame` session reports：run replay, compare hashes, emit mismatch report。（Phase 5/6 covers frame/record/source-object/package manifest hash mismatch localization for current replay evidence；deeper UI diff remains later。）
- [x] Tools：实现 `astra inspect`：package info、asset registry、module table、save file summary。（当前覆盖 package manifest、payload validation 和 mount DTO；save/replay deep inspect 后续完成。）
...
- [x] Release Gate：实现 blocking/non-blocking diagnostics policy。（Runtime-only `astra release-gate` 输出 `astra.release.report.v1` 和 deterministic blocking policy。）
- [x] Release Gate：实现 plugin ABI、permissions、packaged eligibility、binary hash checks。（Runtime-only evidence 复用 Module/Asset release gate 和 package module binary hash；full provider wizard/Editor plugin workflow 后续完成。）
- [x] Observability：实现 structured logging、JSONL file output、console routing、diagnostic mirroring 和全组件 foundation workflow 日志。
- [x] Observability：实现 profiler export、trace capture、frame timing、asset load timing、script timing。（Runtime-only release gate 输出 trace event schema evidence；interactive profiler viewer 后续完成。）
- [x] Observability：实现 crash/error report bundle：logs、diagnostics、build info、last frames。（Runtime-only release gate 输出 crash bundle DTO evidence；platform minidump hardening 后续完成。）
- [x] Tools：按 `docs/design/tools-release-observability.md` 实现 CLI output、validation report、package report、release gate report、trace 和 crash bundle contract。
- [x] Tools：按 `docs/design/release-gate-observability-contract.md` 实现 `ReleaseReport`、`BlockingPolicy`、`TraceEvent` 和 `CrashBundle` production report contract。（Runtime-only DTO/report evidence；Editor UI viewers 后续完成。）
...
- [ ] Sample：`PackageLaunch`：只从 cooked package 启动。
- [ ] Sample：`ScriptParity`：Native Script、Lua-as-AstraVN-source、Graph 和 Timeline 产生同等事件流、save/replay/debug evidence。
- [x] Sample：`MediaBackend`：真实 renderer/text/audio/filter output。（Runtime-only tests cover production provider capture, CPU RGBA FilterGraph, text/audio capture, and driver diff report through `AstraPhaseTests`/release gate evidence。）
...
- [x] Test：NativeVN player automation CTest：`AstraGame` QA success plan、bad assertion 和 bad runtime event diagnostics。
...
- [x] Acceptance：`NativeVN` 从 source content 完成当前 Windows/headless validate、cook、package。（Phase 6 已通过 binary `.astrapkg`；full cross-platform UE-class gate 仍需全平台完整游玩、Editor、AI 和 customization hardening。）
- [x] Acceptance：packaged `NativeVN` 在当前 Windows/headless 无 Editor 环境 launch、play、choice、save、load、replay。（Phase 8 playable demo 已通过 headless/windowed scripted evidence；全平台完整游玩、save migration UI 和更深 replay localization 后续完成。）
- [ ] Acceptance：`NativeVN` 在 Windows、Linux、macOS、iOS、Android、Web 上完成 full-playable package playthrough、save/load、replay、inspect、release-gate。
- [ ] Acceptance：converted `TsuiNoSora` 使用用户本地原始数据生成普通 AstraVN `Content`，并在 Windows、Linux、macOS、iOS、Android、Web 上完成 full-playable packaged playthrough。（仓库只提供转换器、patch schema、文档和本地验收路径。）
- [ ] Acceptance：Lua-as-AstraVN-source 与 Native DSL/Graph/Timeline 具备等价 runtime event、presentation、save/replay、debug 和 hot reload evidence。
...
- [x] Acceptance：runtime 支持 diagnostics、profiling、trace、crash/error report。（Runtime-only release gate outputs trace and crash bundle DTO evidence。）
...
- [x] Acceptance：stress/soak/migration/replay/package tests 通过。（Runtime-only stress/replay/package CTest coverage passes; long-run soak farm remains future hardening if required by release profile。）
...
目标：在 native runtime 达标后交付独立 AstraEmu Toolkit，用于旧 VN 模拟、增强和翻译；它不进入 NativeVN 制作流程，也不反向污染 Core。首批目标使用用户本地合法原始发行数据，并以可提交的 `astra.emu.local_case_report.v1` 记录功能已在目标作品上测试。

- [ ] Expansion：定义 AstraEmuManager、CompatContentProbe、Compat Core descriptor 和 capability set。
- [ ] Expansion：定义 `ICompatRuntimeProvider`、package patch script/result、Legacy VM state、opcode/timeline adapter、LegacyApiMapper 和 coverage report。
- [ ] Expansion：定义 local mount reader、save-state、core cold-swap、Runtime Inspector 和 `astra.emu.local_case_report.v1`。
- [ ] Expansion：定义 enhancement profile、TextCaptureEvent 和 translation Provider bridge。
- [ ] Expansion：实现 package patch script sandbox、hash、reader/index/decode/resource-map 和 protected-content blocking diagnostics。
- [ ] Expansion：实现 Artemis core for `Sakura no Uta 10th Anniversary Edition`：original-release probe、package patch、`foreign-artemis:/` resolver、`.iet/.asb/.ast/.ipt/.sli/.tbl` index、system Lua、minimal `e:*` host API、tag executor 和 100% full-content-flow report。
- [ ] Expansion：实现 KrKr/KAG/TJS/XP3 core for `Senren Banka`：XP3 package patch、KAG executor、TJS required host API、system UI/save/config bridge 和 100% full-content-flow report。
- [ ] Expansion：实现 BGI/Ethornell core for `Subarashiki Hibi 15th Anniversary Edition`：archive package patch、scenario/system script VM、mapper、save/replay 和 100% full-content-flow report。
- [ ] Expansion：实现 mount-only local content diagnostics。
- [ ] Expansion：按 `docs/design/legacy-compatibility-contract.md` 实现 `AstraEmuManager`、`ICompatRuntimeProvider`、`CompatPackagePatchResult`、`CompatCoverageReport`、`LegacyVmSnapshot`、TextCapture 和 local case report boundary contract。
- [ ] Docs：编写 AstraEmu Toolkit Guide、Package Patch Authoring Guide、Enhancement Guide、Compat Core Authoring Guide、Local Commercial Case Report Guide。

验收：

- Artemis、KrKr、BGI cores 输出 AstraVN Events / PresentationCommand，不把 legacy VM control flow 并入 AstraVN source language。
- Package patch script 可处理目标作品所需原始发行数据结构，但不绕过 DRM 或商业保护。
- `Sakura no Uta 10th Anniversary Edition`、`Senren Banka`、`Subarashiki Hibi 15th Anniversary Edition` 均有 100% full-content-flow local case report。
