...
真实 media execution backend、Editor、AI/MCP、Legacy、完整 Artemis VM compatibility 和 full runtime release gate
...
Phase 1/6 注记：当前已建立 dynamic-only `Astra*` runtime/tool DLL targets、示例插件、Phase 1 tests、构建选项边界、`AstraTools` 和 `astra` runtime evidence CLI。`validate`、`import`、`cook`、`package`、`run --headless-smoke`、`replay --compare`、`inspect`、`doc-check` 已覆盖 Phase 0/1 文档、Foundation smoke、`foundation_core_gate`、engine/plugin binary hash evidence 和 NativeVN binary `.astrapkg` package/replay evidence；real media execution backend、full runtime release gate、trace/crash bundle 仍归后续 Tools/Samples 阶段。
...
- [ ] Runtime：按 `docs/design/runtime-production-contract.md` 实现 `RuntimeTickInput`、`RuntimeFrameResult`、serializable scheduler、Director arbitration 和 Actor lifecycle production contract。
...
- [ ] Save/Replay：按 `docs/design/save-replay-production-contract.md` 实现 section provider、migration edge、replay stream 和 mismatch localization contract。
...
Phase 6 注记：当前已实现 `AstraAsset` production Asset Pipeline slice：asset URI/ID 解析、VFS mount、sidecar DTO/validation、registry scan、dependency diagnostics、import preset/project template/review item DTO、watch invalidation plumbing、`ImportRequest`/`IAssetImporter`、`CookRequest`/`ICookProcessor`、built-in image/audio/font/text/filter/script importers and cook processors、local DDC write/reuse/corruption recovery、binary `.astrapkg` writer/reader、zstd payload compression、manifest/payload hash integrity diagnostics、random-access/chunked payload read、read-only package mount policy、hot reload rollback DTO、Asset Release Gate blockers，以及 NativeVN package-only save/replay package hash evidence。真实 renderer upload、font atlas execution、audio mixer playback 和 GPU filter execution 仍属 Phase 7 completion。
...
- [x] Asset：按 `docs/design/asset-pipeline.md` 实现 Importer、Cooker、DDC、Package Manifest、Hot Reload 和 Asset Release Gate contract。（Phase 6 production Asset Pipeline slice；Phase 7 media execution remains separate。）
...
Phase 3 注记：当前已实现 `AstraMedia` foundation slice：PresentationCommand、RenderGraph/text/audio/filter DTO、FilterProfile validation/application、Renderer2D/TextLayout/Audio foundation provider descriptors、mature backend capability probe（SDL3、libpng、libjpeg-turbo、libwebp、FreeType、HarfBuzz、miniaudio）、PNG/JPEG/WebP image metadata inspect API、libpng image decode smoke、image cook artifact metadata、media release-gate foundation validation、HeadlessRenderer2D deterministic capture/hash 和 SDL renderer factory private compile-path stub。真实 decoded texture upload、image cook binary transform/atlas output、font atlas/shaped glyph execution、audio playback/mixing execution、GPU filter execution、timeline runtime 和 production provider replacement 仍属 Phase 7/10 completion。
...
- [ ] Media：实现 image decode/cook path：PNG/JPEG/WebP policy、texture format、atlas option。（当前已通过 libpng/libjpeg-turbo/libwebp 建立格式 capability probe、metadata inspect API、libpng decode smoke 和 cook artifact metadata；decoded texture upload、atlas/cook binary artifact 生成后续完成。）
...
- [ ] Media：按 `docs/design/hardware-media-decode.md` 实现独立 `astra.image_decode`、`astra.audio_decode`、`astra.video_decode` provider slot、capability negotiation、fallback 和 release gate。
- [ ] Media：按 `docs/design/media-backend-production-contract.md` 实现 Renderer2D/TextLayout/Audio/Timeline/FilterGraph provider 执行 contract。
...
Phase 1/3/4/5/6/NativeVN evidence 注记：`AstraTools` 和 `astra` 已实现 `validate`、`import`、`cook`、`package`、`run --headless-smoke`、`replay --compare`、`inspect`、`doc-check`，并使用 CLI11、yaml-cpp、nlohmann_json、OpenSSL SHA-256、zstd、Lua、sol2 和 CTest/Catch2 提供证据。Phase 6 sample validate/headless smoke 会输出 Asset/Media/FilterGraph hash、media backend capability report、image decode smoke、media provider release-gate foundation evidence、binary package evidence；NativeVN evidence slice 会输出 source asset sidecars、Script/AstraVN headless evidence、cook/package manifests、local DDC artifact evidence、PackageReader payload/mount smoke、package manifest hash/provider feature hash save-replay evidence、engine/plugin DLL hash evidence 和 golden replay comparison。下列条目描述的是完整 UE-class Tools/Release Gate/Observability 目标；trace/crash bundle、real media execution backend 和 deeper UI replay diff 仍未完成。
