...
Phase 1 注记：当前已建立 dynamic-only `Astra*` runtime/tool DLL targets、示例插件、Phase 1 tests、构建选项边界、`AstraTools` 和 `astra` runtime evidence CLI。`validate`、`cook`、`package`、`run --headless-smoke`、`replay --compare`、`inspect`、`doc-check` 已覆盖 Phase 0/1 文档、Foundation smoke、`foundation_core_gate`、engine/plugin binary hash evidence 和 NativeVN source-sidecar package/replay evidence；真实 binary asset transforms、real media execution backend、full runtime release gate、trace/crash bundle 仍归后续 Tools/Samples 阶段。
...
Phase 3 注记：当前已实现 `AstraAsset` foundation slice：asset URI/ID 解析、VFS mount、sidecar DTO/validation、registry scan、dependency diagnostics、import preset/project template/review item DTO、watch invalidation plumbing、NativeVN DDC metadata DTO、local DDC artifact write/reuse/corruption recovery evidence、embedded package payload table、PackageReader manifest/hash/payload integrity diagnostics、random-access/chunked payload read 和 package mount DTO。真实 media transform importer/cooker、binary package container streaming、production hot reload rollback 和完整 Asset Release Gate 仍属 Phase 6 completion。
...
- [x] Asset：实现 package manifest：asset table、dependency table、module table、hash、release profile。（NativeVN evidence slice 覆盖 cook artifact table、dependency evidence、module table、embedded payload table、package hash 和 deterministic profile；production binary package table 后续完成。）
- [x] Asset：实现 package reader：streaming read、random access、diagnostics、mount.（NativeVN evidence slice 覆盖 JSON package payload random-access read、text read、mount DTO、payload hash/size/encoding diagnostics；binary streaming container 后续完成。）
...
- [ ] Asset：按 `docs/design/asset-package-production-contract.md` 实现 `ImportRequest`、`CookRequest`、`CookArtifactDescriptor`、`DdcKey`、`PackagePayloadRef`、`PackageMountPolicy` 和 hot reload rollback。
...
Phase 1/3/4/NativeVN evidence 注记：`AstraTools` 和 `astra` 已实现 `validate`、`cook`、`package`、`run --headless-smoke`、`replay --compare`、`inspect`、`doc-check`，并使用 CLI11、yaml-cpp、nlohmann_json、OpenSSL SHA-256/base64、Lua、sol2 和 CTest/Catch2 提供证据。Phase 3 sample validate/headless smoke 会输出 Asset/Media/FilterGraph hash、media backend capability report、image decode smoke 和 media provider release-gate foundation evidence；NativeVN evidence slice 会输出 source asset sidecars、Script/AstraVN headless evidence、cook/package manifests、local DDC artifact evidence、PackageReader payload/mount smoke、engine/plugin DLL hash evidence 和 golden replay comparison。下列条目描述的是完整 UE-class Tools/Release Gate/Observability 目标；production release gate、trace/crash bundle、real media execution backend 和 richer replay mismatch localization 仍未完成。
