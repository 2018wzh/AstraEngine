...
- AstraEMU Family ABI 当前 hard cut 为 v6：所有 dylib lifecycle/VFS 调用使用显式 `StableAbi` wire DTO，大块 scene、encoded media 与 PCM 使用 ABI-owned bulk buffer；v5 binary、fingerprint 和 FVP runtime snapshot 必须 fail-fast。Manager 与 native CLI 的 legacy audio 统一由 PlatformHost-backed bounded worker 补水，不能再由 fixed tick 或 Slint event loop 驱动，也不能保留 Manager 私有 CPAL 链。
