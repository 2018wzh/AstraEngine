...
- Headless 正式性能门禁只接受 `astra.headless_host_profile.v3` 的精确 GPU policy、clean Release build 和同 build/package/profile/input identity。集显与独显报告必须分开；软件 adapter、timestamp query 缺失、trace 丢失/截断/回退、资源计数不闭合或预算 blocked 都立即失败。性能 trace 使用 host-owned Perfetto Trace Event writer；分析端只使用外部 `perfetto-mcp`，不得修改 `astra-mcp`，也不得把第三方 MCP、原始 trace、设备名或本地路径写入仓库。
