...
- AstraEMU Family ABI 当前 hard cut 为 v13：所有 dylib lifecycle、VFS、surface、Hook、writable-file、system-menu、confirmation 与 typed system-command 调用使用显式 `StableAbi` wire DTO；v7/v8/v9/v10/v11/v12 binary、fingerprint 与旧 runtime snapshot 必须 fail-fast，不保留 compatibility shim。descriptor 只接受 `Native + MultiLayer` 与 `Ported + SingleLayer`。family 通过 Host-owned writable surface lease 直接绘制并提交 `Unchanged`、`Full` 或像素坐标 `Rects` damage；provider step 和整笔 retained Layer2D transaction 验证成功后才能公开 staged generation，失败时整批回滚。Family-owned system menu 和 one-shot confirmation 通过有界 transaction 非阻塞发布，Host 只显示层级或调用平台 native confirmation provider，并回送 `Select`/`Dismiss`/`Accepted`/`Cancelled`；菜单窗口/帮助动作使用 typed system-command，Host 只能调用当前平台已绑定能力并回送 `Applied`/`Rejected`/`Unsupported`，不得按 item 名称、确认文本或通用事件字符串解释语义。Manager 的宿主界面使用 Slint，桌面 CLI 使用 `astra-platform` 的平台 provider，Headless 只用序列化物理输入；确认和 system-command 期间 gameplay input 直接阻断，结果只能在后续 fixed step 生效。Manager 与 native CLI 的 legacy audio 统一由 PlatformHost-backed bounded worker 补水，不能再由 fixed tick 或 Slint event loop 驱动，也不能保留 Manager 私有 CPAL 链。Windows/Linux/macOS host 的 gamepad 使用独立 bounded worker + `EventLoopProxy`/host wake，生产渲染、解码和输入路径不得由 fixed polling timer 推进；无 hotplug handle 时才允许显式低频 discovery wait。
...
- AstraEMU v1 首发 family 是 FVP；固定 rfvp revision 的合法输入可观察行为、完整 syscall coverage、原生存档冷启动与脱敏 parity report 是 FVP release gate。Artemis 和其他 family 以后续 probe report 接入，不能阻塞 EngineCore、AstraVN、Editor 和六平台 gate。默认 auto-probe 顺序保持 KrKr、Artemis、BGI、Siglus、SoftPAL、FVP、Musica，显式 case profile 始终优先。
- Musica 不提供 GARbro scheme importer 或自动 key 恢复。`key_file` 只接受严格 `astra.emu.musica.keys.v1`，路径必须相对游戏根并通过 private-file reader 的大小、文件类型、symlink 与 traversal 校验；缺 role、未知字段、非法 key 或 CP932 不可编码值都必须阻断。
...
- 通用 VFS 命令固定为 `astra-emu-cli vfs --family <family>`，family 专用研究命令放独立 CLI。Musica 从 launch profile 指定的相对 `key_file` 读取严格 `astra.emu.musica.keys.v1`，只使用 family-owned 纯 Rust 流式解密；不得建立明文 cache、Luau callback、自动 key importer 或任何 fallback。
