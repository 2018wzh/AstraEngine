...
- Minori 不提供 GARbro scheme importer 或自动 key 恢复。`key_file` 只接受严格 `astra.emu.minori.keys.v1`，路径必须相对游戏根并通过 private-file reader 的大小、文件类型、symlink 与 traversal 校验；缺 role、未知字段、非法 key 或 CP932 不可编码值都必须阻断。
