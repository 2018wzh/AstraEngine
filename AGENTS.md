...
- Family ABI v7 的实时 scene/audio 使用 typed DTO 和 ABI-owned allocation；RGBA8/PCM 在 live 路径只移动所有权，不经 postcard、JSON 或 content hash envelope。NativeVN 产品音频同样只能走 typed live cue；Shipping 热组件只移动一次 owned postcard allocation 并使用 disabled marker，Evidence 才计算认证 digest。VFS range 只绑定 revision、offset、length 和 bounds，result 不计算 per-read content hash；Evidence 需要 hash 时在提交后异步完成，不能进入 Runtime tick 预算。
...
- AstraEMU Family ABI 当前 hard cut 为 v7：所有 dylib lifecycle/VFS 调用使用显式 `StableAbi` wire DTO，大块 scene、encoded media 与 PCM 使用 ABI-owned typed bulk buffer；v5/v6 binary、fingerprint 和 FVP/Minori runtime snapshot 必须 fail-fast。Manager 与 native CLI 的 legacy audio 统一由 PlatformHost-backed bounded worker 补水，不能再由 fixed tick 或 Slint event loop 驱动，也不能保留 Manager 私有 CPAL 链。Windows/Linux/macOS host 的 gamepad 使用独立 bounded worker + `EventLoopProxy`/host wake，生产渲染、解码和输入路径不得由 fixed polling timer 推进；无 hotplug handle 时才允许显式低频 discovery wait。
