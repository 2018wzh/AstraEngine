...
全量测试必须在当前 checkout 的新鲜构建身份下执行。若 `cargo test --workspace` 受到其他 worktree 的旧测试二进制、动态插件或 manifest 常量污染，不能把结果当作当前提交的通过/失败结论；应切换独立 `CARGO_TARGET_DIR`、重新构建后再判断。命令超时、局部测试通过、fixture 通过或静态 report 生成，都不能替代完整 workspace 和真实 host evidence。
