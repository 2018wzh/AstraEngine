...
- AstraEMU Family ABI 当前 hard cut 为 v12：所有 dylib lifecycle、VFS、surface、Hook、writable-file、system-menu 与 confirmation 调用使用显式 `StableAbi` wire DTO；v7/v8/v9/v10/v11 binary、fingerprint 与旧 runtime snapshot 必须 fail-fast，不保留 compatibility shim。descriptor 只接受 `Native + MultiLayer` 与 `Ported + SingleLayer`。family 通过 Host-owned writable surface lease 直接绘制并提交 `Unchanged`、`Full` 或像素坐标 `Rects` damage；provider step 和整笔 retained Layer2D transaction 验证成功后才能公开 staged generation，失败时整批回滚。Family-owned system menu 和 one-shot confirmation 通过有界 transaction 非阻塞发布，Host 只显示层级或调用平台 native confirmation provider，并回送 `Select`/`Dismiss`/`Accepted`/`Cancelled`，不得按 item 名称或确认文本解释语义。Manager 的宿主界面使用 Slint，桌面 CLI 使用 `astra-platform` 的平台 provider，Headless 只用序列化物理输入；确认期间 gameplay input 直接阻断，结果只能在后续 fixed step 生效。Manager 与 native CLI 的 legacy audio 统一由 PlatformHost-backed bounded worker 补水，不能再由 fixed tick 或 Slint event loop 驱动，也不能保留 Manager 私有 CPAL 链。Windows/Linux/macOS host 的 gamepad 使用独立 bounded worker + `EventLoopProxy`/host wake，生产渲染、解码和输入路径不得由 fixed polling timer 推进；无 hotplug handle 时才允许显式低频 discovery wait。
