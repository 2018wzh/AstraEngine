...
| AstraEMU | 旧 VN manager、engine-native family plugin、Artemis v1 family、Luau patch/decode runtime |
...
- AstraEMU 使用 Manager + AstraEngine RuntimeWorld + in-process family plugin 架构。旧引擎语义通过 StateMachine action provider、VFS/archive provider、legacy script provider、media mapper 和 snapshot codec 进入 Runtime；family plugin 不能替换 Runtime tick、MutationLog、Save container 或 Release Gate core checks。
