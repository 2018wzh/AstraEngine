...
def build_sequence() -> Sequence:
    ...
    sequence.await_value("vn.pending_wait_command", "tsui.command.014322", 7200)
    ...
    sequence.await_value("vn.focused_semantic_id", "root/commands/skip_all")
    ...
    sequence.key("Tab")
    sequence.await_value("vn.focused_semantic_id", "root/commands/config")
    sequence.key("Enter")
    sequence.await_value("vn.system_page", "config")
    sequence.await_value("vn.focused_semantic_id", "root/content/master")
    ...
    sequence.key("Tab")
    sequence.key("Tab")
    sequence.await_value("vn.focused_semantic_id", "root/commands/save")
    sequence.key("Enter")
    ...
    sequence.key("Tab")
    sequence.key("Tab")
    sequence.await_value("vn.focused_semantic_id", "root/commands/load")
    sequence.key("Enter")
    ...
    sequence.key("Tab")
    sequence.key("Tab")
    sequence.await_value("vn.focused_semantic_id", "root/commands/backlog")
    sequence.key("Enter")
    ...
    sequence.key("Tab")
    sequence.key("Tab")
    sequence.await_value("vn.focused_semantic_id", "root/commands/auto")
    sequence.key("Enter")
...
def run(arguments: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "checkpoint_artifact_hashes": [item.get("image_hash") for item in checkpoints],
