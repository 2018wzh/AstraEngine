...
def main() -> int:
    ...
    try:
        ...
        copy_notice(
            root / "Emulator" / "Source" / "Families" / SUPPORTED_FAMILIES[args.family][0] / "THIRD_PARTY_NOTICES.md",
            temporary,
            f"{args.family.upper()}_THIRD_PARTY_NOTICES.md",
        )
...
def cargo_build(
    ...
    for line in process.stdout:
        try:
            ...
            continue
        if message.get("reason") == "build-script-executed" and SUPPORTED_FAMILIES[family_id][0] in message.get("package_id", ""):
            candidate = pathlib.Path(message["out_dir"]) / SUPPORTED_FAMILIES[family_id][1]
            descriptor = candidate
    if process.wait() != 0:
        fail("ASTRA_EMU_DESKTOP_CARGO_BUILD_FAILED")
    if descriptor is None:
        candidates = list(target_root.glob(f"{target}/release/build/{SUPPORTED_FAMILIES[family_id][0]}-*/out/{SUPPORTED_FAMILIES[family_id][1]}"))
        if len(candidates) != 1:
            fail("ASTRA_EMU_DESKTOP_FAMILY_DESCRIPTOR_MISSING")
...
def family_library_name(family_id: str, target: str) -> str:
    suffix = SUPPORTED_TARGETS[target][2]
    prefix = "" if suffix == ".dll" else "lib"
    return f"{prefix}astra_emu_{family_id}{suffix}"
...
def copy_notice(source: pathlib.Path, output: pathlib.Path, destination_name: str | None = None) -> None:
    require_file(source, "ASTRA_EMU_DESKTOP_NOTICE_MISSING")
    destination = output / (destination_name or source.name)
    if destination.exists():
        fail("ASTRA_EMU_DESKTOP_NOTICE_COLLISION")
