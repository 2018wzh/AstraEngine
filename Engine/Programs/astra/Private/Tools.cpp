...
CommandReport DocCheck(const CommandOptions& options) {
    ...
    auto command = "powershell -NoProfile -ExecutionPolicy Bypass -File \"" + script.string() +
    ...
    if (result != 0) {
        AddDiagnostic(report, "ASTRA_DOC_CHECK_FAILED", Astra::Core::DiagnosticSeverity::Blocking,
                      "tools/doc-check.ps1 failed.", script);
    }
    return report;
}
