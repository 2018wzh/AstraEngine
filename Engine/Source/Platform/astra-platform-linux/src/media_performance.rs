...
pub const LINUX_MEDIA_PERFORMANCE_BUDGET_ID: &str = "linux.native_media.v1";

pub fn linux_media_performance_budget(
    profile: &PlatformHostProfile,
    product_profile: &str,
) -> Result<PerformanceBudget, PlatformError> {
    if profile.platform != PlatformId::Linux
            ...
            .all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'.' | b'_' | b'-'))
    {
        return Err(PlatformError::new(
            ...
            "native Linux media budget requires a Linux host and safe product profile",
        ));
    }
    Ok(PerformanceBudget {
        schema: PERFORMANCE_BUDGET_SCHEMA.into(),
        budget_id: LINUX_MEDIA_PERFORMANCE_BUDGET_ID.into(),
