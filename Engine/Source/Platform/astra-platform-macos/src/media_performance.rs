...
pub const MACOS_MEDIA_PERFORMANCE_BUDGET_ID: &str = "macos.native_media.v1";

pub fn macos_media_performance_budget(
    profile: &PlatformHostProfile,
    product_profile: &str,
) -> Result<PerformanceBudget, PlatformError> {
    if profile.platform != PlatformId::Macos
            ...
            .all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'.' | b'_' | b'-'))
    {
        return Err(PlatformError::new(
            ...
            "native macOS media budget requires a macOS host and safe product profile",
        ));
    }
    Ok(PerformanceBudget {
        schema: PERFORMANCE_BUDGET_SCHEMA.into(),
        budget_id: MACOS_MEDIA_PERFORMANCE_BUDGET_ID.into(),
