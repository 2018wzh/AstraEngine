...
impl FontEnumerator {
    pub fn new() -> Self {
        // The legacy negative ids describe compatible faces, not redistributable
        // Microsoft font payloads. Use the workspace's pinned OFL fallback until
        // an authorized game font or an installed platform face is resolved.
        let portable_cjk = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../../../Engine/Fixtures/PublicDomainFonts/NotoSansSC-Variable.ttf"
        )) as &[u8];
        let msgothic =
            Font::from_static(portable_cjk).expect("pinned Noto Sans SC fixture must be valid");
        // Cloning a `Font` is a refcount bump, not a deep copy. Keeping separate
        // slots preserves the observable legacy font-face ids without claiming
        // metric identity with proprietary Microsoft fonts.
        let msmincho = msgothic.clone();
        let mspgothic = msgothic.clone();
        let mspmincho = msgothic.clone();
...
impl TextItem {
    fn capture_snapshot_v1(&self) -> TextItemSnapshotV1 {
        TextItemSnapshotV1 {
            ...
            // The RGBA cache is intentionally omitted from save data. Every loaded slot must
            // therefore be rasterized and uploaded after restore, regardless of whether its
            // pre-save cache had already been consumed by the renderer. Canonicalize that
            // derived requirement here so capture -> restore -> capture remains byte-stable.
            dirty: self.dirty || self.loaded,
    ...
    fn apply_snapshot_v1(&mut self, snap: &TextItemSnapshotV1) {
        ...
        // Snapshots no longer duplicate the derived RGBA cache. Only loaded slots own a
        // surface; dimensions may have been configured on an unloaded slot and must not make
        // it loaded as a side effect of restore.
        let (buffer_w, buffer_h) = self.raster_dimensions();
        let expected = (buffer_w as usize)
            .checked_mul(buffer_h as usize)
            .and_then(|v| v.checked_mul(4))
            .unwrap_or(0);
        if snap.loaded {
            self.pixel_buffer = vec![0; expected];
            self.full_buffer = vec![0; expected];
            self.dirty = true;
        } else {
            self.pixel_buffer.clear();
            self.full_buffer.clear();
