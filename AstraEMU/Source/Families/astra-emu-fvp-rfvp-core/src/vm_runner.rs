...
pub struct VmRunner {
    ...
    trace: Vec<VmTraceRecord>,
}

#[derive(Debug, Default, Clone, Copy)]
pub struct VmTraceRecord {
    pub context_id: u32,
    pub pc: u64,
    pub opcode: u8,
}
...
pub struct VmTickReport {
    /// True when the host instruction budget sliced a still-running context.
    /// The context remains resumable on the next deterministic tick.
    pub forced_yield: bool,
    /// Number of contexts sliced by the host budget in this tick.
...
impl VmRunner {
    ...
    fn run_one_context(
        ...
        while !self.tm.get_context_should_break(tid) {
            if let Some(remaining) = self.remaining_instructions.as_mut() {
                if *remaining == 0 {
            ...
            let pc = self.tm.contexts[tid as usize].get_pc();
            let opcode = parser.read_u8(pc)?;
            self.trace.push(VmTraceRecord {
                context_id: tid,
                pc: pc as u64,
                opcode,
            });
